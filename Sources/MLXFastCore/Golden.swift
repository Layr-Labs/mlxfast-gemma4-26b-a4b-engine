...
public struct GoldenDocument: Codable, Equatable {
    ...
    /// Additive provenance metadata (`model_provenance`), carried by the
    /// goldens this fork generates. It pins repository+revision against
    /// `MLXFastConstants.reference*`, which `model_type` does not express, so
    /// it is kept as an ADDITIONAL optional key rather than a replacement.
