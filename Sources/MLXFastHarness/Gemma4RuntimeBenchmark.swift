...
extension Gemma4Runtime {
    public static func benchmark(
        ...
        do {
            ...
            // Qwen loader: `benchmark` is a Qwen-facing entry point, so the
            // golden is held to the model identity the way the reference holds
            // it. The model-agnostic loader is for callers that name no model.
    ...
    static func benchmarkWithWorker(
        ...
        do {
            ...
            // config/tensors here. BenchmarkPreflight.check() calls Gemma4A4BConfig.load,
            // DenseTensorStore, and Gemma4A4BWeightLoader (all EDITABLE MLXFastModel code),
            ...
            progress("golden load start")
            // Qwen loader on the WORKER path too. This branch deliberately does
            // not call BenchmarkPreflight.check (it would run editable model
            // code in the trusted parent), so unlike the in-process branch it
            // has no second, indirect identity check behind it -- the wrapper
            // here is the only thing standing between a foreign-model golden
            // and a scored run.
            let golden = try loadQwenGoldenFixture(from: options.goldenPath)
            progress(
                "golden load complete cases=\(golden.totalCorrectnessCaseCount) "
    ...
    static func measureDecode(
        ...
        var token = try Gemma4Correctness.greedyToken(from: logits)
        try requireBenchmarkMatch(
        ...
        for decodedStep in 0..<timingPlan.decodeSteps {
            ...
            token = try Gemma4Correctness.greedyToken(from: logits)
