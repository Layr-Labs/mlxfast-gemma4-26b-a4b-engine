...
///
/// The `.gemma4A4B` family is THIS track's target
...
public enum SwiftTransform {
    ...
    static func run(
        ...
        case .gemma4A4B:
            try Gemma4A4BCheckpointValidation.validateSelectedTensors(
                ...
                index: index,
                headers: validatedHeaders,
                quantization: Gemma4A4BCheckpointValidation.quantizationSpec(
                    fromConfigRoot: sourceConfigRoot
                )
            )
        case .gemma4:
        ...
        case .qwen35, .gemma4A4B, .laguna:
            ...
            // `Gemma4TextModel` this track scores reads the checkpoint's own
    ...
    static let gemma4A4BExpertCount = 128
    ...
    static func detectModelFamily(
        ...
        if let textConfig = root["text_config"] as? [String: Any] {
            ...
            // ORDER IS LOAD-BEARING. The A4B target and the legacy dense 31B
            // both declare `model_type` "gemma4" at the top level and both
            // nest a `text_config`, so the `return .gemma4` below is
            // unconditional for every Gemma config that reaches it. The MoE
            // discriminator therefore has to run FIRST: falling through would
            // route the A4B checkpoint to the legacy path, which emits the
            // projection and tied-head sidecars and skips tensor validation
            // entirely.
            if isGemma4A4BTextConfig(textConfig) {
                return .gemma4A4B
            }
            return .gemma4
        }
    ...
    static func isGemma4A4BTextConfig(_ textConfig: [String: Any]) -> Bool {
        guard let moeFlag = textConfig["enable_moe_block"] as? NSNumber,
              CFGetTypeID(moeFlag) == CFBooleanGetTypeID(),
              moeFlag.boolValue
        else {
            return false
        }
        guard let experts = textConfig["num_experts"] as? NSNumber,
              ...
              Int(experts.stringValue) == gemma4A4BExpertCount
        else {
            return false
        }
        return true
    ...
    static func makeRuntimeConfigData(
        ...
        case .gemma4A4B:
            ...
            runtimeConfig = textConfig
            // The pinned Gemma checkpoint publishes the SAME block twice, as
            // `quantization` and `quantization_config`, and the parse below
            // requires them to agree before either is emitted.
            //
            // WHAT MUST NOT BE COPIED FROM THE `.qwen35` BRANCH ABOVE: that
            // branch REBUILDS the emitted block from the parsed triple, which
            // is lossless only because the Qwen block has exactly three keys.
            // This block has 123 -- three scalars plus 120 per-tensor
            // overrides promoting the dense MLP and router projections to 8
            // bits -- so rebuilding it from a triple would emit a config
            // declaring uniform 4-bit for tensors the shards were written at
            // 8. Nothing downstream would notice: the names and shapes stay
            // right and only the numerics go wrong. So the SOURCE block is
            // forwarded verbatim and only the duplicate key is dropped.
            _ = try Gemma4A4BCheckpointValidation.quantizationSpec(
                fromConfigRoot: root
            )
            guard let quantization = root["quantization"]
                ?? root["quantization_config"]
            else {
                throw MLXFastError.invalidInput(
                    "reference config.json is missing both quantization and "
                        + "quantization_config"
                )
            }
            runtimeConfig.removeValue(forKey: "quantization_config")
