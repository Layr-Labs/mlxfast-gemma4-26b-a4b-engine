...
public enum BenchmarkPreflight {
    ...
    private static func checkArtifacts(
        ...
        // Qwen loader: preflight is the surface the cross-language loader-parity
        // gate measures, and the reference holds it to the model identity, so
        // this fork must too or a golden naming the WRONG model preflights clean.
        let golden = try loadQwenGoldenFixture(from: goldenPath)
        if requiresBenchmarkOracle {
            ...
            try Gemma4A4BWeightLoader(denseStore: denseStore)
