...
extension Gemma4Runtime {
    ...
    public static func runCorrectness(
        ...
        if let worker {
            ...
            var loader: Gemma4A4BWeightLoader?
            do {
                try requireFile(
                    options.goldenPath,
                    description: "correctness golden file"
                )
                // Qwen loader: `correctness` names the model it is checking, so
                // the golden is held to the model identity here rather than
                // only later, indirectly, inside checkCorrectnessArtifacts.
                ...
                let config = try Gemma4A4BConfig.load(from: options.weightsPath)
                let runtimeLoader = try Gemma4A4BWeightLoader(
                    weightsPath: options.weightsPath
                )
            ...
            do {
                ...
                // Qwen loader, same reason as the in-process branch above.
