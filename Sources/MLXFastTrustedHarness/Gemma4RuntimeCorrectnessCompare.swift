...
extension Gemma4Runtime {
    public static func traceCorrectness(
        ...
        // Qwen loader: the trace reads the same golden `correctness` scores and
        // reports token-level diagnostics from it, so it names the model too.
        ...
        if let workerOptions {
            ...
            let loader = try Gemma4A4BWeightLoader(
                weightsPath: options.weightsPath
            )
            let weightCache = Gemma4A4BRuntimeWeightCache(
                loader: loader,
                config: config
            )
    ...
    static func compareAnchorCached(
        ...
        let actualToken = try Gemma4Correctness.greedyToken(from: logits)
