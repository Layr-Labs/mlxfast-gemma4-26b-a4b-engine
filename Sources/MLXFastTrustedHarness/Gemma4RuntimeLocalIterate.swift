...
extension Gemma4Runtime {
    public static func localIterate(
        ...
        do {
            ...
            // Qwen loader. Local-iterate has the same shape as the ranked
            // worker benchmark -- checkWorkerBenchmarkInputs is existence-only
            // and no BenchmarkPreflight.check runs behind it -- so this is
            // again the only model-identity check on the path.
    ...
    /// which routes to Gemma4Runtime.benchmark and evaluates the hidden
    ...
    static func runLocalPhaseCoolGate(
        ...
        for repeatIndex in 0..<timingRepeats {
            ...
            let prefillLogits = try gemma4Logits(
            ...
            eval(prefillLogits)
            let prefillToken = try Gemma4Correctness.greedyToken(from: prefillLogits)
            let prefillElapsed = secondsSince(prefillStart)
            prefillHeartbeat?.cancel()
            totalPrefillSeconds += prefillElapsed
            Memory.clearCache()
            latestStats = Gemma4Runtime.expertStats(from: weightCache)
            ...
            var logits = try gemma4Logits(
                inputIDs: inputIDsArray(testCase.promptTokens),
                model: model,
                cache: cache,
                positionOffset: 0
            )
            var actualToken = try Gemma4Correctness.greedyToken(from: logits)
            latestStats = Gemma4Runtime.expertStats(from: weightCache)
        ...
        let bandwidth = (gbPerToken: 0.0, source: Gemma4Runtime.bandwidthSource)
        latestStats = Gemma4Runtime.expertStats(from: weightCache)
    ...
    /// exclusively from localIterate's failure path, and Gemma4Runtime.benchmark
    ...
    static func localIterateScore(
        ...
        // Gemma4Runtime.benchmark), and the ranked artifact validator rejects
