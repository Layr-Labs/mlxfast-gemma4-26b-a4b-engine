...
private let dflashWorkerTrustedPath =
    "Sources/MLXFastTrustedHarness/Gemma4RuntimeDFlashWorker.swift"
private let dflashWorkerParticipantPath =
    "Sources/MLXFastHarness/Gemma4RuntimeDFlashWorker.swift"
private let dflashDriverTrustedPath =
    "Sources/MLXFastTrustedHarness/Gemma4RuntimeDFlashDriver.swift"
private let dflashDriverParticipantPath =
    "Sources/MLXFastHarness/Gemma4RuntimeDFlashDriver.swift"
...
@Test(.disabled("Gemma 4 port / vendored adoption 2026-08-22: reads Gemma4RuntimeDFlashWorker.swift from disk, which left with the DFlash arm (upstream deleted the v1 batching engine the vendored DFlash runtime is written against). The MLX-free DFlash driver/protocol logic is still in tree; only the model-side worker is gone. Restore alongside the DFlash-onto-CBv2 lane -- docs/gemma4-port-notes.md OQ-3."))
