...
// weights on box 3, 2026-08-13. If a re-pin ships a different
...
@Test
func gemma4ChatTemplateMatchesTheCheckpointSidecarPlainPath() {
    #expect(Gemma4ChatTemplate.userTurnDisablingThinking("QUESTION") == expectedRenderedTurn)
}

@Test
func gemma4ChatTemplateEmitsNoDefaultSystemBlock() {
    ...
    let rendered = Gemma4ChatTemplate.userTurnDisablingThinking("Q")
    #expect(!rendered.contains("<|im_start|>system"))
    #expect(rendered.hasPrefix("<|im_start|>user\n"))
}

@Test
func gemma4ChatTemplatePreClosesTheThinkingBlock() {
...
@Test
func gemma4ChatTemplateEmbedsThePromptVerbatim() {
    let prompt = "Line one\nLine two with <angle> chars & symbols"
    #expect(Gemma4ChatTemplate.userTurnDisablingThinking(prompt).contains(prompt))
}
...
func endOfTurnTruncationCutsTheFinishedTurnTail() {
    ...
    #expect(Gemma4ChatTemplate.truncatedAtFirstEndOfTurn(tokens, eosTokenId: 99) == [11, 12, 13])
}

@Test
func endOfTurnTruncationCutsAtTheFirstEndOfTurnOnly() {
    let tokens = [7, 99, 8, 99, 9]
    #expect(Gemma4ChatTemplate.truncatedAtFirstEndOfTurn(tokens, eosTokenId: 99) == [7])
}

@Test
func endOfTurnTruncationIsAnIdentityWhenTheTurnNeverEnds() {
    let tokens = [1, 2, 3]
    #expect(Gemma4ChatTemplate.truncatedAtFirstEndOfTurn(tokens, eosTokenId: 99) == tokens)
    // An unknown eos id must not silently empty the capture.
    #expect(Gemma4ChatTemplate.truncatedAtFirstEndOfTurn(tokens, eosTokenId: nil) == tokens)
}
...
func endOfTurnTruncationNeverReturnsAnEmptyCapture() {
    ...
    // diagnosis.
    #expect(Gemma4ChatTemplate.truncatedAtFirstEndOfTurn([99, 1, 2], eosTokenId: 99) == [99])
    #expect(!Gemma4ChatTemplate.truncatedAtFirstEndOfTurn([99], eosTokenId: 99).isEmpty)
}
