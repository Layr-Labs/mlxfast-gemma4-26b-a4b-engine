...
func poolsideLagunaPublicConfigFixturePinsExactArtifactSemantics() throws {
    ...
    // it guards the Gemma 4 26B A4B tower the harness now drives. Assert the
    // negative rather than dropping the coverage -- a gate that accepted BOTH
    // towers would be a gate that pins neither.
