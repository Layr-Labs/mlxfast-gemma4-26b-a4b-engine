...
/// pinned Qwen 3.8 target (revision
/// `eda45ab47f465d08d6558f0353a2346e2eb9d5b3`, published 2026-08-14 -- the
/// backbone landed, so `Constants.swift` compiles the real sha and the manifest
/// is no longer a pending-release stub). The divergence is deliberate: these
/// fixtures record what was
...
/// SHA256 of the 3.6 checkpoint's own `config.json` bytes as published, NOT of
/// the normalized `fixtures/qwen3_6_27b_config.json` re-render.
///
/// It is deliberately NOT the digest the reference manifest carries for
/// `config.json`: the manifest names the pinned 3.8 target, these fixtures
/// record the 3.6 checkpoint they were captured from, and
/// `MLXFastConstants` states that split as a decision ("Tests/Fixtures/
...
/// `qwen36ConfigContractDigestMatchesTheReferenceManifest` pins both sides of
/// that split rather than asserting they are equal.
let qwen36ConfigSHA256 =
    "ede24666ac51e6d5ab948a8a1e6c72fc6effd941ba3aabb6dd942eb517c78043"

/// SHA256 and byte count the checked-in reference manifest carries for the
/// PINNED 3.8 target's `config.json`. Read off
/// `fixtures/reference_qwen3_8_27b_4bit.sha256`, whose body was generated from
/// the published 3.8 target snapshot. The artifact is identified by that
/// manifest's per-file sha256+byte pins, not by a repository name.
let qwen38ManifestConfigSHA256 =
    "558cd605a6f1c16c73f4918534d122a943e12e754d38567b3b704acc93596965"
let qwen38ManifestConfigBytes = 4_094
...
/// The track contract fixture. It carries `target.manifest_sha256` and
/// `target.expected_source_bytes`, which are the third leg of the manifest
/// check: the manifest names the target, and the contract pins the manifest.
let qwen38MTPTrackContractURL = qwen36ArtifactRepositoryRoot
    .appendingPathComponent("fixtures/qwen3_8_27b_mtp_track.json")
