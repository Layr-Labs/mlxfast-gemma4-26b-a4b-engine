...
public final class Gemma4CBv2MTPDrafter: CBv2MTPDrafter {
    ...
    public func draftStep(
        ...
        // Match the upstream assistant contract exactly: concatenate the
        // target token embedding with the raw target hidden state, then let
        // the learned pre-projection consume that pair. RoPE belongs inside
        // the assistant attention layers; rotating this input changes the
        // learned model and destroys draft quality.
        let targetEmbedding = target.embedTokensForDrafter(tokens)
        let alignedHidden: MLXArray
        if targetEmbedding.ndim == hidden.ndim {
            alignedHidden = hidden
        } else if targetEmbedding.ndim == hidden.ndim + 1,
            targetEmbedding.dim(-2) == 1,
            Array(targetEmbedding.shape.dropLast(2)) == Array(hidden.shape.dropLast())
        {
            alignedHidden = hidden.expandedDimensions(axis: -2)
        } else {
            preconditionFailure(
                "Gemma4CBv2MTPDrafter.draftStep: cannot align target embedding "
                    + "\(targetEmbedding.shape) with hidden \(hidden.shape)")
        }
