...
extension Gemma4TextModel {
    ...
    ///
    /// The traversal is layer-major, but every token column enters every
    ...
    /// `column -> layer` into `layer -> column` preserves dependencies:
    /// within each layer the columns still write and attend in serial order.
    /// One glue-chain box per column preserves the ordinary cross-layer
    /// fusion choices.
    fileprivate func cbv2BatchedExactMTPForward(
        ...
        let argmaxColumns = columns.map {
            argMax(applyLMHead(model.norm($0)), axis: -1).asType(.int32)
        }
