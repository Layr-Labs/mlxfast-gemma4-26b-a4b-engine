...
private func gemma4FusedQKVNormHeadMajor(
    ...
    // A fused-RoPE group must reach its threadgroup barrier uniformly. D=256
    // rows are inherently RPT-aligned; odd D=512 rectangles can leave two
    // padded slots, so keep those rare shapes on the stock fallback.
    guard !fusedRope || rows % rowsPerGroup == 0 else { return nil }
