...
///
/// The intermediate `summed` value is explicitly rounded to BF16 before the
/// scalar multiply, preserving the established two-dispatch dtype boundary.
...
private class Gemma4Attention: Module {
    ...
    init(_ config: Gemma4TextConfiguration, layerIdx: Int, forceSharedKV: Bool = false) {
        ...
        // Preserve the exact operand used by MLX so the decode-only paired
        // Q/K specialization does not reconstruct or approximate its angles.
        if isSliding {
...
public class Gemma4DecoderLayer: Module {
    ...
    public func callAsFunction(
        ...
        // The benchmark model has no PLE modules. Fuse its final RMSNorm,
        // residual add, and learned scalar multiply while retaining a guarded
        // fallback for any model configuration that does have PLE.
