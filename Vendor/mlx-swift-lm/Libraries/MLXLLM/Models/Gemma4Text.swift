...
private enum Gemma4ZipRouterV1 {
    ...
    static func run(
        ...
        //
        // TRINORM-002B: the first fence orders the router QMV behind the sum
        // table. Under the three-weight carrier `normed` and the table are
        // outputs of the SAME dispatch, so the edge is an identity: it cannot
        // change the issue order, but it still costs a `Depends` node and
        // marks the `[8, 1, 2816]` row shared, which takes the router QMV's
        // input out of the donatable set. Dropped on the carrier path only.
        //
        // The second fence is NOT touched. It is what holds the dense gate/up
        // back so the encoder pairs them with the router QMV in one barrier
        // stage, and that pairing is the whole point of ZIP-ROUTER; dropping
        // both fences at once measured -5.08% (`a7d0287`).
        let scoreIn =
            carriedRouterNorm != nil
            ? normed
