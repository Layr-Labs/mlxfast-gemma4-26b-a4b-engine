...
private class Gemma4Attention: Module {
    ...
    /// Z-STACK-QKV: sliding Q|K|V in one dispatch. Nil keeps fused Q|K plus
    /// the separate V tierProjection.
    @inline(__always)
    ...
    private func forwardV2(
        ...
        let vRaw: MLXArray
        if let stackedV = fusedQKV?.2 {
            vRaw = stackedV.reshaped(B, L, nKvHeads, effectiveHeadDim)
        } else if let vProj {
