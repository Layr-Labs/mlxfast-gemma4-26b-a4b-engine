...
/// DECODE-OB3: the same one-barrier cross-simdgroup rms combine the prompt
/// pack `pg2` kernel already ships, applied to the batch-eight decode QKV
/// norm-and-RoPE kernel.
///
/// The incumbent zero-fills 32 threadgroup floats, publishes one partial per
/// simdgroup, reduces them on simdgroup 0 alone, writes the reciprocal back to
/// a threadgroup scalar and barriers again so the other simdgroups can read
/// it. Three barriers and a round trip through threadgroup memory, on all
/// thirty layers of every decode round.
///
/// The replacement lets every simdgroup run the same 32-lane combine: lanes
/// below `decode_row_simds` take the published partials, the rest contribute
/// the same `0.0f` the zero-fill would have left, from a register. `simd_sum`
/// therefore sees the identical 32 operands in the identical lane positions,
/// so the reciprocal is bit-identical and no reassociation is introduced.
...
///
/// PROMPT-OFF-052: default flipped to OFF. Three ranked archives carrying this
/// arm drew prompt 1.62400, 1.63727 and 1.63360; four without it drew 1.62654,
/// 1.65699, 1.64930 and 1.65083. The arm is decode-neutral by construction, so
/// the only leg it can move is prefill, and it appears to cost about 0.9%
/// there. Set `DARKBLOOM_GEMMA4_PROMPT_OB3=1` to restore it.
