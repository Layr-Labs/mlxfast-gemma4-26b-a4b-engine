...
public class Gemma4TextModel: Module, LLMModel, KVCacheDimensionProvider {
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        ...
        interleaveExpertScaleBias(sanitized)
        return sanitized
    }
    ...
    /// SB-INTERLEAVE: build each routed-expert layer's merged scale/bias
    /// carriers here, at load, and bind them beside the loaded split planes.
    /// The loaded arrays are left exactly as they are -- prefill's sorted
    /// gathers and every non-decode consumer keep reading the split planes --
    /// so this costs one extra copy of the layer's six metadata planes and
    /// nothing else. Layers or checkpoints outside the exact production
    /// geometry, and the arm's off-state, bind nothing and keep the incumbent
    /// gathers.
    private func interleaveExpertScaleBias(_ sanitized: [String: MLXArray]) {
        guard switchSBInterleaveEnabled else { return }
        ...
        for key in sanitized.keys where key.hasSuffix(gateWeightSuffix) {
            ...
            guard let layerIdx = extractLayerIdx(from: key),
                ...
                let storage = SwitchSBInterleaveStorage(
                    gateScales: gateScales, gateBiases: gateBiases,
                    upScales: upScales, upBiases: upBiases,
                    downScales: downScales, downBiases: downBiases)
            else { continue }
            experts.switchGLU.bindSBInterleaveStorage(storage)
        }
    }
