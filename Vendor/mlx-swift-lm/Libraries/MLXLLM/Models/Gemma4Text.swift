...
private class Gemma4Attention: Module {
    ...
    func callAsFunction(
        ...
        if let ringKV, let shared = sharedKV {
            ...
            // MTP-QUANT-AUTH: on the quant-authoritative road the drafter attends
            // the q4 mirror (the target's own decode read), not the stale BF16 ring.
            let ringAttended: MLXArray?
            if let mirrors = ringKV.mirrors {
                CBv2EngageMark.once("mtp-quant-auth")
                ringAttended = CBv2RaggedTwoPassDecodeAttentionV1.attendRingQuant(
                    queries: ringQueries, mirrors: mirrors, starts: ringKV.starts,
                    scale: scale, slidingWindowLength: ringKV.keys[0].dim(2))
            } else {
                ringAttended = CBv2RaggedTwoPassDecodeAttentionV1.attendRing(
            ...
            guard let ringOutRaw = ringAttended
            else {
                // K16: a declined fast road costs speed, never the run. Rebuild
                // the temporal order the stacked road used -- each row's ring
                // rolled to start at its oldest slot -- and attend with the
                // stock kernel. Every retained position is inside the window,
                // which is why the stacked road's mask was all-true and why no
                // mask is needed here.
                CBv2EngageMark.once("drafter-ring-attn-fallback")
                var ordered: [MLXArray] = []
                var orderedValues: [MLXArray] = []
                ordered.reserveCapacity(ringKV.keys.count)
                orderedValues.reserveCapacity(ringKV.values.count)
                for (index, key) in ringKV.keys.enumerated() {
                    let start = ringKV.starts[index]
                    let value = ringKV.values[index]
                    if start == 0 {
                        ordered.append(key)
                        ...
                        ordered.append(
                            concatenated(
                                [key[0..., 0..., start..., 0...], key[0..., 0..., ..<start, 0...]],
                                axis: 2))
                ...
                let stackedKeys = concatenated(ordered, axis: 0)
                let stackedValues = concatenated(orderedValues, axis: 0)
                let fallbackRaw = MLXFast.scaledDotProductAttention(
                    queries: ringQueries, keys: stackedKeys, values: stackedValues,
                    scale: scale, mask: .none)
                let fallbackOut =
                    fallbackRaw.dtype == ringDType ? fallbackRaw : fallbackRaw.asType(ringDType)
                return (
                    outputProjection(fallbackOut.transposed(0, 2, 1, 3).reshaped(B, L, -1)),
                    (shared.0, shared.1), activePositionOffset
                )
            }
