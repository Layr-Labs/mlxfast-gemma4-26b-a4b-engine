...
internal struct Gemma4TGARPolicy: Sendable {
    ...
    static let resolved: Gemma4TGARPolicy = {
        ...
        let lastN = Int(env["DARKBLOOM_GEMMA4_TGAR_LAST_N"] ?? "8") ?? 8
        let temperature = Float(env["DARKBLOOM_GEMMA4_TGAR_TAU"] ?? "0.85") ?? 0.85
        let entropyFloor = Float(env["DARKBLOOM_GEMMA4_TGAR_ENTROPY_FLOOR"] ?? "0.5") ?? 0.5
        let underRoutedTop1 = Float(env["DARKBLOOM_GEMMA4_TGAR_UNDER_TOP1"] ?? "0.25") ?? 0.25
        let amplify = Float(env["DARKBLOOM_GEMMA4_TGAR_AMPLIFY"] ?? "0.5") ?? 0.5
        let spread = Float(env["DARKBLOOM_GEMMA4_TGAR_SPREAD"] ?? "0.5") ?? 0.5
        let keepRatio = Float(env["DARKBLOOM_GEMMA4_TGAR_KEEP_RATIO"] ?? "0.5") ?? 0.5
        return Gemma4TGARPolicy(
            enabled: true,
            lastN: max(0, lastN),
