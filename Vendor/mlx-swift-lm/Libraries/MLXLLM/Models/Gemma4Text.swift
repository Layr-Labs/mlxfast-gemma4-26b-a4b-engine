...
// by the preceding ranked submission of this content apart from any rotation item declared in its note.
// Candidate EXP-032: crown 4865fbe rebase + DENSE-GEGLU-EPILOGUE integration + prefill expert unsort tail chain 128-bit vectorization & register hoist (1.075s prefill) + restore proven AOT metallib decode kernels (fp16Dequant=false, unroll4=false for 1.649s decode) + decoupled phase-specific RoPE offsets + pre-evaluated switchDownIdentity64 + lock-free pointer GEMV & D512 caches + MTP memoization.
