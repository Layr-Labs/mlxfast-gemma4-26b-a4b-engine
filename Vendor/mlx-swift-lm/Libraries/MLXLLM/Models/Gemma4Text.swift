...
final class Gemma4MTPVerifyQuantizedLinear: QuantizedLinear {
    ...
    override func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        // MTP-PV OCT: the sixteen-row plane in ONE oct-stream dispatch —
        // weights read once per row-octet, per-row arithmetic on the
        // promoted stream bodies (the same accumulation class as the [8,1]
        // decode road, which is the divergence class the rectangular-verify
        // ban responds to on M5 silicon). Falls back to the split stock
        // road whenever any pin declines.
        if CBv2TiedLMHeadQMVV1.verifier16Enabled,
            logicalRows == 16, bias == nil, let affineBiases = biases,
            let fused = CBv2TiedLMHeadQMVV1.matmulVerifier16(
                x: flat, weight: weight, scales: scales, biases: affineBiases,
                inDim: shape.1, outDim: shape.0, bits: bits)
        {
            return fused.reshaped(prefix + [shape.0])
        }
...
/// contract is checked once by `install`; the measured call retains the
/// per-column certified M8 arithmetic followed by the target's fixed softcap.
final class Gemma4MTPVerifyTiedHead {
    ...
    func callAsFunction(_ hidden: MLXArray) -> MLXArray {
        // MTP-PV OCT: [8, columns, K] flattened row-major is exactly the
        // (batch-major, column-minor) row order the per-column road's
        // axis-1 concatenation produces, so the reshape round trip lands
        // every logit plane at the identical position.
        if CBv2TiedLMHeadQMVV1.verifier16Enabled,
            hidden.ndim == 3, hidden.dim(0) * hidden.dim(1) == 16,
            let fused = CBv2TiedLMHeadQMVV1.matmulVerifier16(
                x: hidden.reshaped([16, inDim]), weight: weight,
                scales: scales, biases: biases,
                inDim: inDim, outDim: outDim, bits: 4)
        {
            return fused.reshaped([hidden.dim(0), hidden.dim(1), outDim])
        }
...
extension Gemma4TextModel {
    func makeMTPVerifyTiedHead() -> Gemma4MTPVerifyTiedHead? {
        guard CBv2TiedLMHeadQMVV1.enabled,
            lmHead == nil,
