...
private enum Gemma4FusedLayerGlue {
    ...
    private static let tailKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        \(rmsReduce("sv", into: "local_inv[0]").replacingOccurrences(
            ...
            for (int i = 0; i < 4; i++) {
                ...
                // the layer-scalar multiply with its own stock rounding.
                const T normed = static_cast<T>(
    ...
    /// of the row. Numerics replicate the stock rms sequence over the exact
    /// stored bf16 output values.
...
public class Gemma4DecoderLayer: Module {
    ...
    public func callAsFunction(
        ...
        // guarantees the normed tensor came from exactly this input.
        let h: MLXArray
