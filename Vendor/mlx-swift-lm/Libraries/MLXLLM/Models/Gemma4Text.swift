...
private enum Gemma4FusedLayerGlue {
    ...
    private static let tailChainKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        \(rmsReduce("outv", into: "local_inv[0]").replacingOccurrences(
            ...
            // `mma8_runsum4` adds each four-value half while it is still T,
            // then widens that rounded partial to float. Preserve that
            // otherwise-easy-to-miss BF16 precision boundary exactly.
            const T sum4t =
                nextv[0] + nextv[1] + nextv[2] + nextv[3];
            const float sum4 = (float)sum4t;
