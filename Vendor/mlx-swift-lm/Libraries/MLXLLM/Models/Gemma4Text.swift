...
private class Gemma4Attention: Module {
    ...
    init(_ config: Gemma4TextConfiguration, layerIdx: Int, forceSharedKV: Bool = false) {
        ...
        if isSliding {
            ...
            self.prefillRoPEConfiguration = nil
        }
    ...
    private func forwardV2(
        ...
        // PREP-ONLY: decode (L == 1) and every full-attention plane retain the
        // promoted path. The full K-eq-V crown shares K's exact normalization
        // reduction with V, so this topper is deliberately sliding-only.
        let prefillPrepared: (queries: MLXArray, keys: MLXArray)?
        if B == 8 && L >= 2 && lastQueryCache == nil && isSliding,
            let prefillRoPEConfiguration
        {
