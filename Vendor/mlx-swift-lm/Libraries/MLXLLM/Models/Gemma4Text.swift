...
public class Gemma4DecoderLayer: Module {
    ...
    public func callAsFunction(
        ...
        {
            out = fusedOut
        } else if let prefillOut = Gemma4PrefillGlueV1.normResidual(
            x: attnOut, weight: postAttentionLayernorm.weight,
            residual: residual, eps: postAttentionLayernorm.eps)
        {
            // PREFILL-GLUE-001. Disjoint from the decode gate above by
            // dim(1) >= 2, so exactly one of the two can ever admit.
            out = prefillOut
        } else {
        ...
        // PREFILL-GLUE-001's tail folds `residual2` back in but NOT the layer
        // scalar, so it needs its own flag: it suppresses the postFFLN+residual
        // tail below and nothing else.
        var prefillTailApplied = false
            ...
            let postFeedforwardLayernorm2
        {
                    ...
                    eps: config.rmsNormEps)
            {
                out = fusedTail
                tailApplied = true
            } else if let prefillTail = Gemma4PrefillGlueV1.branchTail(
                h1: h1Raw,
                h2: h2Raw,
                w1: postFeedforwardLayernorm1.weight,
                w2: postFeedforwardLayernorm2.weight,
        ...
        if !tailApplied && !prefillTailApplied {
