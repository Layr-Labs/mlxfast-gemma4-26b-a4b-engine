...
/// M5 NAX uses two 32-row fragments per 64-row target tile. Place one B=8
/// verifier position at the start of each fragment so both positions share a
/// weight load while every live row keeps the same NAX reduction.
public let gemma4M5NAXPositionPackingEnabled =
    GPU.deviceInfo().architecture.contains("M5")

public func gemma4PrepareB8PositionBatches(_ x: MLXArray) -> MLXArray? {
    ...
    let positionMajor = x.swappedAxes(0, 1)
    if gemma4M5NAXPositionPackingEnabled {
        return padded(positionMajor, widths: [0, [0, 24], 0])
    }
    let paddedPositionMajor = padded(
        positionMajor, widths: [0, [0, 1], 0])
    return asStrided(
        paddedPositionMajor,
...
public func gemma4RestoreB8PositionOutput(_ output: MLXArray) -> MLXArray {
    let live = output.dim(1) == 32 ? output[0..., ..<8, 0...] : output
    return live.swappedAxes(0, 1)
}
...
private class Gemma4Attention: Module {
    ...
    private func forwardV2(
        ...
        let fusedProjection: (q: MLXArray, k: MLXArray, v: MLXArray)? = {
            ...
            let output = needsSwap
                ? gemma4RestoreB8PositionOutput(positionOutput) : positionOutput
