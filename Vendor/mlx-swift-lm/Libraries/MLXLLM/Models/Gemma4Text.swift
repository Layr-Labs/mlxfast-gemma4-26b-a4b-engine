...
private class Gemma4Attention: Module {
    ...
    private func forwardV2(
        ...
        // The last-query prompt view also has [8, 1, H] shape but owns a
        // different prefill contract. Keep it on stock QMV; the ordinary
        // [8, 1, H] CBv2 path remains eligible for the tight-grid entry.
...
private class Gemma4MLP: Module {
    ...
    func callAsFunction(_ x: MLXArray, useTightGridQMV: Bool) -> MLXArray {
        let gate: MLXArray
        let up: MLXArray
        if useTightGridQMV {
