...
// CBV2-DIRECT-CACHE-ACCESS kill switch. Direct indexing is ON by default for
// the full per-layer cache set; `0`/`false`/`no`/`off` restores the boxed
// compatibility path for attribution and emergency bisection.
private let gemma4DirectCBv2CacheAccessEnabled: Bool = {
    guard let raw = ProcessInfo.processInfo.environment[
        "DARKBLOOM_GEMMA4_DIRECT_CBV2_CACHE_ACCESS"]
    else { return true }
...
public class Gemma4TextModelInner: Module {
    ...
    private func forwardTrunk(
        ...
        let cacheIsCBv2 =
            cache?.contains { ($0 as? (any CBv2AttendingLayerCache)) != nil } ?? false
        let useDirectCBv2Cache =
            gemma4DirectCBv2CacheAccessEnabled && cacheIsCBv2
            && (cache?.count ?? 0) >= config.numHiddenLayers
        ...
        let unifiedCBv2PositionOffset: Gemma4.PositionOffset? = {
            ...
            if useDirectCBv2Cache, let cache {
                for entry in cache {
        ...
        for (idx, layer) in layers.enumerated() {
            ...
            // CBv2 shared layers borrow the source cache. Direct mode reads
            // the already-typed input array; legacy mode keeps its padded view.
            let layerCache: KVCache? =
                useDirectCBv2Cache ? cache?[idx] : fullCache[idx]
            let v2SharedSource: (any CBv2AttendingLayerCache)? =
                isCBv2 && prevIdx != idx
                ? (useDirectCBv2Cache ? cache?[prevIdx] : fullCache[prevIdx])
