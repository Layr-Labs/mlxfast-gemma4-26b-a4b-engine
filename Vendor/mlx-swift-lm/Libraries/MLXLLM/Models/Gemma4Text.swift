...
private final class Gemma4DenseGateUpStorage {
    ...
    init?(
        ...
        // The concatenation and the six row views are graph nodes until
        // something demands them. Force them here so the 357 MB of copies is
        // charged to model binding, and so the decode path can never be the
        // first consumer to materialise a joined plane inside a timed window.
        ...
        // groups over 2816 inputs is group size 64. Build the prompt plane
        // here rather than on first admission, so the dequantization is
        // charged to model binding and no measured window can be the first
        // consumer. Steady-state bytes are unchanged: this one [2816, 4224]
        // plane replaces the two [2816, 2112] planes the frontier's own table
    ...
    /// built on first prompt use and kept for the model lifetime. Built and
    /// evaluated with no lock held, then inserted, matching the per-projection
    /// table in `Gemma4PrefillDeqGEMMV1`.
