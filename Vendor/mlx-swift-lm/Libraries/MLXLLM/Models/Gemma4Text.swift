...
private class Gemma4Attention: Module {
    ...
    init(_ config: Gemma4TextConfiguration, layerIdx: Int, forceSharedKV: Bool = false) {
        ...
        // Keep the exact parameter input MLX's own primitive consumes, so the
        // paired decode kernel reproduces its arithmetic rather than rebuilding it.
        if isSliding {
            ...
            // `[D / 2]` frequency-shape gate and retain the generic path.
            self.decodeRoPEOperands = Gemma4DecodeRoPEOperands(
    ...
    private func forwardV2(
        ...
        // One two-output dispatch rotates Q and K together on the ruled
        // batch-8 decode step. The gate is deliberately narrow: only when
        // there is no last-query prefill shift (so queryPositionOffset and
        // captured are the same offsets) and the offsets are the per-row batch
        // this path always builds. Everything else -- prefill, last-query
        // prefill with outputStart != 0, any other width -- takes the stock
        // pair of dispatches unchanged.
        var fusedRoPE = false
        if B == 8, lastQueryCache == nil, outputStart == 0,
            let rotated = gemma4FusedDecodeQKRoPE(
                queries: queries, keys: k, offsets: capturedOffsets,
...
public class Gemma4DecoderLayer: Module {
    ...
    public func callAsFunction(
        ...
        // One dispatch for {rmsNorm, add} at B=8 decode; falls through to the
        // stock pair on any other shape, dtype or eps.
        var out =
            ...
            let postFeedforwardLayernorm2
        {
            ...
            // One dispatch for {rmsNorm, rmsNorm, add} at B=8 decode.
            out =
                gemma4FusedDualRMS(
