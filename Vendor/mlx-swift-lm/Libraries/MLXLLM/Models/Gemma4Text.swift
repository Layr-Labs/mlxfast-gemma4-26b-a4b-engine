...
private class Gemma4Attention: Module {
    ...
    /// QKV-VJOIN. Q, K and V read the same activation at decode, so all
    /// three planes concatenate into one dispatch (sliding layers only —
    /// full-attention layers are k_eq_v and ship no v_proj). Nil whenever
    /// the shapes, the quantization parameters or the arm's switch say
    /// otherwise; the caller then keeps the fused Q|K dispatch plus the
    /// separate V tierProjection.
    /// MMA-RS-001: `rsTable` is the layer input's shared run-sum table,
    /// indexed by activation row and 64-group of K only — never by N — so
    /// the joined dispatch reads the same entries the separate dispatches
    /// would.
    @inline(__always)
    private func fusedQKVProjection(
        ...
        guard let q = qProj as? QuantizedLinear, q.bias == nil,
            ...
            q.groupSize == k.groupSize, q.bits == k.bits, q.mode == k.mode,
            k.groupSize == v.groupSize, k.bits == v.bits, k.mode == v.mode
        else { return nil }
    ...
    private func forwardV2(
        ...
        // dispatch too, and the K-eq-V structure is untouched (keyValueShared
        // stays vProj == nil). `DARKBLOOM_GEMMA4_QKFUSE_SLIDING=0` restores
        // the vProj == nil gate.
        // QKV-VJOIN: sliding layers with a vProj join V into the same fused
        // dispatch (one encoder instead of two per layer). The kernel admits
        // only the exact sliding decode geometry and computes each output
        // column from that column's own plane row, so the three halves are
        // bit-identical to the separate dispatches.
        // `DARKBLOOM_GEMMA4_QKV_VJOIN=0` restores the fused Q|K dispatch plus
        // the separate V tierProjection; either arm off, or any guard miss,
        // falls back to the arm below. Prefill, last-query, shared-KV and
        // other batch widths keep their incumbent dispatches.
        // MMA-RS-001: the fused dispatches consume the shared run-sum
        ...
        // entries the separate Q, K and V dispatches would.
        ...
        let fusedQK: (MLXArray, MLXArray)? =
            (fusedQKV == nil && lastQueryCache == nil && !usesSharedKV
        ...
        if let vProj {
            vRaw = (fusedQKV?.2
                ?? tierProjection(vProj, x, rsTable: qkvRunsumTable))
                .reshaped(B, L, nKvHeads, effectiveHeadDim)
        } else {
            vRaw = kRaw
        }
