...
/// `DARKBLOOM_CBV2_ROUTE_STATS=1`: one stderr line per MoE layer per decode or
/// wide-verify forward (prompt chunks above 64 rows are skipped) with the
/// sorted-by-expert run-length histogram. Debug only: the readback blocks.
...
public class Gemma4TextModelInner: Module {
    ...
    /// The promoted decode trunk with the pre-norm hidden retained: the
    /// fused final norm and head activation sums of `callWithMMAHeadSums`.
...
/// The hidden-capturing trunk (MTP seed rounds and verify rectangles) takes
/// the promoted B=8 decode road: the attention-branch-prefix glue and the
/// fused final norm with head activation sums, the same kernels plain decode
/// runs. `DARKBLOOM_CBV2_HIDDEN_PROMOTED_TRUNK=0` keeps the unfused glue.
