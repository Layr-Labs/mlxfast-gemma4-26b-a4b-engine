...
"""
    ...
    /// OPROJ-UR4-58EF. Selects the loop unroll directive on the o-projection
    /// run-sum group loop: `#pragma clang loop unroll_count(4)` when on, the
    /// incumbent pragma when off. Loop order, addresses, and arithmetic are
    /// unchanged in both positions; the two scored run-sum bodies have exact
    /// trip counts of 32 and 64, both divisible by four, so the requested
    /// factor changes only the compiler's schedule.
    ///
