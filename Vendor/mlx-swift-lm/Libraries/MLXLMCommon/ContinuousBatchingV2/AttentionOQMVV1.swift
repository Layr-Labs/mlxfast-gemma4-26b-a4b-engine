...
"""
    ...
    /// C2X: the incumbent body with the packed codes carried two groups deep
    /// and the activation run loaded one group ahead. Derived textually from
    /// `mma8KernelHeader`; every arithmetic statement is the incumbent's and
    /// every address is a function of the group index alone, so each trip
    /// consumes the values the in-place reads produced.
    ...
    /// Wide verify mapping: `rows` keeps RT row tiles inside one simdgroup
    /// pair sharing each weight fetch; `ztile` gives every row tile its own
    /// threadgroup (grid.z) running the eight-row body unchanged.
    ...
    public static let defaultBody: Body = envChoice("DARKBLOOM_GEMMA4_ATTN_O_BODY", .carry1)
