...
#pragma unroll
  for (int gi = 0; gi < nGroups; ++gi) {
    ...
    // Each operand is filled immediately before the step that reads it,
    // so one fill is live at a time instead of eight. `r0`/`r1` are not
    // written across the run, so every fill yields the value it yielded
    // before and the accumulation order into `C` is unchanged.
