...
// Two output tiles share the activation-side fragment build. The weight
// traffic remains one pass over the o_proj plane, while the x loads, run-sum
// butterfly and B0...B7 fills are paid once for both adjacent tiles.
...
#pragma clang loop unroll(full)
  for (int t = 0; t < TILES; ++t) {
    const int nt = n0 + t * 8;
    wrow[t] =
        (const device uint8_t*)w + (nt + c.fm) * (K / 2) + 4 * c.fn;
...
"""
    ...
    private static let mma8MultiTileKernelK4096 = MLXFast.metalKernel(
        name: "cbv2_b8_l1_attention_o_mma8_affine4_g64_k4096_mt2_unroll_v1",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
    ...
    private static let mma8MultiTileKernelK8192 = MLXFast.metalKernel(
        name: "cbv2_b8_l1_attention_o_mma8_affine4_g64_k8192_mt2_unroll_v1",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
    ...
    public static func matmul(
        ...
        if mma8Enabled {
            // Two adjacent eight-column tiles share the activation fragment
            // construction. The output width is 2816, so its 352 tiles are
            // always pairable under the live geometry gate.
            let yTiles = outputWidth / outputsPerGroup
            if yTiles % 2 == 0 {
                let kernel =
                    inDim == 8192 ? mma8MultiTileKernelK8192 : mma8MultiTileKernelK4096
                return kernel(
