...
METAL_FUNC void attention_o_qmv_mma8_affine4_g64_impl(
  ...
  // Two-stage packed-word carry, the form the Q/K/V tier already runs: the
  // walk holds the next group's word and the one after it, so a trip's load
  // has two trips of latency to land instead of one. The clamp is the bound,
  // so the last trips re-read their own group and discard it at loop exit;
  // every address formed is an address the one-stage walk forms on some trip.
...
#pragma unroll
  for (int gi = 0; gi < nGroups; ++gi) {
    ...
    // at a time rather than eight — the form the dense-down tier promoted.
    // `r0`/`r1` are not written between here and the last step, and the
    // accumulation order into `C` is unchanged.
