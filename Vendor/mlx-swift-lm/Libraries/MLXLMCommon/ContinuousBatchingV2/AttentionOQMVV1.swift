...
METAL_FUNC void attention_o_qmv_mma8_affine4_g64_impl(
  ...
  // Two-stage weight operand prefetch pipeline: group g0 and g0+1 are read
  // before the unrolled walk, and each trip reads group gi+2 ahead into
  // wv_next2 while the current trip consumes wv_next. Scales and biases stay
  // one group ahead. Addresses are functions of the group index alone and
  // clamped to the simdgroup's last group.
