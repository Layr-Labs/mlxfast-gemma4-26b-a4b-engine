...
#pragma clang loop unroll(full)
  for (int t = 0; t < TILES; ++t) {
    ...
    if (SPLIT == 0) {
      ...
      // Fused Q||K||V plane: columns below SPLIT belong to Q, SPLIT..SPLIT2 to K, SPLIT2..N to V.
      const int col = nt + c.fm;
      if (col < SPLIT) {
        y[c.fn * SPLIT + col] = static_cast<T>(acc0[t]);
        y[(c.fn + 1) * SPLIT + col] = static_cast<T>(acc1[t]);
...
"""
    ...
    /// QKV-SLIDING-FUSE arm. Default ON.
    /// `DARKBLOOM_GEMMA4_QKV_FUSE_SLIDING=0` restores the separate V dispatch.
    ...
    /// QKV-SLIDING-FUSE. One dispatch for the layer's Q, K, and V projections on sliding layers.
    /// Fuses Q (4096), K (2048), and V (2048) into a single concatenated plane of width 8192,
    /// executing a single GPU kernel launch and returning (q, k, v).
