...
#define MMA8_STEP_W(CC, BB, J, WW) A.thread_elements()[0] = float(extract_bits(WW.x, 4 * (J), 4)); A.thread_elements()[1] = float(extract_bits(WW.y, 4 * (J), 4)); simdgroup_multiply_accumulate(CC, A, BB, CC);
...
#pragma clang loop unroll(full)
    ...
    MMA8_SETB(B, x, lo)
    MMA8_STEP_W(C0, B, 0, wv_cur[0])
    MMA8_STEP_W(C1, B, 0, wv_cur[1])

    MMA8_SETB(B, x, hi)
    MMA8_STEP_W(C0, B, 1, wv_cur[0])
    MMA8_STEP_W(C1, B, 1, wv_cur[1])

    MMA8_SETB(B, y, lo)
    MMA8_STEP_W(C0, B, 2, wv_cur[0])
    MMA8_STEP_W(C1, B, 2, wv_cur[1])

    MMA8_SETB(B, y, hi)
    MMA8_STEP_W(C0, B, 3, wv_cur[0])
    MMA8_STEP_W(C1, B, 3, wv_cur[1])

    MMA8_SETB(B, z, lo)
    MMA8_STEP_W(C0, B, 4, wv_cur[0])
    MMA8_STEP_W(C1, B, 4, wv_cur[1])

    MMA8_SETB(B, z, hi)
    MMA8_STEP_W(C0, B, 5, wv_cur[0])
    MMA8_STEP_W(C1, B, 5, wv_cur[1])

    MMA8_SETB(B, w, lo)
    MMA8_STEP_W(C0, B, 6, wv_cur[0])
    MMA8_STEP_W(C1, B, 6, wv_cur[1])

    MMA8_SETB(B, w, hi)
    MMA8_STEP_W(C0, B, 7, wv_cur[0])
    MMA8_STEP_W(C1, B, 7, wv_cur[1])
...
"""
    ...
    private static let multiTileKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_qkv_mma8_affine4_g64_tight_mt2_k2816_carry2_operand_bfill_v6",
