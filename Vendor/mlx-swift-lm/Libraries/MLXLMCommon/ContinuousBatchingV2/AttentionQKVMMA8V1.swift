...
"""
    ...
    /// the promoted registrations in the same executable.
    ...
    /// Derive only the final rsp multi-tile body from the existing header.
    /// Replace its runtime output stride with a compile-time template value.
