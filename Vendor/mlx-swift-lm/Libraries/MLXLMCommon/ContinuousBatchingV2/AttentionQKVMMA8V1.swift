...
#pragma clang loop unroll(full)
  for (int t = 0; t < TILES; ++t) {
    ...
    if (SPLIT == 0) {
      ...
      // Fused Q||K||V plane: columns below SPLIT belong to Q, below SPLIT2
      // to K, the rest to V. Both rows of a store pair share one column, so
      // the branch is uniform. SPLIT2 == 0 keeps the two-output Q||K form
      // byte for byte: the middle arm folds to the incumbent y2 store.
...
"""
    ...
    /// QKV-TRIPLEFUSE. Three-output twins of the fused Q||K kernels: the
    /// second split point (SPLIT2) carves the V plane out of the same fused
    /// weight/scales/biases concatenation. Sliding: 4096|2048|2048;
    /// full: 8192|1024|1024.
    private static let tripleSlidingKernel = MLXFast.metalKernel(
    ...
    private static let tripleFullKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_qkv_mma8_affine4_g64_tight_mt2_k2816_carry2_qkv10240_v1",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y", "y2", "y3"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
    ...
    private static let tripleSlidingRspKernel = MLXFast.metalKernel(
    ...
    private static let tripleFullRspKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_qkv_mma8_affine4_g64_tight_mt2_k2816_carry2_qkv10240_rsp_v1",
        inputNames: ["x", "w", "scales", "biases", "rs_table"],
        outputNames: ["y", "y2", "y3"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
    ...
    /// QKV-TRIPLEFUSE arm. Default ON.
    /// `DARKBLOOM_GEMMA4_QKV_FUSE_QKV=0` restores the fused-Q||K plus
    /// separate-V dispatches.
    ...
    /// QKV-TRIPLEFUSE. One dispatch for the layer's Q, K and V projections.
    ///
    /// V reads the same activation at decode and shares K=2816, group size,
    /// bit width and quantization mode with Q/K, so its weight plane joins
    /// the concatenation along the output axis. Row r of the triple output
    /// is row r of whichever source plane it came from: the tier
    /// accumulates each output column independently, so this is bit-exact,
    /// not a reassociation. The whole saving beyond QKFUSE-001 is V's own
    /// per-layer encoder (one dispatch per non-shared layer per step).
    ///
    /// The concatenation is a pure re-layout of the shipped quantized words.
    /// No value is re-quantized and no numerical format changes. The rsp
    /// table argument is identical to fusedQKMatmul's: per-row, per-64-group
    /// entries independent of N. Nil keeps the incumbent Q||K + separate-V
    /// dispatches.
    public static func fusedQKVMatmul(
        ...
        guard (qWidth == 4096 && kWidth == 2048 && vWidth == 2048)
            || (qWidth == 8192 && kWidth == 1024 && vWidth == 1024),
            qScales.shape == [qWidth, inputWidth / Self.groupSize],
            qBiases.shape == qScales.shape,
            kScales.shape == [kWidth, inputWidth / Self.groupSize],
            kBiases.shape == kScales.shape,
        ...
        let kernel =
            qWidth == 4096
            ? (tableReady ? tripleSlidingRspKernel : tripleSlidingKernel)
            : (tableReady ? tripleFullRspKernel : tripleFullKernel)
        ...
        let outputs = kernel(
            ...
            grid: (simdWidth, (yTiles / tilesPerGroup) * simdGroups, 1),
            threadGroup: (simdWidth, simdGroups, 1),
            outputShapes: [[batch, sequence, qWidth], [batch, sequence, kWidth], [batch, sequence, vWidth]],
