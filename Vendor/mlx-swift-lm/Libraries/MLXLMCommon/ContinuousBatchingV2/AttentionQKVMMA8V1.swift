...
// Exact B=8,L=2 twin of the promoted M8 body.  The input and output retain
// their native interleaved [batch, length, width] layout.  Each column owns an
// independent copy of the M8 accumulator chain, while the packed weight,
// scale, and affine-bias packet is loaded once and consumed by both chains.
template <typename T, int KS>
METAL_FUNC void qkv_mma16_affine4_g64_impl(
    const device uint32_t* w,
    const device T* scales,
    const device T* biases,
    const device T* x,
...
"""
    ...
    private static let mma16Kernel = MLXFast.metalKernel(
        name: "cbv2_b8_l2_qkv_mma16_affine4_g64_tight_v1",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
            threadgroup float4 red[32];
            qkv_mma16_affine4_g64_impl<T, 2>(
                w, scales, biases, x, y,
                x_shape[x_ndim - 1], w_shape[0], int(tid.y) * 8, red,
                simdgroup_index_in_threadgroup,
                thread_index_in_simdgroup);
