// Tight-grid dispatch for the accepted affine4/g64 four-row stream QMV.
//
...
public enum CBv2AttentionQMV4QuadStreamV1 {
    private static let enabled: Bool = {
        guard
            let raw = ProcessInfo.processInfo.environment[
                "DARKBLOOM_CBV2_ATTENTION_QMV4_TIGHT_GRID"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()

    private static let batch = 8
    private static let inputDimension = 2816
    ...
    private static let supportedOutputDimensions = [1024, 2048, 4096, 8192]
    ...
    /// Returns nil unless the tensor and frozen affine quantization match the
    /// exact B=8 singleton attention projection contract.
    public static func matmul(
        ...
        guard enabled,
            let biases,
            supportedOutputDimensions.contains(outputDimension),
            x.dtype == .bfloat16,
