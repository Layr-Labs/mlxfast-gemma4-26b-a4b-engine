...
enum CBv2AttentionV1 {
    ...
    static func updateAndAttendLastQuery(
        ...
        return attendLastQueryCommitted(rows: rows, kind: kind, queries: queries,
            committed: committed, scale: scale, sinks: effectiveSinks, softcap: softcap)
    }

    /// Direct producers and ordinary row updates share exactly the same
    /// post-commit attention selection, including its disabled-path fallback.
    static func attendLastQueryCommitted(
        rows: [CBv2SequenceKV], kind: CBv2LayerKind, queries: MLXArray,
        committed: [(keys: MLXArray, values: MLXArray)],
