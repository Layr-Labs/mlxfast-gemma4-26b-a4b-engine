...
enum CBv2AttentionV1 {
    ...
    ///
    /// PREFILL-VECTOR-2PASS: Opt-in to lower block width to 8, enabling MLX's
    /// vector SDPA for sliding layers (head_dim=256). With L=8, vector kernel
    /// requirements are met (L<=8, head_dim in {64,96,128,256}, L*gqa<=32).
    ///
    /// M4 measurement: REGRESSION (prefill +14%) due to 8x more dispatches
    /// (128 vs 16 q-blocks/layer) outweighing scores traffic savings on this device.
    /// May be a win on M5 (ranked box) where dispatch overhead is lower.
    ///
    /// Exactness: Vector SDPA is bit-identical to composed path.
    ///
    /// Kill switch / opt-in: DARKBLOOM_CBV2_PREFILL_VECTOR_2PASS=1 enables 8.
    static let wideHeadQueryBlockSize: Int = {
        if let raw = ProcessInfo.processInfo.environment["DARKBLOOM_CBV2_ATTN_QUERY_BLOCK_WIDE"],
           let value = Int(raw), value >= 0
        {
            return value
        }
        if let raw = ProcessInfo.processInfo.environment["DARKBLOOM_CBV2_PREFILL_VECTOR_2PASS"],
    ...
    private static func effectiveQueryBlockSize(
        ...
        if wideHeadQueryBlockSize == 8 {
            CBv2EngageMark.once("prefill-vector-2pass")
        }
