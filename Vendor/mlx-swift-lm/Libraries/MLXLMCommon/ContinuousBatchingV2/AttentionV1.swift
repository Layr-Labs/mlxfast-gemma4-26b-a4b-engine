...
enum CBv2AttentionV1 {
    ...
    /// WRITE-023 kill switch.  The enabled path folds full sliding-ring K/V
    /// writes into attention pass A; `0` restores the promoted standalone
    /// slice-update path inside the same binary.
        ...
        if L == 1 {
                ...
                scale: scale, sinks: effectiveSinks, softcap: softcap)
            {
                ...
                if ringRows.count == B && ringRows.allSatisfy({ $0.decodeRingView != nil }) {
                    ...
                    {
                        let views = ringRows.compactMap { $0.fullDecodeRingBeforeWrite() }
                        if views.count == B,
                            let result = CBv2RaggedTwoPassDecodeAttentionV1
                                .attendRingWriting(
                                    queries: queries, newKeys: keys, newValues: values,
                                    keys: views.map(\.keys), values: views.map(\.values),
