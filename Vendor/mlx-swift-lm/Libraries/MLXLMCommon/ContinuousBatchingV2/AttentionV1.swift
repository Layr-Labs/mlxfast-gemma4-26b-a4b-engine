...
enum CBv2AttentionV1 {
    ...
    /// Wide MTP verify, row-major: `queries` `[B*C, H, 1, D]`, K/V
    /// `[B*C, KV, 1, D]` (row = cohort row * C + column). Serves the sliding
    /// layers' q4 fused road in ONE dispatch per layer and the full layers'
    /// ragged D512 road in one dispatch per stage, with every column's
    /// arithmetic the single-column kernel's; nil when the road is not
    /// available (the caller folds and serializes per column instead).
    static func updateAndAttendColumns(
        ...
        let startArray: MLXArray = {
            if let base = CBv2MTPWideVerifyContext.deviceBase {
                return ((base + Int32(1)) % Int32(ringRows[0].window)).asType(.uint32)
            }
            return MLXArray(preWrite.map { UInt32($0.start) }, [B])
        }()
    ...
    /// Full-attention twin of the sliding columns road: one ragged D512 store
    /// for every column, then per-(row, column) key lengths `base + c + 1`
    /// through the `_columns_v1` kernels. Host counters never lag below the
    /// device truth, so they bound the last column's grid and scratch stride.
    private static func updateAndAttendFullColumns(
        ...
        guard fullRows.count == rows.count,
            ...
            let fused = CBv2RaggedComposedD512DecodeAttentionV1.updateAndAttendWritingRagged(
                ...
                scale: scale, sinks: nil, softcap: nil,
                lengths: CBv2MTPWideVerifyContext.deviceBase.map { $0 + Int32(1) }
    ...
    static func updateAndAttend(
        ...
        if L == 1 {
                ...
                scale: scale, sinks: effectiveSinks, softcap: softcap)
            {
                ...
                if ringRows.count == B && ringRows.allSatisfy({ $0.decodeRingView != nil }) {
                        ...
                        allowFusedRingWrite, let decodeRingWriteFence
                    {
                        ...
                        // Chained MTP rounds: the first retained slot after
                        // this column's write is (base + column + 1) mod
                        // window on device; the host counters lag by the
                        // previous round's unconfirmed columns.
                        let startArray: MLXArray = {
                            if let base = CBv2MTPWideVerifyContext.deviceBase {
                                let window = Int32(ringRows[0].window)
                                return ((base + Int32(deviceColumn + 1)) % window).asType(.uint32)
                            }
                            return MLXArray(preWrite.map { UInt32($0.start) }, [B])
                        }()
                        if preWrite.count == B,
            ...
            // Rows at DIFFERENT offsets (per-row MTP commits) cannot take the
            // uniform fused road below; the ragged twin stores and attends
            // each row at its own length through the same kernel bodies.
            if let decodeRingWriteFence, allowFusedRingWrite,
                ...
                let fused = CBv2RaggedComposedD512DecodeAttentionV1
                    .updateAndAttendWritingRagged(
                        ...
                        previousWriteFence: decodeRingWriteFence.value,
                        scale: scale, sinks: effectiveSinks, softcap: softcap,
                        lengths: CBv2MTPWideVerifyContext.deviceBase.map {
                            $0 + Int32(deviceColumn + 1)
                        } ?? MLXArray(fullRows.map { Int32($0.absoluteOffset + 1) }),
                        // Host counters never lag below the device truth, so
                        // they bound the grid and the scratch stride.
