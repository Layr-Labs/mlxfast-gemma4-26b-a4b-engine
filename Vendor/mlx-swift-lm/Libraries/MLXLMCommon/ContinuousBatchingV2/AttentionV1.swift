...
enum CBv2AttentionV1 {
    ...
    static func updateAndAttendLastQuery(
        ...
        var cachedKeyRows: [MLXArray] = []
        var cachedValueRows: [MLXArray] = []
        cachedKeyRows.reserveCapacity(batch)
        cachedValueRows.reserveCapacity(batch)
        for (index, row) in rows.enumerated() {
            let (cachedKeys, cachedValues) = row.update(
                keys: keys[index ..< index + 1],
                values: values[index ..< index + 1])
        ...
        if canUseRaggedTwoPassFullDecode(
            batch: batch, cacheKind: kind, queryKind: kind,
            scale: scale, sinks: effectiveSinks, softcap: softcap),
            let output = CBv2RaggedTwoPassFullDecodeAttentionV1.attend(
                queries: queries, keys: cachedKeyRows, values: cachedValueRows,
                scale: scale)
        {
            return output
    ...
    private static func canUseRaggedTwoPassFullDecode(
        ...
        if case .full = cacheKind.attention { return true }
        return false
    }
