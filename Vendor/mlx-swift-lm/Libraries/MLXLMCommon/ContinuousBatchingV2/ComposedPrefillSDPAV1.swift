...
enum CBv2ComposedPrefillSDPAV1 {
    ...
    /// Fuses QK^T dot product, causal/sliding mask, online softmax normalization,
    ...
    private static let flashAttentionKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_fused_prefill_flash_sdpa_v2",
        ...
        source: """
            ...
            const int q_abs_pos = valid_q ? ((KL - L) + q_row) : 0;
            ...
            const int k_start = (WINDOW > 0 && valid_q) ? max(0, q_abs_pos - WINDOW + 1) : 0;
            ...
            int group_k_start = 0;
            if (WINDOW > 0) {
                const int min_q_abs_pos = (KL - L) + q_row_base;
                group_k_start = max(0, min_q_abs_pos - WINDOW + 1);
            }
            ...
            for (int k_idx = group_k_start; k_idx < group_k_end; ++k_idx) {
                ...
                if (valid_q && k_idx >= k_start && k_idx < k_end) {
                    float dot_val = 0.0f;
                    #pragma clang loop unroll(full)
                    for (int e = 0; e < vecs_per_lane; ++e) {
