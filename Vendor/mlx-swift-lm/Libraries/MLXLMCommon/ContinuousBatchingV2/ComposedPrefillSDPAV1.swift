...
enum CBv2PrefillSoftmaxVecV1 {
    ...
    // These values are immutable launch metadata. Keeping the common key
    // lengths resident avoids rebuilding and evaluating the params carrier
    // for every prefill rectangle.
    ...
    nonisolated(unsafe) private static let precomputedCausalParams: [Int: MLXArray] = {
        var table: [Int: MLXArray] = [:]
        for axis in [128, 256, 384, 512, 640, 768, 896, 1024] {
            let tg = ((axis + 3) / 4 + 31) / 32 * 32
            let numSimdgroups = tg / 32
            let arr = MLXArray([UInt32(axis), UInt32(numSimdgroups), UInt32(128)])
            eval(arr)
            table[axis] = arr
        }
        return table
    ...
    @inline(__always)
    fileprivate static func getCausalParams(axisSize: Int, numSimdgroups: Int, causalLen: Int) -> MLXArray {
        if causalLen == 128, let hit = precomputedCausalParams[axisSize] {
            return hit
        }
        return MLXArray([UInt32(axisSize), UInt32(numSimdgroups), UInt32(causalLen)])
    }
...
enum CBv2PrefillAttnTrafficV1 {
    ...
    static let causalStatsEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_CAUSAL_STATS_ELIDE_DRAM_V1"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()
    ...
    private static let causalStatsSource: String? = statsSource.flatMap { text in
        ...
        // Anchor must occur exactly once. On drift the at2 twin is simply
        // absent and every call falls back to the at1 kernel; nothing is
        // written to any stream inside the scored process.
        guard text.components(separatedBy: load).count == 2 else { return nil }
    ...
    static func statistics(
        ...
        let paramsArray =
            causalLen > 0
            ? CBv2PrefillSoftmaxVecV1.getCausalParams(
                axisSize: axisSize, numSimdgroups: numSimdgroups, causalLen: causalLen)
            : CBv2PrefillSoftmaxVecV1.getParams(
    ...
    static func attend(
        ...
        guard let stats = statistics(scores: scores, values: values, causalRows: causalRows)
        else { return nil }
