...
enum CBv2ComposedPrefillSDPAV1 {
    ...
    /// bounds the family at eight distinct kernels. Ranked wide-head prefill
    /// uses query blocks of 64, so `kL` advances from 64 through 1024; this
    /// specialization handles the eight multiples of 128, about 53% of the
    /// score elements. Intervening 64-multiples keep the stock call because
    /// its padded lanes require the guarded arm. A longer full-attention
    /// history also stays on stock rather than adding a JIT compile.
