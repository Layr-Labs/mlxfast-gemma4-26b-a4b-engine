...
public enum CBv2DenseMLPGateUpMMAV1 {
    ...
    private static let enabled: Bool = {
        ...
        // This projection train changes the FP32 accumulation tree. Keep the
        // exact tight-QMV path as the shipped default; research runs must opt
        // into the MMA train explicitly.
        else { return false }
    ...
    /// An affine-8/group-64 activation-sum plane produced by an upstream
    /// kernel while writing `source`.  The source identity is part of the
    /// contract: a table for one normalized tensor must never be consumed by
    /// another tensor that merely has the same shape.
    public struct ActivationSums {
        fileprivate let source: MLXArray
        fileprivate let values: MLXArray
    }

    /// Whether an upstream producer should spend work emitting this table.
    /// Keeping this tied to the MMA kill switch makes ablations restore the
    /// prior graph, including its standalone prepass behavior.
    public static var acceptsProducedActivationSums: Bool { enabled }

    /// Seal an upstream-produced plane after validating the exact ranked
    /// geometry.  Construction remains source-owned; consumers additionally
    /// require pointer identity before using the values.
    public static func producedActivationSums(
        values: MLXArray, for source: MLXArray
    ) -> ActivationSums? {
        let cells = batch * (inputWidth / groupSize)
        guard acceptsProducedActivationSums,
            source.dtype == .bfloat16,
            source.shape == [batch, sequence, inputWidth],
            source.size == batch * sequence * inputWidth,
    ...
    public static func apply(
        ...
        let xSums: MLXArray
        if let activationSums, activationSums.source === x,
            ...
            activationSums.values.size == sumCells
        {
            ...
            CBv2EngageMark.once("gemma4-dense-gateup-producer-xsum")
        } else {
            let sumThreads = 128
            let sumThreadgroups = (sumCells + sumThreads - 1) / sumThreads
            xSums = activationSumKernel(
                [flatX],
