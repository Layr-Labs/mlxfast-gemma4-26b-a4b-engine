...
"""
    ...
    private static let compiledPairEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        guard environment["MLX_DISABLE_COMPILE"] == nil else { return false }
        guard let raw = environment["DARKBLOOM_GEMMA4_COMPILED_DENSE_MLP"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()
    ...
    public static var compiledPairAvailable: Bool {
        compiledPairEnabled && MLXHardwareInfo.isCompiledDecodeSupported
            && StreamOrDevice.default == .gpu
            && enabled && denseGeluEpilogueEnabled && mma8DownEnabled
            && mma8DownStaticKEnabled && mma8DownStaticNEnabled
            && !mma8DownLaneSumsEnabled
    }

    // One reusable identity across layers. All projection tensors and the
    // router-score ordering dependency are substituted from current arguments.
    private static let compiledPair: @Sendable ([MLXArray]) -> [MLXArray] =
        MLX.compile(shapeless: false) { inputs in
            let activated = gateUpGeluCall(Array(inputs[0..<4]))
            let held = MLX.depends(input: activated, dependencies: [inputs[7]])
            return [downStaticKNCall([held, inputs[4], inputs[5], inputs[6]])]
        }

    /// The default ZIP dense pair: x, joined GU w/s/b, DOWN w/s/b, scores.
    /// Both projections must use the supplied affine quantization contract.
    /// The score tensor is an ordering dependency, not a numerical operand.
    public static func compiledGateUpGeluDown(
    ...
    private static func downStaticKNCall(_ inputs: [MLXArray]) -> MLXArray {
