...
// shape, stride or quantization geometry fails closed to the prior path.
...
"""
    ...
    @inline(__always)
    private static func rowContiguous(_ a: MLXArray, rowStride: Int) -> Bool {
        let strides = a.strides
        return strides.count == 2
            && strides[0] == rowStride
            && strides[1] == 1
    }
    ...
    public static func matmul(
        ...
        guard liveShape(inDim: inDim, outDim: outDim),
            ...
            rowContiguous(weight, rowStride: inDim * Self.bits / 32),
            rowContiguous(scales, rowStride: inDim / Self.groupSize),
            rowContiguous(biases, rowStride: inDim / Self.groupSize)
        else { return nil }
