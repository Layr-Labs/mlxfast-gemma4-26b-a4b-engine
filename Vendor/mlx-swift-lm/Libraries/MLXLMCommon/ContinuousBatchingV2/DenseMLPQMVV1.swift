...
public enum CBv2DenseMLPQMVV1 {
    ...
    public static var fusedGateUpGeluProducerEnabled: Bool {
        enabled && fusedGateUpGeluEnabled
    }
    ...
    private static let fusedSimdGroups = 4
...
"""
    ...
    /// DMLP-FUSED-GU-004 keeps the promoted DMLP-002 dot-product order, but
    ...
    ///
    /// Four SIMD groups own two output rows each. The original fused draft used
    /// two groups with four rows each, doubling the incumbent single-plane
    /// accumulator set from sixteen to thirty-two floats per thread before
    /// packed weights, scales and biases. This 4x2 ownership keeps the same
    /// eight-row tile and identical row arithmetic while returning the live
    /// accumulator count to sixteen. The same close can still emit the exact
    /// promoted down-plane lane table when that opt-in path is requested.
...
"""
    ...
    private static let fusedGateUpGeluKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_dense_mlp_dual_plane_gelu_xsum_v2",
        inputNames: [
            "x",
    ...
    /// The current promoted frontier defaults down-lane tables OFF. Derive an
    /// otherwise identical producer that stops after the BF16 GELU output, so
    /// the larger gate/up fusion does not silently reintroduce that table's
    /// threadgroup reduction or stores.
    private static let fusedGateUpGeluNoLaneHeader: String = {
        var result = fusedGateUpGeluHeader
        ...
        replaceOnce(
            "qmv_affine8_g64_dual_plane_gelu_xsum_impl(",
            with: "qmv_affine8_g64_dual_plane_gelu_impl(")
        replaceOnce(
            """
                device T* y3,
                device float2* lane_sums,
        ...
        let startMarker =
            "\n  threadgroup_barrier(mem_flags::mem_threadgroup);"
        guard let start = result.range(of: startMarker),
            let end = result.range(
                of: "\n}",
                options: .backwards,
                range: start.upperBound..<result.endIndex)
        else {
            preconditionFailure("DMLP-FUSED-GU no-lane transform marker missing")
        }
        result.removeSubrange(start.lowerBound..<end.lowerBound)
        return result
    }()

    private static let fusedGateUpGeluNoLaneKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_dense_mlp_dual_plane_gelu_v2",
        inputNames: [
            "x",
            "gateWeight", "gateScales", "gateBiases",
            "upWeight", "upScales", "upBiases",
        ...
        source: """
            ...
            qmv_affine8_g64_dual_plane_gelu_impl<T, 64, 8>(
                gateWeight, gateScales, gateBiases,
                upWeight, upScales, upBiases,
                x + first_m * in_vec_size,
                x + (first_m + 1) * in_vec_size,
            ...
            return;
            """,
        header: fusedGateUpGeluNoLaneHeader,
        ensureRowContiguous: true)
...
"""
    ...
    public static func fusedGateUpGelu(
        ...
        let grid = (
            ...
            (outDim / outputsPerGroup) * fusedSimdGroups,
            1
        )
        let values: MLXArray
        let downLaneSums: DownLaneSums?
        if downLaneSumsProducerEnabled {
            let groups = outDim / groupSize
            let outputs = fusedGateUpGeluKernel(
                inputs,
                template: [("T", x.dtype)],
                grid: grid,
                threadGroup: (simdWidth, fusedSimdGroups, 1),
                outputShapes: [
            ...
            values = fusedGateUpGeluNoLaneKernel(
                inputs,
                template: [("T", x.dtype)],
                grid: grid,
                threadGroup: (simdWidth, fusedSimdGroups, 1),
