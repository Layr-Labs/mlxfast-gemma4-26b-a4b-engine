...
"""
    ...
    /// Dense MLP down-plane output stride supplied at kernel selection. The
    /// admission predicate pins that stride: the only admitted pair with an
    /// input width of 2112 has an output width of 2816, and this branch is
    /// entered only on that pair. The promoted body nevertheless reads it from
    /// `w_shape[0]` inside the kernel.
    ///
    ...
    /// The body no longer names `w_shape`, so the shape parameter the custom
    /// kernel path emits for that operand is not emitted either.
    ///
    /// Exactness. The constant is the value the admission predicate already
    ...
    /// the carry-two group walk, the BFILL steps and the KS = 2 close are the
    /// promoted text word for word, and the Metal libraries on this path are
    /// built with fast math explicitly disabled. Bit-identical by construction.
    ///
    /// Scope. The down plane only. The gate/up plane, the runtime-width
    /// registration, the lane-sum registrations and the activation-sum
    /// registrations keep their promoted headers and their promoted names.
    ///
    /// `DARKBLOOM_GEMMA4_MLP_DOWN_STATIC_N=0` restores the promoted down
    /// kernel, under its promoted name and its promoted header text, in the
    /// same executable. Engage mark: `mlp-down-static-n`.
    private static let mma8DownStaticNEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_MLP_DOWN_STATIC_N"]
        else { return true }
        return !["0", "false", "no", "off"].contains(
            raw.trimmingCharacters(in: .whitespaces).lowercased())
    }()

    /// `mma8DownStaticKHeader` with the runtime `N` parameter bound next to
    /// the body's own compile-time `K`.
    private static let mma8DownStaticKNHeader: String = {
    ...
    private static let mma8DownStaticKNKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_dense_mlp_mma8_affine8_g64_down_k2112_n2816_carry2_bfill_v1",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
    ...
    private static let mma8GateUpGeluHeader: String = {
        ...
        func replaceOnce(_ old: String, with new: String) {
            precondition(
                ...
                "mma8GateUpGeluHeader: pattern not found exactly once")
            result = result.replacingOccurrences(of: old, with: new)
        }
        // Written as explicit "\n"-joined strings: the pasted Metal body is
        // indented two and four spaces, and a Swift multiline literal here
        // re-indents it, so the patterns silently stop matching and the
        // precondition traps with no message in a release build.
        replaceOnce(
            "gemma4_qmv_mma8_affine8_g64_gateup_k2816_impl(",
            with: "gemma4_qmv_mma8_affine8_g64_gateup_k2816_acc(")
        // No store: the destination and its row pitch go away.
    ...
    public static let denseGeluEpilogueEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_DENSE_GELU_EPILOGUE"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()
    ...
    public static func gateUpGelu(
        ...
        guard denseGeluEpilogueEnabled, enabled,
            groupSize == Self.groupSize, bits == Self.bits, mode == .affine,
            let biases,
            x.dtype == .bfloat16, scales.dtype == x.dtype,
            biases.dtype == x.dtype, weight.dtype == .uint32,
    ...
    /// Dense MLP gate/up output stride supplied at kernel selection, the down
    /// plane's `DARKBLOOM_GEMMA4_MLP_DOWN_STATIC_N` on the other plane.
    ///
    /// Mechanism. The stride reaches the body only through the store
    /// expressions at the tail, `y[c.fn * N + n0 + c.fm]`, so binding it as a
    /// compile-time constant lets those addresses be formed from a literal.
    ...
    /// kernel path emits for that operand is not emitted either.
    ///
    /// Exactness. The constant is the value the dispatch already tested. The
    /// change is integer addressing only. No floating-point expression is
    /// introduced, removed, reordered, retyped or reassociated; the K = 2816
    /// group walk, the BFILL steps and the KS = 2 close are the promoted text
    /// word for word. Bit-identical by construction.
    ///
    /// Scope. The joined gate|up plane only. `liveShape` admits TWO output
    /// widths against the 2816 input -- 4224 for the joined plane and 2112 for
    /// the split gate-only or up-only call -- so unlike down the width is not
    /// pinned by admission. It is pinned by the dispatch instead: this kernel
    /// is selected only on `outDim == 4224`, and the 2112 arm keeps the
    /// promoted runtime-width registration under its promoted name.
    ///
    ...
    /// `mma8GateUpStaticKHeader` with the runtime `N` parameter bound next to
    /// the body's own compile-time `K`.
    ...
    private static let mma8GateUpStaticKNKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_dense_mlp_mma8_affine8_g64_gateup_k2816_n4224_u2_v1",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
