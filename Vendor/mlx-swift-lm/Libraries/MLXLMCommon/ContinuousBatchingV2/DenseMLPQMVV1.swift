...
//
// IPG3 tight pack (M8_IPG=3, IPG3_TIGHT, PACK=3+3+2, TAIL=2,
// DIRECT_NIBBLES=false). The stock affine-8 wide-N tier claimed four rows
// per group (`first_m = tid.x * 4`, two live x-groups). This launch owns the
// two dense MLP geometries and dispatches three x-groups packing 3+3+2:
// groups 0 and 1 through the affine8 triple lemma, group 2 through the pair.
// Not two quads. The y grid, threadgroup shape, K loop, g64-aligned tail and
// simd reductions stay the stream family's own.
//
...
public enum CBv2DenseMLPQMVV1 {
    ...
    private static let m8Ipg = 3
    private static let ipg3Tight = true
    private static let packHead = 3
    private static let packTail = 2
    private static let directNibbles = false
    ...
    /// Tight x-group count for M=8, IPG=3, PACK=3+3+2: two triple tiles plus
    /// one TAIL=2 pair, not `batch / 4` quads.
    private static let xGroups: Int = {
        precondition(ipg3Tight)
        precondition(!directNibbles)
        precondition(m8Ipg == packHead)
        let tail = batch % m8Ipg
        return (batch / m8Ipg) + (tail == packTail ? 1 : 0)
    }()
    ...
    /// The affine-8 helpers below are the current promoted stream bodies.
    /// Pair and triple reuse `load_vector` and `qdot_affine8_registered`
    /// already carried by the tied-head replica. Quad stays in the vendored
    /// MLX kernel; this tight launch routes 3+3+2 through triple then pair.
    ///
...
"""
    ...
    /// which replace only the activation sum. Pair and triple still load x
    /// into the same register array; every qdot, K accumulation and simd
    /// reduction remains text-for-text identical.
    private static let activationSumKernelHeader: String = {
        ...
        func replaceAll(_ old: String, with new: String, occurrences: Int) {
            precondition(result.components(separatedBy: old).count == occurrences + 1)
            result = result.replacingOccurrences(of: old, with: new)
        }
        ...
        for (old, new, occurrences) in loads {
            precondition(result.components(separatedBy: old).count == occurrences + 1)
            result = result.replacingOccurrences(of: old, with: new)
        }
        return result
    ...
    private static let kernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_dense_mlp_qmv_affine8_g64_ipg3_stream_v1",
    ...
    private static let activationSumQMVKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_dense_mlp_qmv_affine8_g64_ipg3_stream_xsum_v1",
