        ...
        replaceOnce(
            ...
            with: """
              ...
              for (int gi = 0; gi < nGroups; ++gi) {
                const int g = g0 + gi;
            """)
        // DMLP-GATEUP-CARRY2-5F4B: the two-deep weight-operand register carry
        // already resident on the dense down plane (DMLP-DOWN-CARRY2-021,
        // this file), transplanted verbatim onto the gate|up plane. Every
        // address is a function of the group index alone, the loop count is a
        // compile-time constant here, and `g` is monotone, so the hand-off is
        // register renaming: one group's codes/scale/bias stay resident while
        // the next group's are read an iteration ahead of their use. The
        // look-aheads are min-clamped to the last valid group, so the final
        // one re-reads a group already read and discards it. The consuming
        // statements keep their position, so the emitted arithmetic — and the
        // floating-point contraction the close depends on — is word for word
        // what the incumbent emits.
        replaceOnce(
            """
              simdgroup_float8x8 B0, B1, B2, B3, B4, B5, B6, B7;
            """,
