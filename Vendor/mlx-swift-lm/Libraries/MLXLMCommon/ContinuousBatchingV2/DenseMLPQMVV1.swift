...
public enum CBv2DenseMLPQMVV1 {
    ...
    /// Give the now-promoted gate/up matrix-unit body its fixed K=2816 at
    /// compile time without forcing an unroll or adding operand carries.
    /// This preserves the incumbent instruction schedule while allowing
    /// address arithmetic and loop bounds to constant-fold.
    private static let mma8GateUpConstKEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_MLP_GATEUP_CONST_K"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()
...
"""
    ...
    /// A conservative gate/up specialization: only K and its derived group
    /// count become compile-time constants. The group order, loads, shuffles,
    /// MMA sequence, and accumulation order remain the incumbent body's.
    private static let mma8GateUpConstKHeader: String = {
        var result = mma8KernelHeader
        func replaceOnce(_ old: String, with new: String) {
            precondition(result.components(separatedBy: old).count == 2)
            result = result.replacingOccurrences(of: old, with: new)
        ...
        replaceOnce(
            "gemma4_qmv_mma8_affine8_g64_impl(",
            with: "gemma4_qmv_mma8_affine8_g64_gateup_k2816_const_impl(")
        replaceOnce(
            """
                device T* y,
                const int K,
    ...
    private static let mma8GateUpConstKKernel = MLXFast.metalKernel(
        name: "cbv2_b8_l1_dense_mlp_mma8_affine8_g64_gateup_k2816_const_v1",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y"],
        source: """
            const uint3 tid = threadgroup_position_in_grid;
            threadgroup float2 red[32];
            gemma4_qmv_mma8_affine8_g64_gateup_k2816_const_impl<T, 2>(
                w, scales, biases, x, y,
                w_shape[0], int(tid.y) * 8, red,
                simdgroup_index_in_threadgroup,
                thread_index_in_simdgroup);
...
"""
    ...
    public static func matmul(
        ...
        if isGateUp ? mma8GateUpEnabled : mma8DownEnabled {
            ...
            if isGateUp && mma8GateUpConstKEnabled {
                CBv2EngageMark.once("mlp-gateup-const-k")
                selectedMMA = mma8GateUpConstKKernel
