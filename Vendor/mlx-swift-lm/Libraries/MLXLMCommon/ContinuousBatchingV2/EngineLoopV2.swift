...
public final class EngineLoopV2: @unchecked Sendable {
    ...
    /// Exact scored-shape shortcut around logits transforms and sampling.
    /// It is deliberately stricter than the ordinary greedy pipeline: the
    /// production sampler must be installed, all eight rows must request
    /// unmodified greedy decoding, and no constraint or logprob output may
    /// observe logits. The model still runs the complete target forward.
    private func directGreedyDecode(
        rowStates: [[CBv2SequenceKV?]],
        tokens: MLXArray,
        params: [CBv2SamplingParams],
        requestIDs: [CBv2RequestID]
    ) -> (sampledTokens: MLXArray, cacheInnerState: [MLXArray])? {
        let scoredBatch = 8
        guard tokens.shape == [scoredBatch, 1],
            rowStates.count == scoredBatch,
            params.count == scoredBatch,
            requestIDs.count == scoredBatch,
            sampler is CBv2DefaultSampler,
            let directModel = model as? CBv2DirectGreedySteppableModel,
            directModel.supportsDirectGreedyTokens
        else { return nil }

        for (index, param) in params.enumerated() {
            guard param.temperature < LogitsPipelineV2.greedyEpsilon,
                param.topP == 1,
                param.topK == 0,
                param.minP == 0,
                param.repetitionPenalty == 1,
                param.frequencyPenalty == 0,
                param.presencePenalty == 0,
                param.logitBias.isEmpty,
                param.topLogprobs == 0,
                scheduler.record(for: requestIDs[index])?.request.tokenConstraint == nil
            else { return nil }
        }
        ...
        precondition(
            sampled.dtype == .int32 && sampled.shape == [scoredBatch],
            "CBv2 direct greedy model must return int32 [8]")
    ...
    func executeMixed(_ plan: CBv2StepPlan) -> CBv2InFlightStep? {
        ...
        if !decodeRows.isEmpty {
            ...
            if let direct = directGreedyDecode(
                ...
                params: decodeParams, requestIDs: decodeIDs)
            {
                decodeSampled = direct.sampledTokens
