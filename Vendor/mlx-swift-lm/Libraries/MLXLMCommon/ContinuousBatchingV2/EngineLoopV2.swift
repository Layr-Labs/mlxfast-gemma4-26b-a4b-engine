...
/// COMPACT-INFLIGHT-PARTICIPANTS-D7E0. The participant set of one step is
/// built from a plan-order row list that is already unique and never exceeds
/// the batch width, so the hash table the incumbent builds per step buys
/// nothing over a linear scan of that same list. Membership answers and the
/// visited set are identical; only the container and the visit order change,
/// and every consumer of the visit is a per-id independent update.
/// `DARKBLOOM_CBV2_COMPACT_INFLIGHT_PARTICIPANTS=0` restores the hashed
