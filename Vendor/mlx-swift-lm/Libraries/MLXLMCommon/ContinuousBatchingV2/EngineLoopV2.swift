...
let cbv2PrefillWriteTailFenceEnabled: Bool = {
    guard let raw = ProcessInfo.processInfo.environment["DARKBLOOM_CBV2_PREFILL_WRITE_TAIL_FENCE"]
    else { return false }
    return !["0", "false", "no", "off"].contains(raw.lowercased())
}()
...
final class CBv2InFlightStep {
    ...
    var launchedByChain = false
    var gateFencedNanos: UInt64?
...
public final class EngineLoopV2: @unchecked Sendable {
    ...
    private func finalize(_ step: CBv2InFlightStep, now: ContinuousClock.Instant) {
        ...
        if CBv2MTPDeviceGate.measurementActive,
            step.launchedByChain, step.mtpRound == nil,
            step.participants.count == 8, step.sampledRows.count == 8
        {
            _ = CBv2MTPDeviceGate.recordWarmStep(
                .chainedPlain(rows: 8),
                wallTimeNanos: DispatchTime.now().uptimeNanoseconds &- step.wallStartedNanos)
        }
        if CBv2StepProfiler.enabled {
        ...
        if step.mtpRound != nil {
            ...
            if CBv2MTPDeviceGate.measurementActive,
                step.launchedByChain,
                let verify = step.mtpRound?.verify,
                verify.rows.count == 8,
                CBv2MTPWideVerifyContext.enabled
            {
                if CBv2MTPDeviceGate.recordWarmStep(
                    .chainedWideRound(depth: verify.k, rows: verify.rows.count),
                    wallTimeNanos: (step.gateFencedNanos
                        ?? DispatchTime.now().uptimeNanoseconds)
                        &- step.wallStartedNanos)
                {
                    mtp?.removeAllRequestState()
                }
            }
            if !CBv2MTPDeviceGate.speculationEnabled {
                mtp?.removeAllRequestState()
            }
        }
