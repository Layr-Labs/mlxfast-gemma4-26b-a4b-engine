...
public enum Gemma4DecodeFusedGUV1 {
    ...
    /// Prepare the current activation once; never change checkpoint storage.
    /// The off path retains the incumbent kernel descriptor and its inputs.
    private static let inputPrepackEnabled: Bool = {
        let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_GU_INPUT_PREPACK9_V1"]
        return raw.map {
    ...
    static func call(_ inputs: [MLXArray], taggedRoute: Bool = false) -> MLXArray {
        let prepacked = inputPrepackEnabled && runCap == 4 && inputs.count == 6
            && inputs[3].dtype == .bfloat16 && inputs[3].size == 8 * 2816
            && inputs[3].shape.last == 2816
        var arguments = inputs
        if prepacked {
            arguments[3] = activationPrepackKernel(
                [inputs[3]], grid: (2816, 1, 1), threadGroup: (256, 1, 1),
                outputShapes: [[8, 3168]], outputDTypes: [.float32])[0]
        }
        let selected = prepacked
            ? (taggedRoute ? prepackedKernelTagged : prepackedKernelGeneral)
    ...
    private static func makeKernel(
        tagged: Bool, prepacked: Bool = false
    ) -> MLXFast.MLXFastKernel {
        MLXFast.metalKernel(
        name: "gemma4_b8_decode_gateup_geglu_threadgroup_v2_solo1"
            + (tagged ? "_tagged_v1" : "")
            + (prepacked ? "_input_prepack9_v1" : ""),
...
"""#,
        header: kernelHeader(tagged: tagged, prepacked: prepacked),
        ensureRowContiguous: true)
    }

    private static func kernelHeader(
        tagged: Bool, prepacked: Bool = false
    ) -> String {
        let source = "#define GU_RUN_CAP \(runCap)\n"
...
"""#
        return prepacked ? prepackedHeader(source) : source
    }

    /// The model guard fixes K=2816: eleven complete 256-value blocks.
    /// The four helpers keep every weight, affine, accumulation and close
    /// expression. Only their activation loaders and packet strides differ.
    private static func prepackedHeader(_ source: String) -> String {
        let changes: [(String, String)] = [
            ("template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_pair_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device T* x0,\n    const device T* x1,\n    threadgroup T* y0,\n    threadgroup T* y1,\n    const constant int& in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x0_thread[values_per_thread];\n  thread float x1_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n  thread float result1[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * values_per_thread;\n  x1 += simd_lid * values_per_thread;\n  y0 += out_row;\n  y1 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] = *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum0 = load_vector<T, float, values_per_thread, 4>(x0, x0_thread);\n    float sum1 = load_vector<T, float, values_per_thread, 4>(x1, x1_thread);\n\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      float dot0;\n      float dot1;\n      qdot_affine4_pair_word<float, values_per_thread>(\n          packed[row], x0_thread, x1_thread, scale_local[row], bias_local[row], sum0, sum1, dot0, dot1);\n      result0[row] += dot0;\n      result1[row] += dot1;\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += block_size;\n    x1 += block_size;\n  }\n\n  // Every Gemma 4 caller entering this specialized g64 path has K aligned to\n  // 64.  The final block therefore contains an integral number of complete\n  // eight-value lane packets (32 lanes for K=2816, 24 for expert down_proj\n  // K=704); no active lane needs the generic dynamic safe-tail loops.\n  const uint active_tail_lanes =\n      uint((in_vec_size - k) / values_per_thread);\n  if (simd_lid < active_tail_lanes) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] = *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum0 =\n        load_vector<T, float, values_per_thread, 4>(x0, x0_thread);\n    float sum1 =\n        load_vector<T, float, values_per_thread, 4>(x1, x1_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      float dot0;\n      float dot1;\n      qdot_affine4_pair_word<float, values_per_thread>(\n          packed[row], x0_thread, x1_thread, scale_local[row], bias_local[row], sum0, sum1, dot0, dot1);\n      result0[row] += dot0;\n      result1[row] += dot1;\n    }\n  }\n\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    result1[row] = simd_sum(result1[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n      y1[row] = static_cast<T>(result1[row]);\n    }\n  }\n}", "template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_pair_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device float* x0,\n    const device float* x1,\n    threadgroup T* y0,\n    threadgroup T* y1,\n    const constant int& in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x0_thread[values_per_thread];\n  thread float x1_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n  thread float result1[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * 9;\n  x1 += simd_lid * 9;\n  y0 += out_row;\n  y1 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] = *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum0 = gu_load_prepacked8(x0, x0_thread);\n    float sum1 = gu_load_prepacked8(x1, x1_thread);\n\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      float dot0;\n      float dot1;\n      qdot_affine4_pair_word<float, values_per_thread>(\n          packed[row], x0_thread, x1_thread, scale_local[row], bias_local[row], sum0, sum1, dot0, dot1);\n      result0[row] += dot0;\n      result1[row] += dot1;\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += (block_size / 8) * 9;\n    x1 += (block_size / 8) * 9;\n  }\n\n  // Every Gemma 4 caller entering this specialized g64 path has K aligned to\n  // 64.  The final block therefore contains an integral number of complete\n  // eight-value lane packets (32 lanes for K=2816, 24 for expert down_proj\n  // K=704); no active lane needs the generic dynamic safe-tail loops.\n  // K is the guarded fixed 2816, exactly eleven full 256-value blocks.\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    result1[row] = simd_sum(result1[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n      y1[row] = static_cast<T>(result1[row]);\n    }\n  }\n}"),
            ("template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_solo_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device T* x0,\n    threadgroup T* y0,\n    const constant int& in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x0_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * values_per_thread;\n  y0 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] = *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum0 = load_vector<T, float, values_per_thread, 4>(x0, x0_thread);\n\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x0_thread, scale_local[row], bias_local[row], sum0);\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += block_size;\n  }\n\n  // Same whole-packet tail contract as the pair path: the only caller enters\n  // with K=guK=2816, a whole number of 256-value blocks, so the final block\n  // holds complete eight-value lane packets and no lane takes this branch.\n  const uint active_tail_lanes =\n      uint((in_vec_size - k) / values_per_thread);\n  if (simd_lid < active_tail_lanes) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] = *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum0 =\n        load_vector<T, float, values_per_thread, 4>(x0, x0_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x0_thread, scale_local[row], bias_local[row], sum0);\n    }\n  }\n\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n    }\n  }\n}", "template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_solo_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device float* x0,\n    threadgroup T* y0,\n    const constant int& in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x0_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * 9;\n  y0 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] = *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum0 = gu_load_prepacked8(x0, x0_thread);\n\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x0_thread, scale_local[row], bias_local[row], sum0);\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += (block_size / 8) * 9;\n  }\n\n  // Same whole-packet tail contract as the pair path: the only caller enters\n  // with K=guK=2816, a whole number of 256-value blocks, so the final block\n  // holds complete eight-value lane packets and no lane takes this branch.\n  // K is the guarded fixed 2816, exactly eleven full 256-value blocks.\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n    }\n  }\n}"),
            ("template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_triple_stream_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device T* x0,\n    const device T* x1,\n    const device T* x2,\n    threadgroup T* y0,\n    threadgroup T* y1,\n    threadgroup T* y2,\n    const int in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n  thread float result1[results_per_simdgroup] = {0};\n  thread float result2[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * values_per_thread;\n  x1 += simd_lid * values_per_thread;\n  x2 += simd_lid * values_per_thread;\n  y0 += out_row;\n  y1 += out_row;\n  y2 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] =\n          *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum = load_vector<T, float, values_per_thread, 4>(x0, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = load_vector<T, float, values_per_thread, 4>(x1, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result1[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = load_vector<T, float, values_per_thread, 4>(x2, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result2[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += block_size;\n    x1 += block_size;\n    x2 += block_size;\n  }\n\n  const int remaining = clamp(\n      static_cast<int>(in_vec_size - k - simd_lid * values_per_thread),\n      0,\n      values_per_thread);\n  if (remaining > 0) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] =\n          *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum =\n        load_vector_safe<T, float, values_per_thread, 4>(x0, x_thread, remaining);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum =\n        load_vector_safe<T, float, values_per_thread, 4>(x1, x_thread, remaining);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result1[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum =\n        load_vector_safe<T, float, values_per_thread, 4>(x2, x_thread, remaining);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result2[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n  }\n\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    result1[row] = simd_sum(result1[row]);\n    result2[row] = simd_sum(result2[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n      y1[row] = static_cast<T>(result1[row]);\n      y2[row] = static_cast<T>(result2[row]);\n    }\n  }\n}", "template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_triple_stream_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device float* x0,\n    const device float* x1,\n    const device float* x2,\n    threadgroup T* y0,\n    threadgroup T* y1,\n    threadgroup T* y2,\n    const int in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n  thread float result1[results_per_simdgroup] = {0};\n  thread float result2[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * 9;\n  x1 += simd_lid * 9;\n  x2 += simd_lid * 9;\n  y0 += out_row;\n  y1 += out_row;\n  y2 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] =\n          *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum = gu_load_prepacked8(x0, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = gu_load_prepacked8(x1, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result1[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = gu_load_prepacked8(x2, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result2[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += (block_size / 8) * 9;\n    x1 += (block_size / 8) * 9;\n    x2 += (block_size / 8) * 9;\n  }\n\n  // K is the guarded fixed 2816, exactly eleven full 256-value blocks.\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    result1[row] = simd_sum(result1[row]);\n    result2[row] = simd_sum(result2[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n      y1[row] = static_cast<T>(result1[row]);\n      y2[row] = static_cast<T>(result2[row]);\n    }\n  }\n}"),
            ("template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_quad_stream_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device T* x0,\n    const device T* x1,\n    const device T* x2,\n    const device T* x3,\n    threadgroup T* y0,\n    threadgroup T* y1,\n    threadgroup T* y2,\n    threadgroup T* y3,\n    const int in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n  thread float result1[results_per_simdgroup] = {0};\n  thread float result2[results_per_simdgroup] = {0};\n  thread float result3[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * values_per_thread;\n  x1 += simd_lid * values_per_thread;\n  x2 += simd_lid * values_per_thread;\n  x3 += simd_lid * values_per_thread;\n  y0 += out_row;\n  y1 += out_row;\n  y2 += out_row;\n  y3 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] =\n          *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum = load_vector<T, float, values_per_thread, 4>(x0, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = load_vector<T, float, values_per_thread, 4>(x1, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result1[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = load_vector<T, float, values_per_thread, 4>(x2, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result2[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = load_vector<T, float, values_per_thread, 4>(x3, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result3[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += block_size;\n    x1 += block_size;\n    x2 += block_size;\n    x3 += block_size;\n  }\n\n  const int remaining = clamp(\n      static_cast<int>(in_vec_size - k - simd_lid * values_per_thread),\n      0,\n      values_per_thread);\n  if (remaining > 0) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] =\n          *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum =\n        load_vector_safe<T, float, values_per_thread, 4>(x0, x_thread, remaining);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum =\n        load_vector_safe<T, float, values_per_thread, 4>(x1, x_thread, remaining);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result1[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum =\n        load_vector_safe<T, float, values_per_thread, 4>(x2, x_thread, remaining);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result2[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum =\n        load_vector_safe<T, float, values_per_thread, 4>(x3, x_thread, remaining);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result3[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n  }\n\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    result1[row] = simd_sum(result1[row]);\n    result2[row] = simd_sum(result2[row]);\n    result3[row] = simd_sum(result3[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n      y1[row] = static_cast<T>(result1[row]);\n      y2[row] = static_cast<T>(result2[row]);\n      y3[row] = static_cast<T>(result3[row]);\n    }\n  }\n}", "template <typename T, const int group_size, const int bits>\nMETAL_FUNC void tg_qmv_affine4_g64_quad_stream_impl(\n    const device uint32_t* w,\n    const device T* scales,\n    const device T* biases,\n    const device float* x0,\n    const device float* x1,\n    const device float* x2,\n    const device float* x3,\n    threadgroup T* y0,\n    threadgroup T* y1,\n    threadgroup T* y2,\n    threadgroup T* y3,\n    const int in_vec_size,\n    uint3 tid [[threadgroup_position_in_grid]],\n    uint simd_gid [[simdgroup_index_in_threadgroup]],\n    uint simd_lid [[thread_index_in_simdgroup]]) {\n  constexpr int num_simdgroups = 2;\n  constexpr int results_per_simdgroup = 4;\n  constexpr int values_per_thread = 8;\n  constexpr int block_size = values_per_thread * SIMD_SIZE;\n  constexpr int bytes_per_thread = 4;\n  constexpr int scale_step_per_thread = 8;\n\n  const device uint8_t* ws = (const device uint8_t*)w;\n  thread float x_thread[values_per_thread];\n  thread uint packed[results_per_simdgroup];\n  thread float scale_local[results_per_simdgroup];\n  thread float bias_local[results_per_simdgroup];\n  thread float result0[results_per_simdgroup] = {0};\n  thread float result1[results_per_simdgroup] = {0};\n  thread float result2[results_per_simdgroup] = {0};\n  thread float result3[results_per_simdgroup] = {0};\n\n  const int in_vec_size_w = in_vec_size / 2;\n  const int in_vec_size_g = in_vec_size / 64;\n  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +\n      simd_gid * results_per_simdgroup;\n\n  ws += out_row * in_vec_size_w + simd_lid * bytes_per_thread;\n  scales += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  biases += out_row * in_vec_size_g + simd_lid / scale_step_per_thread;\n  x0 += simd_lid * 9;\n  x1 += simd_lid * 9;\n  x2 += simd_lid * 9;\n  x3 += simd_lid * 9;\n  y0 += out_row;\n  y1 += out_row;\n  y2 += out_row;\n  y3 += out_row;\n\n  int k = 0;\n  for (; k <= in_vec_size - block_size; k += block_size) {\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      packed[row] =\n          *((const device uint*)(ws + row * in_vec_size_w));\n      scale_local[row] = scales[row * in_vec_size_g];\n      bias_local[row] = biases[row * in_vec_size_g];\n    }\n\n    float sum = gu_load_prepacked8(x0, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result0[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = gu_load_prepacked8(x1, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result1[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = gu_load_prepacked8(x2, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result2[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n    sum = gu_load_prepacked8(x3, x_thread);\n    for (int row = 0; row < results_per_simdgroup; row++) {\n      result3[row] += qdot_affine4_registered_word<float, values_per_thread>(\n          packed[row], x_thread, scale_local[row], bias_local[row], sum);\n    }\n\n    ws += block_size / 2;\n    scales += block_size / 64;\n    biases += block_size / 64;\n    x0 += (block_size / 8) * 9;\n    x1 += (block_size / 8) * 9;\n    x2 += (block_size / 8) * 9;\n    x3 += (block_size / 8) * 9;\n  }\n\n  // K is the guarded fixed 2816, exactly eleven full 256-value blocks.\n  for (int row = 0; row < results_per_simdgroup; row++) {\n    result0[row] = simd_sum(result0[row]);\n    result1[row] = simd_sum(result1[row]);\n    result2[row] = simd_sum(result2[row]);\n    result3[row] = simd_sum(result3[row]);\n    if (simd_lid == 0) {\n      y0[row] = static_cast<T>(result0[row]);\n      y1[row] = static_cast<T>(result1[row]);\n      y2[row] = static_cast<T>(result2[row]);\n      y3[row] = static_cast<T>(result3[row]);\n    }\n  }\n}"),
            ("template<typename T>\nMETAL_FUNC void tg_execute_projection(const device uint* w,const device T* scales,const device T* biases,\n    const device T* x,const device uint* lhs,threadgroup T* y0,int rowStride,\n    const constant int& outputN,uint assignment,uint count,uint3 tid,uint sg,uint lane) {\n    const device T* x0=x+lhs[assignment]*2816;\n    if(count==1){tg_qmv_affine4_g64_solo_impl<T,64,4>(w,scales,biases,x0,y0,guK,tid,sg,lane);return;}\n    const device T* x1=x+lhs[assignment+1]*2816;threadgroup T* y1=y0+rowStride;\n    if(count==2){tg_qmv_affine4_g64_pair_impl<T,64,4>(w,scales,biases,x0,x1,y0,y1,guK,tid,sg,lane);return;}\n#if GU_RUN_CAP >= 3\n    const device T* x2=x+lhs[assignment+2]*2816;threadgroup T* y2=y1+rowStride;\n    if(count==3){tg_qmv_affine4_g64_triple_stream_impl<T,64,4>(w,scales,biases,x0,x1,x2,y0,y1,y2,guK,tid,sg,lane);return;}\n#endif\n#if GU_RUN_CAP >= 4\n    const device T* x3=x+lhs[assignment+3]*2816;threadgroup T* y3=y2+rowStride;\n    tg_qmv_affine4_g64_quad_stream_impl<T,64,4>(w,scales,biases,x0,x1,x2,x3,y0,y1,y2,y3,guK,tid,sg,lane);\n#endif\n}", "template<typename T>\nMETAL_FUNC void tg_execute_projection(const device uint* w,const device T* scales,const device T* biases,\n    const device float* x,const device uint* lhs,threadgroup T* y0,int rowStride,\n    const constant int& outputN,uint assignment,uint count,uint3 tid,uint sg,uint lane) {\n    const device float* x0=x+lhs[assignment]*3168;\n    if(count==1){tg_qmv_affine4_g64_solo_impl<T,64,4>(w,scales,biases,x0,y0,guK,tid,sg,lane);return;}\n    const device float* x1=x+lhs[assignment+1]*3168;threadgroup T* y1=y0+rowStride;\n    if(count==2){tg_qmv_affine4_g64_pair_impl<T,64,4>(w,scales,biases,x0,x1,y0,y1,guK,tid,sg,lane);return;}\n#if GU_RUN_CAP >= 3\n    const device float* x2=x+lhs[assignment+2]*3168;threadgroup T* y2=y1+rowStride;\n    if(count==3){tg_qmv_affine4_g64_triple_stream_impl<T,64,4>(w,scales,biases,x0,x1,x2,y0,y1,y2,guK,tid,sg,lane);return;}\n#endif\n#if GU_RUN_CAP >= 4\n    const device float* x3=x+lhs[assignment+3]*3168;threadgroup T* y3=y2+rowStride;\n    tg_qmv_affine4_g64_quad_stream_impl<T,64,4>(w,scales,biases,x0,x1,x2,x3,y0,y1,y2,y3,guK,tid,sg,lane);\n#endif\n}"),
        ]
        var result = source
        ...
        precondition(result.components(separatedBy: first).count == 2)
        return result.replacingOccurrences(
            of: first, with: "\ninline float gu_load_prepacked8(const device float* x, thread float* terms) {\n  for (int i = 0; i < 8; ++i) terms[i] = x[i];\n  return x[8];\n}\n\n" + first)
    }

    private static let activationPrepackKernel = MLXFast.metalKernel(
        name: "gemma4_gu_activation_prepack9_v1",
        inputNames: ["x"], outputNames: ["packed"],
        source: "\nconst uint packet = thread_position_in_grid.x;\nif (packet >= 2816u) return;\nthread float terms[8];\nconst float sum = load_vector<bfloat, float, 8, 4>(x + packet * 8u, terms);\nfor (uint i = 0; i < 8u; ++i) packed[packet * 9u + i] = terms[i];\npacked[packet * 9u + 8u] = sum;\n",
        header: kernelHeader(tagged: false),
        ensureRowContiguous: true)
    ...
    private static let prepackedKernelGeneral: MLXFast.MLXFastKernel =
        makeKernel(tagged: false, prepacked: true)
    private static let prepackedKernelTagged: MLXFast.MLXFastKernel =
        makeKernel(tagged: true, prepacked: true)
}
