...
public enum Gemma4DecodeFusedGUV1 {
    ...
    /// EXPERT-QUARTET-REUSE: emit the activated expert rows and their
    /// four-value BF16 quartet sums from the same gate/up invocation.
    /// The downstream affine projection can reuse those sums instead of
    /// rebuilding them for every output row.
    static func callWithDownQuartets(_ inputs: [MLXArray]) -> [MLXArray] {
        kernelTaggedDownQuartets(
            inputs,
            grid: (32, 176 * 2, 64),
            threadGroup: (32, 2, 1),
...
static_assert(GU_PAIRS == 1 && (GU_RUN_CAP == 2 || GU_RUN_CAP == 4), "quartet admission");
