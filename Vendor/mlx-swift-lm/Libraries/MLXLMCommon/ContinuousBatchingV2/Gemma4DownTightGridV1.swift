...
METAL_FUNC void gather_qmv_gemma4_down_tile(
  ...
  // `gemma4_tight_down_N` -- program-scope constants 704 and 2816 (:1117-1118).
  // Both callee helpers already carry these as template constants; the walker
  // that drives them still took them as references, so its own
  // `assignment * out_vec_size` address arithmetic stayed a runtime multiply.
  // Keep the launch ABI, expose the bounds to the Metal optimizer.
  constexpr int in_vec_size = K;
