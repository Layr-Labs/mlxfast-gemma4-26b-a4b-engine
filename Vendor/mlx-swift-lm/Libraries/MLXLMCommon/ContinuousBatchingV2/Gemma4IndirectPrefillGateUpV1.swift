...
/// Read normalized token rows through an explicit sorted-row map instead of
/// writing the eightfold expert-sorted activation plane. Same tiled contraction
/// and BF16 GeGLU close as the promoted gathered-RHS kernel.
public enum Gemma4IndirectPrefillGateUpV1 {
    public static let enabled: Bool = {
        ...
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_PREFILL_INDIRECT_A"] else { return true }
    ...
    public static func admits(x: MLXArray, indices: MLXArray) -> Bool {
        enabled && x.ndim == 2 && x.dim(1) == 2816
            && x.dim(0) >= 512 && x.dim(0) % 8 == 0
            && x.dtype == .bfloat16 && indices.dtype == .uint32
            && indices.shape == [x.dim(0), 8]
    }

    public static func call(
        x: MLXArray, weight: MLXArray, scales: MLXArray, biases: MLXArray,
        rowOrder: MLXArray, sortedKeys: MLXArray
    ) -> MLXArray {
        let rows = rowOrder.size
        precondition(x.ndim == 2 && x.dim(1) == 2816 && x.dtype == .bfloat16)
        precondition(rows == x.dim(0) * 8 && rows >= 512 && rows % 64 == 0)
        precondition(rowOrder.dtype == .uint32 && rowOrder.shape == [rows])
        precondition(sortedKeys.dtype == .uint32 && sortedKeys.shape == [rows])
        precondition(weight.dtype == .uint32 && weight.shape == [128, 1408, 352])
        precondition(scales.dtype == .bfloat16 && scales.shape == [128, 1408, 44])
        precondition(biases.dtype == .bfloat16 && biases.shape == scales.shape)
        let output = kernel(
            [x, weight, scales, biases, sortedKeys, rowOrder],
            template: [("T", x.dtype), ("M", rows), ("N", 1408), ("K", 2816)],
            grid: (22 * 32, (rows / 64) * 2, 2),
            threadGroup: (32, 2, 2),
            outputShapes: [[rows, 1, 1408]], outputDTypes: [x.dtype]
        )[0]
        // The incumbent GeGLU epilogue writes the compact physical prefix.
        // Keep its allocation and view boundary, including pointer bounds.
        return output.flattened()[..<(rows * 704)].reshaped(rows, 1, 704)
    }

    private static let kernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "gemma4_prefill_indirect_a_nax_geglu_b64_v1",
...
static_assert(N==1408 && K==2816 && M>=512 && M%64==0,
    "indirect Gemma prefill requires complete production tiles");
  ...
  // Original rows are addressed through row_order.
  ...
  while (n < tgp_bm) {
    ...
    const device T* xn = x;
    const uint first_row=uint(y_row+tm);
          ...
          if (seg_partial && kAlignedM.value) {
            ...
            for (int kk1 = 0; kk1 < BK; kk1 += SK) {
              ...
              for (short mm = 0; mm < TM; mm++) {
                ...
                if (fr < seg_hi && short(fr + Dtile.kFragRows) > seg_lo) {
                  indirect_load_frag_row(mm, Atile, xn + kk1, K, row_order, first_row);
            ...
            for (int kk1 = 0; kk1 < BK; kk1 += SK) {
              ...
              if constexpr (kAlignedM.value) {
                indirect_load_tile(Atile, xn + kk1, K, row_order, first_row);
              } else {
...
// Same lane/fragment coordinates and BF16 elements as BaseNAXFrag::load.
// Only the physical input-row address is gathered through an explicit map.
template <typename U, typename ATile>
METAL_FUNC void indirect_load_frag_row(
    short mm, thread ATile& tile, const device U* src, int ld,
    const device uint* row_order, uint first_row) {
  const short2 coord = ATile::NAXFrag_t::get_coord();
  STEEL_PRAGMA_UNROLL
  for (short kk=0; kk<ATile::kTileCols; ++kk) {
    thread auto& frag=tile.frag_at(mm,kk);
    STEEL_PRAGMA_UNROLL
    for (short i=0;i<ATile::NAXFrag_t::kElemRows;++i) {
      const uint logical_row=first_row + uint(mm*ATile::kFragRows + coord.y
          + i*ATile::NAXFrag_t::kElemRowsJump);
      const size_t row_offset=size_t(row_order[logical_row])*size_t(ld);
      STEEL_PRAGMA_UNROLL
      for (short j=0;j<ATile::NAXFrag_t::kElemCols;++j) {
        frag[i*ATile::NAXFrag_t::kElemCols+j] = U(src[row_offset
            + kk*ATile::kFragCols + coord.x + j]);
      }
    }
  }
}
template <typename U, typename ATile>
METAL_FUNC void indirect_load_tile(
    thread ATile& tile,const device U* src,int ld,
    const device uint* row_order,uint first_row) {
  STEEL_PRAGMA_UNROLL
  for (short mm=0;mm<ATile::kTileRows;++mm)
    indirect_load_frag_row(mm,tile,src,ld,row_order,first_row);
}
