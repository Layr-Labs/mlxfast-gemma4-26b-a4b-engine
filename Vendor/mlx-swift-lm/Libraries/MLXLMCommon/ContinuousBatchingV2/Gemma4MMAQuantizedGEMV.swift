...
//
// M=8 dense quantized projections on the simdgroup matrix units, for the exact
// affine-4/affine-8 group-64 weights the B=8 decode round dispatches.
//
...
// projections are ALU-bound, far under memory bandwidth. The matrix units do
// the same MACs at a fraction of the cost.
//
// EXACT WEIGHTS. The 4-bit / 8-bit codes are staged as bfloat INTEGERS
// (0...15 / 0...255 are exact in bfloat16); the affine transform is applied
// per 64-wide group in float32 afterwards:
//   sum_k x_k * (s_g * q_k + b_g) == s_g * (sum_k x_k q_k) + b_g * (sum_k x_k)
// so no target weight is re-represented or rounded. The group's activation sum
// reproduces the stock kernel's arithmetic exactly (bf16 quad sums for 4-bit,
// widened sums for 8-bit), so the bias term carries stock's rounding. The only
// difference from the scalar kernel is float32 summation order in the dot
// product (MMA tree within a group, fixed-order K-split combine), the same
// class of last-bit difference the promoted batch-wide attention introduced.
//
// Fail-closed gating in the host wrappers; `DARKBLOOM_GEMMA4_MMA_QMV=0`
// disables every route for same-binary controls.
...
public enum Gemma4MMAQuantizedGEMV {
    ...
    static let columnsPerThreadgroup = 16
    ...
    // The single-output kernel (v3 body, KSPLIT templated). One threadgroup per
    // 16 output columns; KSPLIT simdgroups each own a contiguous quarter/third
    // of K; partials combine in a fixed order.
    private static let kernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "gemma4_mma_affine_qmv_m8_v4",
        ...
        source: """
            ...
            constexpr int GPS = KG / KSPLIT;   // groups per simdgroup (KG % KSPLIT == 0, host-checked)
    ...
    // The fused-projection kernel: identical math, three output buffers.
    // Global column n maps to (y0, n) for n < NQ, (y1, n - NQ) for n < NQ + NK,
    // else (y2, n - NQ - NK). A threadgroup's 16 columns never straddle a
    // boundary because NQ and NK are multiples of 16.
    private static let fusedKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "gemma4_mma_affine_qmv_m8_fused3_v4",
        inputNames: ["x", "w", "scales", "biases"],
        outputNames: ["y0", "y1", "y2"],
        source: """
            ...
            if (sgi == 0) {
                ...
                if (n0 < NQ) {
                    ...
                    yr = y1 + fm * NK; nb = n0 - NQ;
                } else {
                    yr = y2 + fm * NV; nb = n0 - NQ - NK;
                }
                yr[nb + fn] = T(o0);
                yr[nb + fn + 1] = T(o1);
                yr[nb + 8 + fn] = T(o2);
    ...
    static func ksplit(forGroups kg: Int) -> Int? {
        if kg % 4 == 0 { return 4 }
        if kg % 3 == 0 { return 3 }
        return nil
    }

    /// Single-tensor form. x: 8 rows x K bf16; w: [N, K*bits/32] uint32;
    ...
    public static func apply(
        ...
        let K = x.dim(-1)
        guard x.size == rows * K, K % 64 == 0, let split = ksplit(forGroups: K / 64) else { return nil }
        let N = weight.dim(0)
        guard N % columnsPerThreadgroup == 0,
        ...
        return kernel(
            ...
            template: [("T", x.dtype), ("BITS", bits), ("K", K), ("N", N), ("KSPLIT", split)],
            grid: (N / columnsPerThreadgroup * 32 * split, 1, 1),
    ...
    /// Fused two- or three-projection form over one shared input. All segments
    /// share K, bits 4 or 8, group 64. `nv == 0` leaves the third output unwritten.
    public static func applyFused(
        x: MLXArray, weight: MLXArray, scales: MLXArray, biases: MLXArray,
        bits: Int, nq: Int, nk: Int, nv: Int
    ) -> (MLXArray, MLXArray, MLXArray)? {
        guard enabled, bits == 4 || bits == 8,
            x.dtype == .bfloat16, scales.dtype == .bfloat16, biases.dtype == .bfloat16,
            weight.dtype == .uint32, weight.ndim == 2,
            nq % columnsPerThreadgroup == 0, nk % columnsPerThreadgroup == 0,
            nv % columnsPerThreadgroup == 0 || nv == 0
        else { return nil }
        let K = x.dim(-1)
        let N = nq + nk + nv
        guard x.size == rows * K, K % 64 == 0, let split = ksplit(forGroups: K / 64),
            weight.shape == [N, K * bits / 32],
            scales.shape == [N, K / 64], biases.shape == [N, K / 64]
        else { return nil }
        let x2 = x.ndim == 2 ? x : x.reshaped([rows, K])
        let out = fusedKernel(
            [x2, weight, scales, biases],
            template: [
                ("T", x.dtype), ("BITS", bits), ("K", K), ("NQ", nq), ("NK", nk), ("NV", nv),
                ("KSPLIT", split),
            ],
            grid: (N / columnsPerThreadgroup * 32 * split, 1, 1),
            threadGroup: (32 * split, 1, 1),
            outputShapes: [[rows, nq], [rows, nk], [rows, max(nv, 1)]],
...
/// Lazily concatenated packed weights for a fused projection group. The packed
/// codes, scales and biases are CONCATENATED AS STORED (row stacking; no value
/// changes) — an input-independent derived tensor, the same legality class as a
/// dequantized-weight cache. Built once on first eligible decode call.
public final class Gemma4MMAFusedWeights {
    public init() {}
    private(set) var weight: MLXArray?
    private(set) var scales: MLXArray?
    private(set) var biases: MLXArray?
    private(set) var bits = 0
    private(set) var segments: [Int] = []
    private var attempted = false

    /// Returns the concat for the given QuantizedLinear list when every member
    /// is affine group-64 with a common bits width and bias-free; nil otherwise.
    public func resolve(_ projections: [QuantizedLinear]) -> (MLXArray, MLXArray, MLXArray, Int, [Int])? {
        if let w = weight, let s = scales, let b = biases {
            return (w, s, b, bits, segments)
        }
        guard !attempted else { return nil }
        attempted = true
        guard let first = projections.first else { return nil }
        let commonBits = first.bits
        for p in projections {
            guard p.groupSize == 64, p.bits == commonBits, p.bias == nil,
                p.scales.dtype == .bfloat16, p.weight.dtype == .uint32
            else { return nil }
            guard p.biases != nil else { return nil }
        }
        let w = concatenated(projections.map(\.weight), axis: 0)
        let s = concatenated(projections.map(\.scales), axis: 0)
        let b = concatenated(projections.map { $0.biases! }, axis: 0)
        eval(w, s, b)
        weight = w; scales = s; biases = b
        bits = commonBits
        segments = projections.map { $0.weight.dim(0) }
        return (w, s, b, commonBits, segments)
    }
}
