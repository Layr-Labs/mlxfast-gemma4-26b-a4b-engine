...
public enum Gemma4MMAQuantizedGEMV {
    ...
    /// Graph-free capability gate for `applyArgmax`. Callers use this before
    /// constructing a cache-mutating model forward, so a disabled or
    /// unsupported head can fall back without building the trunk twice.
    public static func supportsArgmax(
        w: MLXArray,
        scales: MLXArray,
        biases: MLXArray?,
        groupSize: Int,
        ...
        guard enabled, version == 26, let biases else { return false }
        guard groupSize == 64, bits == 4, k > 0 else { return false }
        ...
        guard n >= minOutputWidth, n % cols == 0 else { return false }
        ...
        return scales.dim(1) == k / groupSize && biases.dim(1) == k / groupSize
    }
    ...
    public static func applyArgmax(
        ...
        guard
            supportsArgmax(
                ...
                groupSize: groupSize, bits: bits, k: k)
        else { return nil }
