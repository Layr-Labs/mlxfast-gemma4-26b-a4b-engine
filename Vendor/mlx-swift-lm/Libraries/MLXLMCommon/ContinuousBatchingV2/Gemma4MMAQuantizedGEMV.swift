...
public enum Gemma4MMAQuantizedGEMV {
    ...
    private static let sourceV27Carry: String = {
        ...
        replaceOnce(
            ...
            uint4 carryLo0 = uint4(0);
            uint4 carryHi0 = uint4(0);
            uint4 carryLo1 = uint4(0);
            uint4 carryHi1 = uint4(0);
            uint4 carryLo2 = uint4(0);
            uint4 carryHi2 = uint4(0);
            uint4 carryLo3 = uint4(0);
            uint4 carryHi3 = uint4(0);
            T carryS0 = T(0);
            T carryS1 = T(0);
            T carryS2 = T(0);
            T carryS3 = T(0);
            // Four lanes build different column pairs from the same packed
            // weight row. One owner loads each row; its peers receive the
            // identical bits through the SIMD crossbar.
            const ushort carryOwner = ushort(
                (fragmentRow & 3u) * 2u + (fragmentRow & 4u) * 4u);
            if (lane == carryOwner) {
            ...
            carryLo0 = simd_shuffle(carryLo0, carryOwner);
            carryHi0 = simd_shuffle(carryHi0, carryOwner);
            carryLo1 = simd_shuffle(carryLo1, carryOwner);
            carryHi1 = simd_shuffle(carryHi1, carryOwner);
            carryLo2 = simd_shuffle(carryLo2, carryOwner);
            carryHi2 = simd_shuffle(carryHi2, carryOwner);
            carryLo3 = simd_shuffle(carryLo3, carryOwner);
            carryHi3 = simd_shuffle(carryHi3, carryOwner);
            carryS0 = simd_shuffle(carryS0, carryOwner);
            carryS1 = simd_shuffle(carryS1, carryOwner);
            carryS2 = simd_shuffle(carryS2, carryOwner);
            carryS3 = simd_shuffle(carryS3, carryOwner);
            """
        )
