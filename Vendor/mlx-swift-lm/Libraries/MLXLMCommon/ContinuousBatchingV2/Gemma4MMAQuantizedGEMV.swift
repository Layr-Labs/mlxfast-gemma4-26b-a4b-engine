...
public enum Gemma4MMAQuantizedGEMV {
    ...
    /// DIRECT-GREEDY-TOP1 switch — DEFAULT OFF. An 8-leg repeated-run cohort
    /// battery is deterministic with this fold disabled and probabilistically
    /// divergent (~1-in-6, one stable alternate token plane) with it enabled:
    /// a carrier-slot coverage / designated-lane hazard in the two-stage
    /// reduction is under investigation. The ranked runner sets no
    /// environment, so OFF is what ships; set
    /// `DARKBLOOM_CBV2_DIRECT_GREEDY_TOP1=1` to re-arm for local forensics.
    ...
    private static let kernelV26Top1: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "gemma4_mma_affine4_qmv_m8_v27_top1_unroll",
        inputNames: ["x", "w", "scales", "biases", "xSums"],
