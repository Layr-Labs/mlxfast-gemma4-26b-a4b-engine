...
public enum Gemma4MMAQuantizedGEMV {
    ...
    /// Reduce the compact per-32-column winners with an explicit threadgroup
    /// tree. The former two-level shuffle reduction changed a row-0 winner in
    /// the P2 composition even though a serial scan of the same partials chose
    /// the incumbent token. This keeps the same lower-token tie break while
    /// making every reduction edge explicit in threadgroup memory.
    private static let sourceTop1PartialReduce: String = """
        ...
        for (uint idx = lid; idx < PARTIALS_PER_ROW; idx += THREADS_PER_ROW) {
            const uint2 pair = rowPartials[idx];
            const float v = as_type<float>(pair.x);
            const uint id = pair.y;
        ...
        threadgroup float tg_best_val[THREADS_PER_ROW];
        threadgroup uint tg_best_id[THREADS_PER_ROW];
        tg_best_val[lid] = best_val;
        tg_best_id[lid] = best_id;
        threadgroup_barrier(mem_flags::mem_threadgroup);

        for (uint stride = THREADS_PER_ROW / 2; stride > 0; stride >>= 1) {
            if (lid < stride) {
                const float other_val = tg_best_val[lid + stride];
                const uint other_id = tg_best_id[lid + stride];
                if (other_val > tg_best_val[lid]
                    || (other_val == tg_best_val[lid] && other_id < tg_best_id[lid]))
                {
                    tg_best_val[lid] = other_val;
                    tg_best_id[lid] = other_id;
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
        ...
        if (lid == 0) {
            outTokens[b] = int32_t(tg_best_id[0]);
        }
    """
