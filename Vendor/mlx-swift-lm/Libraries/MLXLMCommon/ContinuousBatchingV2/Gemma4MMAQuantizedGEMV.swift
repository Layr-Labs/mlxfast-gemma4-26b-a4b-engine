...
public enum Gemma4MMAQuantizedGEMV {
    ...
    private static let xSumKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "gemma4_mma_affine4_xsum_m8_v27_unroll_u4_v2",
        inputNames: ["x"],
        outputNames: ["xSums"],
        source: """
            constexpr uint M_ROWS = 8;
            ...
            for (uint c = 0; c < GROUP / 8; ++c) {
                const uint i = c * 8;
                const uint4 packed =
                    *((const device uint4*)(xp + i));
                thread T xv[8];
                xv[0] = gemma4_xsum_u16<T>::cast(ushort(packed.x));
                xv[1] = gemma4_xsum_u16<T>::cast(ushort(packed.x >> 16));
                xv[2] = gemma4_xsum_u16<T>::cast(ushort(packed.y));
                xv[3] = gemma4_xsum_u16<T>::cast(ushort(packed.y >> 16));
                xv[4] = gemma4_xsum_u16<T>::cast(ushort(packed.z));
                xv[5] = gemma4_xsum_u16<T>::cast(ushort(packed.z >> 16));
                xv[6] = gemma4_xsum_u16<T>::cast(ushort(packed.w));
                xv[7] = gemma4_xsum_u16<T>::cast(ushort(packed.w >> 16));
                s += xv[0] + xv[1] + xv[2] + xv[3];
                s += xv[4] + xv[5] + xv[6] + xv[7];
            }
            xSums[cell] = s;
            """,
        header: """
            template <typename T, bool TWO_BYTE = (sizeof(T) == 2)>
            struct gemma4_xsum_u16 {
                static inline T cast(ushort u) { return T(0); }
            };
            template <typename T>
            struct gemma4_xsum_u16<T, true> {
                static inline T cast(ushort u) { return as_type<T>(u); }
            };
            """,
        ensureRowContiguous: true
    )
