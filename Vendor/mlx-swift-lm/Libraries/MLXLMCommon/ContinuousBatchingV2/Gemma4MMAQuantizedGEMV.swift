...
public enum Gemma4MMAQuantizedGEMV {
    ...
    /// Version 16 with the vocabulary store replaced by an in-register top-1
    ...
    private static let sourceV16Argmax: String = {
        var result = sourceV16

        func replaceOnce(_ old: String, with new: String) {
            let count = result.components(separatedBy: old).count
            precondition(count == 2, "sourceV16Argmax replacement count \(count): \(old)")
            result = result.replacingOccurrences(of: old, with: new)
        }
        ...
        precondition(!result.contains("out["), "sourceV16Argmax still stores logits")
        return result
    }()

    private static let kernelV16Argmax: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "gemma4_mma_affine4_qmv_m8_v16_argmax",
        inputNames: ["x", "w", "scales", "biases", "xSums"],
        outputNames: ["pv", "pi"],
        source: sourceV16Argmax,
        header: "#include <metal_simdgroup_matrix>\n",
        ensureRowContiguous: true
    )
    ...
    public static func admitsArgmax(
        ...
        guard logitslessEnabled, enabled, version == 16 else { return false }
