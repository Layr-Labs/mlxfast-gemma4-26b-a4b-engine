...
public enum Gemma4MMAQuantizedGEMV {
    ...
    /// Exact affine-4/group-64 sums produced while an upstream kernel writes
    /// a B8/L1/K2816 activation.  The producer and consumer must share the
    /// same `MLXArray` object; shape equality alone is insufficient.
    public struct ActivationSums {
        fileprivate let source: MLXArray
        fileprivate let values: MLXArray
    }

    public static var acceptsAttentionProducedActivationSums: Bool {
        attentionProjectionTrainEnabled
    }

    /// Seal an upstream-produced table after checking the exact live plane.
    public static func producedActivationSums(
        values: MLXArray, for source: MLXArray
    ) -> ActivationSums? {
        guard source.dtype == .bfloat16,
            source.shape == [mRows, 1, 2816],
            source.size == mRows * 2816,
    ...
    public static func applyAttentionProjectionTrain(
            ...
            activationSums.values.size == sumCells
        {
            ...
            CBv2EngageMark.once("gemma4-attention-producer-xsum")
        } else {
            xSums = xSumKernel(
                [flatX],
    ...
    private static let attentionProjectionTrainEnabled: Bool = {
        ...
        // Unlike the promoted tied-head path, this Q/K/V train changes the
        // projection accumulation tree. Default to the exact tight-QMV
        // fallback and require an explicit research opt-in.
        else { return false }
        switch raw.trimmingCharacters(in: .whitespaces).lowercased() {
        case "0", "false", "no", "off": return false
        default: return true
