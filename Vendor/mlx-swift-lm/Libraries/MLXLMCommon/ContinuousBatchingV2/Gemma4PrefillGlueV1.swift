...
public enum Gemma4PrefillGlueV1 {
    ...
    ///
    /// Default OFF. The barrier elision is bit-exact but measured -0.27% on the
    /// prompt leg when it shipped enabled, so the zero-fill reduction stays the
    /// shipped arm and this is opt-in only.
    /// Opt-in: `DARKBLOOM_GEMMA4_PREFILL_GLUE_BARRIER=1` selects the masked
    /// combine.
