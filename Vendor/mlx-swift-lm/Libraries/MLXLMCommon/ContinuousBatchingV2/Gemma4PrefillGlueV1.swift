...
public enum Gemma4PrefillGlueV1 {
    ...
    ///
    /// GLUE-VEC4-BD17. Each thread owns `GLUE_NREADS = 8` consecutive features
    /// of one row, and `base` is a multiple of eight on a 2816-wide row, so the
    /// three per-thread device spans (`h1`, `res2`, and the `out`/`normed`
    /// stores) are two naturally aligned four-element vectors. Reading and
    /// writing them as vectors changes the memory transaction width only; the
    /// element order into `av`/`ov` and out of `tv` is unchanged, every
    /// `static_cast` and multiply keeps its operands and its position, and the
    /// two reduction helpers see the same arrays in the same order.
    ///
    /// The eight routing slots' `inverse_order` and `route_weights` words are
    /// loop-invariant across the feature loop — the incumbent re-read them
    /// eight times per thread. Hoisting them into registers ahead of the loop
    /// reads the identical words and feeds the accumulator in the identical
    /// slot order, so the bf16 accumulation chain is unchanged.
