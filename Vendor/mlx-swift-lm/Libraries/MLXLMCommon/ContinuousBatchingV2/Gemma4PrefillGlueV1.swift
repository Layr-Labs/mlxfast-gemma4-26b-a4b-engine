...
public enum Gemma4PrefillGlueV1 {
    ...
    private static let expertTailChainKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "gemma4_prefill_expert_unsort_tail_chain_2816_vec4_v7",
        inputNames: [
        ...
        source: """
            ...
            const uint w_base = lid * GLUE_NREADS;
            const vec<T, 4> w1_v0 = *((const device vec<T, 4>*)(w1 + w_base));
            const vec<T, 4> w1_v1 = *((const device vec<T, 4>*)(w1 + w_base + 4));
            const vec<T, 4> w2_v0 = *((const device vec<T, 4>*)(w2 + w_base));
            const vec<T, 4> w2_v1 = *((const device vec<T, 4>*)(w2 + w_base + 4));

            float tv[GLUE_NREADS];
            #pragma clang loop unroll(full)
            for (int i = 0; i < 4; i++) {
                const T n1_0 = static_cast<T>(w1_v0[i] * static_cast<T>(av[i] * inv_a));
                const T n2_0 = static_cast<T>(w2_v0[i] * static_cast<T>(bv[i] * inv_b));
                tv[i] = static_cast<float>(static_cast<T>(n1_0 + n2_0));

                const T n1_1 = static_cast<T>(w1_v1[i] * static_cast<T>(av[i + 4] * inv_a));
                const T n2_1 = static_cast<T>(w2_v1[i] * static_cast<T>(bv[i + 4] * inv_b));
                tv[i + 4] = static_cast<float>(static_cast<T>(n1_1 + n2_1));
            }
            ...
            const vec<T, 4> w3_v0 = *((const device vec<T, 4>*)(w3 + w_base));
            const vec<T, 4> w3_v1 = *((const device vec<T, 4>*)(w3 + w_base + 4));
            float ov[GLUE_NREADS];
            vec<T, 4> out_v0;
            vec<T, 4> out_v1;
            #pragma clang loop unroll(full)
            for (int i = 0; i < 4; i++) {
                const T normed3_0 = static_cast<T>(w3_v0[i] * static_cast<T>(tv[i] * inv_t));
                const T normed3_1 = static_cast<T>(w3_v1[i] * static_cast<T>(tv[i + 4] * inv_t));
            ...
            *((device vec<T, 4>*)(out + base + 4)) = out_v1;
            ...
            const vec<T, 4> wn_v0 = *((const device vec<T, 4>*)(wn + w_base));
            const vec<T, 4> wn_v1 = *((const device vec<T, 4>*)(wn + w_base + 4));
            vec<T, 4> normed_v0;
            vec<T, 4> normed_v1;
            #pragma clang loop unroll(full)
            for (int i = 0; i < 4; i++) {
                normed_v0[i] = wn_v0[i] * static_cast<T>(ov[i] * inv_n);
                normed_v1[i] = wn_v1[i] * static_cast<T>(ov[i + 4] * inv_n);
            }
