...
//   * router products and every reduction node remain BF16.  The eight slots
//     use a balanced three-level tree instead of the legacy seven-dependency
//     left fold; this deliberately changes only the BF16 addition grouping.
//
...
private let gemma4RoutedDownReduceKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
    ...
    source: """
        ...
        // One simdgroup owns one original token.  Eight consecutive lanes
        // own the eight top-K slots for one feature, so all router products
        // are issued together and reduced as a balanced BF16 tree.  Four
        // features fit in one SIMDgroup; two passes cover this eight-feature
        // output tile.  Every addition still rounds to T, but the dependency
        // depth drops from seven serial adds to three shuffle/add levels.
        if (simd_gid < TOKENS) {
            const uint slot = lane & 7;
            const uint feature_in_quad = lane >> 3;
            const uint assignment_base = simd_gid * TOPK;
            const uint assignment = assignment_base + slot;
            const uint sorted_row = uint(inverse_order[assignment]);

            for (uint feature_quad = 0; feature_quad < 2; ++feature_quad) {
                const uint feature = feature_quad * 4 + feature_in_quad;
                T partial = static_cast<T>(
                    static_cast<float>(
                        projected[sorted_row * outputs_per_tile + feature]) *
                    static_cast<float>(router_weights[assignment]));

                const T adjacent = simd_shuffle_down(partial, ushort(1));
                if ((slot & 1) == 0) {
                    partial = partial + adjacent;
                }
                const T pair = simd_shuffle_down(partial, ushort(2));
                if ((slot & 3) == 0) {
                    partial = partial + pair;
                }
                const T upper_half = simd_shuffle_down(partial, ushort(4));
                if (slot == 0) {
                    partial = partial + upper_half;
                    output[simd_gid * OUTN + output_base + feature] = partial;
                }
            }
        }
        """,
