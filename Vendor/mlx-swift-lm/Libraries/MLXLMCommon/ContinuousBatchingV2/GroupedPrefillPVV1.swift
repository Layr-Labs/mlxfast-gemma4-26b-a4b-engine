...
enum CBv2GroupedPrefillPVV1 {
    ...
    private static let geometry: Geometry? = {
        ...
        if suffix == "s" || suffix == "c" || suffix == "d" {
            // M=128 has two BM64 tiles. A four-way swizzle launches two
            // additional M tiles that return before the GEMM; omit only those.
            return Geometry(bm: 64, bk: 256, wm: 2,
                swizzle: tightSwizzleEnabled ? 1 : 2)
        }
