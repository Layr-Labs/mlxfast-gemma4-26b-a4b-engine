...
enum CBv2GroupedPrefillPVV1 {
    ...
    // One diagnostic per process, only after every optimization guard passes.
    // No prompts, tokens, paths, or tensor contents are logged.
    private static let engagementRecorded: Void = {
        let architecture = GPU.deviceInfo().architecture
        let message = "mlxfast: grouped-pv-final engaged arch=\(architecture)\n"
