...
public final class CBv2LayerCache: CBv2AttendingLayerCache {
    ...
    /// define correctness: the tensor must be derived from a PRE-step
    /// capture of the shared position chain as
    /// `(offsets + L - window) % window`, which holds whenever the ring
    /// branch engages (retainedCount == window per row). The offsets
    /// advance and the output are identical to the 5-parameter path.
    public func updateAndAttend(
        queries: MLXArray, keys: MLXArray, values: MLXArray,
