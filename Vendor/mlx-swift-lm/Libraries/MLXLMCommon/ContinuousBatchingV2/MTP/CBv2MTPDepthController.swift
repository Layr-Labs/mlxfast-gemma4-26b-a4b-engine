...
final class CBv2MTPDepthController {
    ...
    ///
    /// This tuning candidate enables the existing adaptive controller so the
    /// width-1 benchmark can measure the envelope-requested depth ceilings.
    /// The sealed verification mode remains `.serialTarget`; only the policy
    /// lever changes here.
