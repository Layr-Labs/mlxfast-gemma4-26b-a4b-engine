...
final class CBv2MTPDepthController {
    ...
    ///
    /// When false, every plan is target-only regardless of
    /// `CBv2MTPRoundDriver.submissionDraftDepth`. Keep true so the adaptive
    /// ceiling (`effectiveDraftCeiling`) actually applies.
