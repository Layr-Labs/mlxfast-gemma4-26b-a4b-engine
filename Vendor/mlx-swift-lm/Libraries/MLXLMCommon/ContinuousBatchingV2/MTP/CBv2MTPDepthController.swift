...
final class CBv2MTPDepthController {
    ...
    ///
    /// Phase A admits one draft only when the round driver retained the exact
    /// construction-installed `(B=8,K=1)` verifier geometry. Other installed
    /// shapes are clamped before any seed or verify work is planned.
