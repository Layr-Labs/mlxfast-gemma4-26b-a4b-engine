...
final class CBv2MTPDepthController {
    ...
    /// This submission enables MTP and pins the tested track depth below.
    ...
    init(maxDepth: Int, fixedDepth: Int?) {
        ...
        // The adaptive cost policy disabled MTP on the ranked M5 run.
        self.fixedDepth = fixedDepth.map { min(max($0, 0), resolvedMax) } ?? resolvedMax
    }
