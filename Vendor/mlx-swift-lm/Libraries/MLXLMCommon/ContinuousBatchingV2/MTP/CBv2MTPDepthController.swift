...
final class CBv2MTPDepthController {
    ...
    ///
    /// This submission enables speculation. When `fixedDraftTokens` is absent,
    /// the round driver passes the effective draft ceiling as `fixedDepth`, so
    /// each eligible round uses that fixed positive depth. The controller does
    /// not adapt depth per round on this arm.
