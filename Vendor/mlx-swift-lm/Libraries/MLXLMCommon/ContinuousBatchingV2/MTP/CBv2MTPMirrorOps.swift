...
/// Public view of the speculation policy for modules outside MLXLMCommon
/// (the MLXLLM drafter adapter warms the exact round shape at head load).
public enum CBv2MTPSpeculationPolicy {
    public static var speculationEnabled: Bool { CBv2MTPDepthController.speculationEnabled }
    /// The fixed depth a scored round runs at under this track's envelope
    /// (`maxDraftTokens` 3).
...
public enum CBv2MTPWideVerifyContext {
    ...
    /// Chained rounds: the `[8]` int32 device position of column 0 per row.
    /// Attention hosts derive ring slots and key lengths from it instead of
    /// the (lagging) host counters. nil = host counters are exact.
    ...
    /// Row tiles a pinned eight-row kernel host may widen to: 1 for the
    /// decode cell itself, 2...4 for a wide verify rectangle of 16/24/32
    /// rows, 0 when the row count is not admissible at all.
...
struct CBv2MTPMirrorRestoreLayer {
    ...
    /// `[8]` int32 slot of column 0 per row (host or device built).
...
enum CBv2MTPMirrorOps {
    ...
    /// `start` is a `[1]` int32 slot index, host or device built.
    static func dequantize(
    ...
    /// `slotBases` is `[8]` int32 (slot of column 0 per row), host or device built.
    static func capture(
        mirrors: [MLXArray], slotBases: MLXArray, depth k: Int,
        kvHeads: Int, headDim: Int, window: Int, fence: MLXArray
    ) -> (undo: MLXArray, fence: MLXArray) {
        precondition(mirrors.count == 8 && slotBases.shape == [8] && k > 0)
        let params = concatenated(
            [slotBases.asType(.uint32), MLXArray([UInt32(k)])], axis: 0)
    ...
    static func restore(
        ...
        precondition(mirrors.count == 8 && slotBases.shape == [8])
        precondition(firstRestored.shape == [8] && firstRestored.dtype == .int32)
        precondition(k > 0)
        // Params live on device: the first restored column per row is the
        // accept walk's result, computed in the same graph.
        let params = concatenated(
            [
...
/// The drafter's attention over the target's q4 sliding mirror: the bytes the
/// target attends, read in place instead of dequantized, stacked and masked.
public enum CBv2MTPMirrorAttention {
    /// `queries` `[8, 16, 1, 256]` bf16; `slotBases` the `[8]` uint32 boundary
    /// slot per row (anchor mod window), which is skipped like the sliding
    /// mask skips it; `fence` is consumed for ordering only. nil off the
    /// eight-row D256 ring geometry.
