...
extension EngineLoopV2 {
    ...
    /// The in-flight round can seed the next one when it handed off device
    /// state, its cohort is exactly the running set (same order, nobody
    /// cancelled, paused or discarded), and the controller still speculates.
    func mtpChainedRoundCandidate(_ previous: CBv2InFlightStep) -> (CBv2MTPRoundInFlight.Verify, [CBv2RequestID])? {
        ...
        for (rec, id) in zip(running, ids) {
            ...
            // Enough budget for a whole rectangle (finalize clamps at the end).
    ...
    /// Build + submit round n+1 from round n's device hand-off. Returns nil
    /// (state untouched) when the scheduler refuses the reservation.
