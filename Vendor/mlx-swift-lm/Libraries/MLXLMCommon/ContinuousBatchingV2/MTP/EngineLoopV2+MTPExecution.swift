...
extension EngineLoopV2 {
    /// Kill switch for MTP-DRAFT-EARLY-SUBMIT (default on).
    static let mtpDraftEarlySubmitEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment["DARKBLOOM_GEMMA4_MTP_DRAFT_EARLY_SUBMIT"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()
    ...
    private func mtpBuildVerifyGraph(
        ...
        for row in verifyRows {
            ...
            if Self.drafterRingAttnEnabled,
                ...
                let ring = ringRow.drafterRingOrMirrorView
            {
        ...
        // MTP-DRAFT-EARLY-SUBMIT: hand the finished draft graph to the device now,
        // so it runs while the host builds the verify trunk up to the ladder's
        // first submit (layers 0-1) instead of riding along with that submit.
        if Self.mtpDraftEarlySubmitEnabled, !mtp1Timing {
            CBv2EngageMark.once("mtp-draft-early-submit")
            asyncEval(draftIDs)
        }
