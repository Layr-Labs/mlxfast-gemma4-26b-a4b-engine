...
/// Model-family hook for a target that can construct sealed serial semantics
/// as one lazy graph. MLXLLM registers the concrete Gemma target when its MTP
/// drafter binds, avoiding a wider ordinary-model protocol for this one path.
...
/// Default ON; zero restores the original blocking serial-column oracle.
...
extension EngineLoopV2 {
    ...
    /// retains the blocking column oracle below.
        ...
        if !useRectangular {
                ...
                let targetID = mtp.model.mtpTargetIdentity
            {
                exactOutput = CBv2MTPBatchedExactTargetRegistry.forward(
                    targetID: targetID, columns: columns, caches: caches)
            } else {
                exactOutput = nil
            }
