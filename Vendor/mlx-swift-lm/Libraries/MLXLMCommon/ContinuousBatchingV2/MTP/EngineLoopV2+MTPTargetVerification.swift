...
extension EngineLoopV2 {
    ...
    /// `deviceBase`: chained rounds' `[B]` int32 device position of column 0
    /// per row; the attention hosts derive ring slots and key lengths from it
    /// while the host counters lag.
    func mtpBuildTargetVerification(
