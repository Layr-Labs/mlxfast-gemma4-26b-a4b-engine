...
private let decodeMetaMemoEnabled: Bool = {
...
private let decodeMetaMemoLock = NSLock()
nonisolated(unsafe) private var ringStartsKey: [UInt32]?
nonisolated(unsafe) private var ringStartsValue: MLXArray?

fileprivate func memoizedRingStarts(_ values: [UInt32], _ shape: [Int]) -> MLXArray {
    guard decodeMetaMemoEnabled else { return MLXArray(values, shape) }
    decodeMetaMemoLock.lock()
    defer { decodeMetaMemoLock.unlock() }
    if let cached = ringStartsValue, ringStartsKey == values {
        return cached
    }
    let made = MLXArray(values, shape)
    eval(made)
    ringStartsKey = values
    ringStartsValue = made
    CBv2EngageMark.once("decode-meta-memo")
    return made
}

nonisolated(unsafe) private var paramsKey: [UInt32]?
nonisolated(unsafe) private var paramsValue: MLXArray?

fileprivate func memoizedParams(_ values: [UInt32]) -> MLXArray {
    guard decodeMetaMemoEnabled else { return MLXArray(values) }
    decodeMetaMemoLock.lock()
    defer { decodeMetaMemoLock.unlock() }
    if let cached = paramsValue, paramsKey == values {
        return cached
    }
...
public enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    private static let portQuantFusedWriteResidentKernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_ey29_ey32_yp3_ey51",
            inputNames: [
                "queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
                "starts", "new_keys", "new_values", "write_fence",
    ...
    private static let portQuantFusedWriteResidentNormRopeKernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_f4_normrope_v1_ey29_ey32_yp3_ey51",
            inputNames: [
                "raw_queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
    ...
    private static let portQuantFusedWriteResidentNormRopeORunsumKernel:
        MLXFast.MLXFastKernel = MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_f4_normrope_ors_v1_ey29_ey32_yp3_ey51",
            inputNames: [
                "raw_queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
                "starts", "raw_keys", "raw_values", "q_weight", "k_weight",
...
public enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    static func updateAndAttendWriting22(
        ...
        let paramsArray = memoizedParams(params)
