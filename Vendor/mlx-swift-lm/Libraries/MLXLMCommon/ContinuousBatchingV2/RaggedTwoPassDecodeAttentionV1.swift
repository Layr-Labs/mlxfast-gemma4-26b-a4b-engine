...
public enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    private static let ringPassAQuantKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_sdpa_ring_2pass_a_q8_d256_g2_b\(blocks)_v5w",
        inputNames: [
            "queries",
    ...
    private static let fusedRingPassAQuantKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_ringwrite_sdpa_2pass_a_q8_d256_g2_b\(blocks)_v6w",
        inputNames: [
            "queries",
    ...
    /// KVQ-WARM: compile the three novel quant pipelines at constructor
    /// time, before any measured window. The ranked box spawns a FRESH
    /// worker per gated phase, so a pipeline this tree introduces has no
    /// system shader-cache entry on its first run and would otherwise
    /// first-compile inside a parent-clocked (and step-watchdogged) window.
    /// Dummy zero inputs at the exact production template/geometry; outputs
    /// discarded. Prompt-independent, output-inert, untimed.
    public static func warmQuantPipelines() {
        guard CBv2WindowedSequenceKV.quantEnabled, enabled,
            blocks > 0, blocks.isMultiple(of: 32)
        else { return }
        let mirrors = (0 ..< batch).map { _ in
            MLXArray.zeros(
                [2, kvHeads, sequenceLength, (headDim + 4) / 4], dtype: .uint32)
        }
        let queries = MLXArray.zeros(
            [batch, queryHeads, 1, headDim], dtype: .bfloat16)
        let startArray = MLXArray(
            Array(repeating: UInt32(0), count: batch), [batch])
        ...
        let newKeys = MLXArray.zeros(
            [batch, kvHeads, 1, headDim], dtype: .bfloat16)
        let fenceIn = MLXArray.zeros([1], dtype: .int32)
        let fused = fusedRingPassAQuantKernel(
            [queries] + mirrors + [startArray, newKeys, newKeys, fenceIn],
            template: template,
            grid: (kvHeads * 32, batch * gqa, blocks),
            threadGroup: (32, gqa, 1),
        ...
        let rings = (0 ..< batch).map { _ in
            MLXArray.zeros(
                [1, kvHeads, sequenceLength, headDim], dtype: .bfloat16)
        }
        let store = ringBF16WriteKernel(
            rings + rings + [startArray, newKeys, newKeys, fused[3]],
            template: [
                ("T", queries.dtype),
                ("D", headDim),
                ("N", sequenceLength),
        ...
        eval(fused[0], store[0])
        CBv2EngageMark.once("kvq8warm")
    }
