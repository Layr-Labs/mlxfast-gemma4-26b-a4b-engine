...
public enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    // DECODE-META-MEMO-001 (kill switch DARKBLOOM_CBV2_DECODE_META_MEMO=0).
    // Within a single decode step, all 25 sliding layers construct an identical
    // `[8]` UInt32 ring-start array, and all 5 full-attention layers construct
    // an identical `[10]` UInt32 params array. Memoizing each per-step reduces
    // 30 host allocations and GPU uploads per decode step to 2.
    private static let decodeMetaMemoEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
    ...
    private static let metaMemoLock = NSLock()
    nonisolated(unsafe) private static var memoRingKey: [UInt32]? = nil
    nonisolated(unsafe) private static var memoRingValue: MLXArray? = nil
    nonisolated(unsafe) private static var memoParamsKey: [UInt32]? = nil
    nonisolated(unsafe) private static var memoParamsValue: MLXArray? = nil

    fileprivate static func memoizedRingStarts(_ values: [UInt32], _ shape: [Int]) -> MLXArray {
        guard decodeMetaMemoEnabled else { return MLXArray(values, shape) }
        metaMemoLock.lock()
        if let cached = memoRingValue, memoRingKey == values {
            metaMemoLock.unlock()
            return cached
        }
    ...
    fileprivate static func memoizedParams(_ values: [UInt32]) -> MLXArray {
        guard decodeMetaMemoEnabled else { return MLXArray(values) }
        metaMemoLock.lock()
        if let cached = memoParamsValue, memoParamsKey == values {
            metaMemoLock.unlock()
            return cached
        }
        let made = MLXArray(values)
        memoParamsKey = values
        memoParamsValue = made
        metaMemoLock.unlock()
        CBv2EngageMark.once("decode-meta-memo")
        return made
    }
    ...
    private static let portQuantFusedWriteResidentKernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_ey29_ey32_yp3_ad1",
            inputNames: [
                "queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
                "starts", "new_keys", "new_values", "write_fence",
    ...
    private static let portQuantFusedWriteResidentNormRopeKernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_f4_normrope_v1_ey29_ey32_yp3_ad1",
            inputNames: [
                "raw_queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
    ...
    private static let portQuantFusedWriteResidentNormRopeORunsumKernel:
        MLXFast.MLXFastKernel = MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_f4_normrope_ors_v1_ey29_ey32_yp3_ad1",
            inputNames: [
                "raw_queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
                "starts", "raw_keys", "raw_values", "q_weight", "k_weight",
    ...
    static func attendRingQuant(
        ...
        let startArray = memoizedRingStarts(starts.map(UInt32.init), [batch])
        let partialShape = [batch, queryHeads, 1, blocks, headDim]
        let summaryShape = [batch, queryHeads, 1, blocks]
...
public enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    static func updateAndAttendWriting22(
        ...
        let paramsArray = CBv2RaggedTwoPassDecodeAttentionV1.memoizedParams(params)
