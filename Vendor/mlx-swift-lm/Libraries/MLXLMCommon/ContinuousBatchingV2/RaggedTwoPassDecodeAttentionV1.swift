        ...
        MLXFast.metalKernel(
            ...
            source: """
                ...
                for (int token = block; token < N; token += BLOCKS) {
                    ...
                    const float2 scores = simd_sum(float2(score_lo, score_hi));

                    const float2 cur_max = float2(max_lo, max_hi);
                    const float2 new_max = max(cur_max, scores);
                    const float2 old_factor = fast::exp(cur_max - new_max);
                    const float2 score_factor = fast::exp(scores - new_max);
                    max_lo = new_max.x;
                    max_hi = new_max.y;
                    sum_lo = sum_lo * old_factor.x + score_factor.x;
                    sum_hi = sum_hi * old_factor.y + score_factor.y;
                    for (int element = 0; element < values_per_lane; ++element) {
                        const float value_element =
                            fma(float((vw >> (4 * element)) & 0xfu), vs, vb);
                        acc_lo[element] = acc_lo[element] * old_factor.x
                            + score_factor.x * value_element;
                        acc_hi[element] = acc_hi[element] * old_factor.y
                            + score_factor.y * value_element;
                    }
