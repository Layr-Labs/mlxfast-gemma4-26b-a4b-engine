...
public enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    private static let portQuantFusedWriteResidentKernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_ey29_ey32_yp3_ey46",
            inputNames: [
                "queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
                "starts", "new_keys", "new_values", "write_fence",
    ...
    private static let portQuantFusedWriteResidentNormRopeKernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_f4_normrope_v1_ey29_ey32_yp3_ey46",
            inputNames: [
                "raw_queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
    ...
    private static let portQuantFusedWriteResidentNormRopeORunsumKernel:
        MLXFast.MLXFastKernel = MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_colred_vload_c3_f4_normrope_ors_v1_ey29_ey32_yp3_ey46",
            inputNames: [
                "raw_queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
                "starts", "raw_keys", "raw_values", "q_weight", "k_weight",
...
public enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    /// QKSOFTMAX-D512 kill switch: `DARKBLOOM_GEMMA4_D512_QKSOFTMAX` set to
    /// `0`/`false`/`no`/`off` restores the separate fenced QKᵀ dispatch and
    /// the `_sv1` softmax dispatch. Default ON.
    private static let qkSoftmaxEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_D512_QKSOFTMAX"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()

    /// Physical simdgroups per 128-key softmax slab in the fused kernel:
    /// `DARKBLOOM_GEMMA4_D512_QKSOFTMAX_SPLIT` = 1 (one simdgroup computes
    /// the slab's sixteen 8-key fold batches) or 2 (two simdgroups take
    /// eight batches each and join their row sums through threadgroup
    /// memory). Both are bit-identical; the split only changes the launch
    /// shape (288 vs 576 threads per threadgroup in the ranked window).
    private static let qkSoftmaxSplit: Int = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_D512_QKSOFTMAX_SPLIT"], let value = Int(raw)
        else { return 2 }
        return value == 1 ? 1 : 2
    }()

    /// Diagnostic: `DARKBLOOM_GEMMA4_D512_QKSOFTMAX_XCHECK=1` also runs the
    /// stock dispatches 1-2 on every fused call and appends the number of
    /// differing bf16 probability words to `/tmp/qks_xcheck.log`.
    private static let qkSoftmaxXcheck: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_D512_QKSOFTMAX_XCHECK"]
        else { return false }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()

    /// Dispatch 1+2 fused — QKᵀ and the row softmax in one kernel, no score
    /// buffer. Grid: (row, kv head, quad of the GQA-8 group) threadgroups of
    /// SPLIT · 32·ceil(kL/128) threads (SPLIT × the `_sv1` softmax
    /// threadgroup). A threadgroup owns the whole key axis of its four score
    /// rows, so the row maximum and row sum never cross threadgroups; the
    /// price is that each kv head's key plane is streamed twice (once per
    /// quad) instead of once.
    ///
    /// Exactness against dispatch 1 + dispatch 2 (`_sv1`):
    ///
    /// - Scores. Dispatch 1 gives each lane the columns `lane*4 + 128*i +
    ///   tn` of every key row, accumulates `k * q` for one (head, key) over
    ///   `(i, tn)` in order into a zeroed float, and folds the 32 lane
    ///   partials with the five-stage xor butterfly (16, 8, 4, 2, 1). Its
    ///   value for a (key, head) does not depend on which 4-key block or
    ///   threadgroup computed it (the tail shift recomputes keys and stores
    ///   the same words). This kernel's fold batch is 8 keys × 4 heads
    ///   instead of 4 keys × 8 heads: the same per-lane column set, the same
    ///   accumulation statement in the same `(i, tn)` order, and the fold
    ///   copied verbatim — only the (head, key) an accumulator slot names
    ///   changes, and lane `l` lands slot `l` = (head `l>>3`, key `l&7`).
    ///   Each landed score is rounded through `T` exactly as dispatch 1
    ///   stores it and dispatch 2 reloads it. Keys past `key_length` read a
    ///   clamped row (a valid address) and are masked to `-INFINITY` before
    ///   the maximum, as dispatch 2 pads its row.
    /// - Maximum. Order-free (a maximum is one of its inputs; the only
    ///   ambiguity, the sign of a zero, cannot change `x - max` for any
    ///   `x` that reaches `exp`).
    /// - Row sum. Dispatch 2's thread `lid` sums `exp` over keys
    ///   `4lid..4lid+3` in order into `0.0f`, `simd_sum`s the 32 thread sums
    ///   of its simdgroup (keys `128s..128s+127`) and adds the simdgroup sums
    ///   in ascending `s` into `0.0f`. Here the four `exp` values of keys
    ///   `4b..4b+3` sit in four adjacent lanes; they are gathered in that
    ///   order into `0.0f`, the 32 four-key sums of a 128-key slab are moved
    ///   so that lane `b` holds the sum of keys `4b..4b+3` (SPLIT=2: the
    ///   upper sixteen cross from the sibling simdgroup through threadgroup
    ///   memory), and the SAME `simd_sum` intrinsic and the same ascending
    ///   loop finish the row. `1.0f / normalizer` and
    ///   `static_cast<T>(exp * inv)` are dispatch 2's expressions.
    private static func qkSoftmaxSource(split: Int) -> String {
        let exchange: String
        if split == 2 {
            exchange = """
                if (slice == 1) {
                    ...
                    for (int h = 0; h < HQ; ++h) {
                        if (lane >= 16) {
                            local_xchg[(h * 16 + rsg) * 16 + (lane - 16)] = rn[h];
                        }
                    }
                }
                threadgroup_barrier(mem_flags::mem_threadgroup);
                if (slice == 0) {
                    ...
                    for (int h = 0; h < HQ; ++h) {
                        if (lane >= 16) {
                            rn[h] = local_xchg[(h * 16 + rsg) * 16 + (lane - 16)];
                        }
                        const float total = simd_sum(rn[h]);
                        if (lane == 0) {
                            local_normalizer[h * 32 + rsg] = total;
                        }
                    }
                }
                threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        return """
            ...
            constexpr int HQ = 4;
            constexpr int KB = 8;
            constexpr int SPLIT = \(split);
            constexpr int NB = 16 / SPLIT;
            constexpr int n_iter = D / 128;

            const int key_length = int(params[0]);
            const int z = int(threadgroup_position_in_grid.z);
            const int quad = z & 1;
            const int row_kv = z >> 1;
            const int row = row_kv >> 1;
            const int kv_head = row_kv & 1;
            const int lane = int(thread_index_in_simdgroup);
            const int psg = int(simdgroup_index_in_threadgroup);
            const int rsg = psg / SPLIT;
            const int slice = psg - rsg * SPLIT;
            const int num_role_simdgroups = (key_length + 127) / 128;
            const int num_phys_simdgroups = num_role_simdgroups * SPLIT;
            ...
            const int head0 = row * 16 + kv_head * 8 + quad * HQ;
            const device T* query = queries + size_t(head0) * D;
            device T* prob_rows = probs + size_t(head0) * key_length;

            const int hq = lane >> 3;
            const int koff = lane & 7;

            threadgroup float local_max[HQ * 32];
            threadgroup float local_normalizer[HQ * 32];
        \(split == 2 ? "    threadgroup float local_xchg[HQ * 16 * 16];" : "")

            typedef vec<T, 4> T4;
            // sc[j] holds fold batch c = NB - 1 - j of this simdgroup's
            // slice: head hq, key 128*rsg + 8*(slice*NB + c) + koff.
            float sc[NB];
            #pragma clang loop unroll(full)
            for (int j = 0; j < NB; ++j) {
                sc[j] = 0.0f;
            ...
            for (int c = 0; c < NB; ++c) {
                const int key0 = 128 * rsg + 8 * (slice * NB + c);
                float result[HQ * KB] = {0.0f};
                T4 k_tile[KB];
                float q_coeff[4];
                int bn = lane * 4;
                for (int i = 0; i < n_iter; ++i) {
                    #pragma clang loop unroll(full)
                    for (int tm = 0; tm < KB; ++tm) {
                        const int krow = min(key0 + tm, key_length - 1);
                        k_tile[tm] = *reinterpret_cast<const device T4*>(
                            key_plane + size_t(krow) * D + bn);
                    }
                    #pragma clang loop unroll(full)
                    for (int h = 0; h < HQ; ++h) {
                        const T4 q_raw = *reinterpret_cast<const device T4*>(
                ...
                sc[0] = static_cast<float>(static_cast<T>(result[0]));
            }

            // Dispatch 2 from here: pad past the row end, row maximum.
            float maxval = -3.402823466e+38F;
            #pragma clang loop unroll(full)
            for (int j = 0; j < NB; ++j) {
                const int key =
                    128 * rsg + 8 * (slice * NB + (NB - 1 - j)) + koff;
                sc[j] = (key < key_length) ? sc[j] : -INFINITY;
                maxval = (maxval < sc[j]) ? sc[j] : maxval;
            }
            {
                float other = simd_shuffle_xor(maxval, ushort(1));
                maxval = (maxval < other) ? other : maxval;
                other = simd_shuffle_xor(maxval, ushort(2));
                maxval = (maxval < other) ? other : maxval;
                other = simd_shuffle_xor(maxval, ushort(4));
                maxval = (maxval < other) ? other : maxval;
            }
            if (koff == 0) {
                local_max[hq * 32 + psg] = maxval;
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
            maxval = -3.402823466e+38F;
            for (int s = 0; s < num_phys_simdgroups; ++s) {
                const float sm = local_max[hq * 32 + s];
                maxval = (maxval < sm) ? sm : maxval;
            }
            ...
            #pragma clang loop unroll(full)
            for (int j = 0; j < NB; ++j) {
                e[j] = fast::exp(sc[j] - maxval);
            }

            // Four-key sums in dispatch 2's thread order: keys 4b..4b+3 are
            // the four adjacent lanes (lane & ~3) + 0..3 of one register.
            const int quad_base = lane & ~3;
            float n[NB];
            #pragma clang loop unroll(full)
            for (int j = 0; j < NB; ++j) {
                float acc = 0.0f;
                acc += simd_shuffle(e[j], ushort(quad_base));
                acc += simd_shuffle(e[j], ushort(quad_base + 1));
                acc += simd_shuffle(e[j], ushort(quad_base + 2));
                acc += simd_shuffle(e[j], ushort(quad_base + 3));
                n[j] = acc;
            }

            // Move the four-key sum of keys 4b..4b+3 (head h) to lane b:
            // it sits in lanes 8h + 4(b&1) + 0..3 of the register for the
            // 8-key batch b>>1.
            float rn[HQ];
            #pragma clang loop unroll(full)
            for (int h = 0; h < HQ; ++h) {
            ...
            #pragma clang loop unroll(full)
            for (int j = 0; j < NB; ++j) {
                const int cb = slice * NB + (NB - 1 - j);
                #pragma clang loop unroll(full)
                for (int h = 0; h < HQ; ++h) {
                    const float v = simd_shuffle(
                        n[j], ushort(8 * h + 4 * (lane & 1)));
                    rn[h] = ((lane >> 1) == cb) ? v : rn[h];
                }
            }
        \(exchange)
            float normalizer = 0.0f;
            for (int s = 0; s < num_role_simdgroups; ++s) {
                normalizer += local_normalizer[hq * 32 + s];
            }
            const float inv_normalizer = 1.0f / normalizer;
            ...
            for (int j = 0; j < NB; ++j) {
                const int key =
                    128 * rsg + 8 * (slice * NB + (NB - 1 - j)) + koff;
                if (key < key_length) {
                    prob_rows[size_t(hq) * key_length + key] =
                        static_cast<T>(e[j] * inv_normalizer);
                }
            }
        """
    }

    private static let qkSoftmaxInputNames: [String] = [
        "queries",
        "k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7",
        "params", "store_fence",
    ]

    private static let qkSoftmaxKernelSplit1: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_d512_qksoftmax_bf16_g4_xfold_sp1_qks1",
            inputNames: qkSoftmaxInputNames,
            outputNames: ["probs"],
            source: qkSoftmaxSource(split: 1),
            ensureRowContiguous: true
        )

    private static let qkSoftmaxKernelSplit2: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_d512_qksoftmax_bf16_g4_xfold_sp2_qks1",
            inputNames: qkSoftmaxInputNames,
            outputNames: ["probs"],
            source: qkSoftmaxSource(split: 2),
            ensureRowContiguous: true
        )

    private static var qkSoftmaxActive: MLXFast.MLXFastKernel {
        qkSoftmaxSplit == 1 ? qkSoftmaxKernelSplit1 : qkSoftmaxKernelSplit2
    }

    /// The fused kernel's threadgroup is SPLIT × the softmax threadgroup and
    /// must stay within 1024 threads.
    private static var qkSoftmaxMaxKeyLength: Int {
        qkSoftmaxSplit == 1 ? maxKeyLength : 2048
    }

    private static let qkSoftmaxXcheckLock = NSLock()

    private static func qkSoftmaxXcheckLog(_ line: String) {
        qkSoftmaxXcheckLock.lock()
        defer { qkSoftmaxXcheckLock.unlock() }
        let path = "/tmp/qks_xcheck.log"
        let data = Data((line + "\n").utf8)
    ...
    static func updateAndAttendWriting22(
        ...
        // QKSOFTMAX-D512: dispatches 1 and 2 as one kernel; the score
        // buffer is never materialized.
        let probs: MLXArray
        if qkSoftmaxEnabled, keyLength <= qkSoftmaxMaxKeyLength {
            let fusedThreads = softmaxThreads * qkSoftmaxSplit
            probs = qkSoftmaxActive(
                [liveQueries] + keyBuffers + [paramsArray, storeFence],
                template: template,
                grid: (fusedThreads, 1, batch * kvHeads * 2),
                threadGroup: (fusedThreads, 1, 1),
                outputShapes: [scratchShape],
                outputDTypes: [.bfloat16]
            )[0]
            CBv2EngageMark.once("d512-qk-softmax")
            if qkSoftmaxXcheck {
                let reference = stockProbs()
                let differing = (probs.view(dtype: .uint16)
                    .!= reference.view(dtype: .uint16))
                    .asType(.int32).sum().item(Int32.self)
                qkSoftmaxXcheckLog(
                    "kL=\(keyLength) split=\(qkSoftmaxSplit) differing=\(differing) of \(batch * queryHeads * keyLength)")
            }
        } else {
            probs = stockProbs()
        }
