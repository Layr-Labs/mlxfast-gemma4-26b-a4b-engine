...
enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    private static let passBKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        source: """
            ...
            const float inv_sum = sum_exp_score == 0.0f ? 1.0f : (1.0f / sum_exp_score);
            #pragma clang loop unroll(full)
            for (int element = 0; element < values_per_lane; element += 2) {
                float2 red2 = float2(accumulator[element], accumulator[element + 1]);
                for (int stride = 1; stride < COLS; stride <<= 1) {
                    red2 += simd_shuffle_xor(red2, ushort(stride));
                }
                if (block_lane == 0) {
                    out[element] = T(sum_exp_score == 0.0f ? red2.x : red2.x * inv_sum);
                    out[element + 1] = T(sum_exp_score == 0.0f ? red2.y : red2.y * inv_sum);
                }
            }
        """,
        ensureRowContiguous: true
