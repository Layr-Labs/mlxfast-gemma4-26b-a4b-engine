...
enum CBv2RaggedComposedD512DecodeAttentionV1 {
    /// Kill-switch. DEFAULT ON (`DARKBLOOM_GEMMA4_D512_DECODE_SDPA=0`
    /// restores the per-row chain): +2.2% decode on the local B=8 warm
    /// driver (p50 19.56 -> 19.13 ms), bit-exact per parity-d512.
    private static let enabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
