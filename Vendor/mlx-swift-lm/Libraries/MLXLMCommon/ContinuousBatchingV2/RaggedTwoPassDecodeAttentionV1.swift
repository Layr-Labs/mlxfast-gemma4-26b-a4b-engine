...
enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    /// A/B switch for the block-major grid permutation of the WRITING pass A
    /// (`fusedRingPassAKernelBlockMajor`). DEFAULT OFF — the established
    /// kv-head-major grid stays the production path until the permutation is
    /// gate-measured. `DARKBLOOM_CBV2_PASSA_BLOCKMAJOR=1` enables.
    private static let passABlockMajorEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_CBV2_PASSA_BLOCKMAJOR"]
        else { return false }
    ...
    /// `fusedRingPassAKernel` with the threadgroup GRID axes permuted:
    /// `block` rides the fastest-launching x axis and `kv_head` the slowest z
    /// axis, so the threadgroups co-resident on the GPU at any instant stream
    /// CONSECUTIVE blocks of the SAME (batch, kv_head) K/V plane front-to-back
    /// instead of 64 (kv_head, batch) planes at a few block offsets each.
    ///
    /// Pure scheduling change. The kernel body below is a byte-for-byte copy
    /// of `fusedRingPassAKernel` except the two coordinate-derivation lines
    /// (`kv_head` and `block` swap x and z; `batch_index` stays on y): every
    /// TG computes `(kv_head, batch_index, block)` to the same VALUES from
    /// permuted coordinates and then performs the identical loads in the
    /// identical order with the identical accumulation — bit-exact by
    /// construction. The fused ring write stays race-free unchanged: the
    /// writer predicate (`block == 0 && query_head_in_group == 0`) and the
    /// write target (`(ring_start + N - 1) % N`, from `starts` only) are
    /// functions of the DERIVED values, never of raw grid coordinates, and no
    /// TG reads that slot under either order (logical token `N - 1` is served
    /// from `new_keys`/`new_values`). New kernel NAME so the Metal pipeline
    /// cache can never alias the two grids onto one compiled pipeline.
    ///
    /// Selected by `DARKBLOOM_CBV2_PASSA_BLOCKMAJOR=1` (default off) together
    /// with the matching grid swap at the call site.
    private static let fusedRingPassAKernelBlockMajor: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_ringwrite_sdpa_2pass_a_bf16_d256_g2_b\(blocks)_bmaj_v1",
            inputNames: [
                "queries",
                "k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7",
                "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7",
            ...
            source: """
                ...
                const int kv_head = int(threadgroup_position_in_grid.z);
                const int batch_index = int(threadgroup_position_in_grid.y);
    ...
    static func attendRingWriting(
        ...
        // Grid-axis permutation A/B (default off): the block-major variant
        // derives the SAME (kv_head, batch_index, block) values from permuted
        // grid coordinates, so the grid swap here and the kernel swap must
        // stay under the one switch.
        let blockMajor = passABlockMajorEnabled
        if blockMajor { CBv2EngageMark.once("passabm") }
        let selectedPassAKernel =
            blockMajor ? fusedRingPassAKernelBlockMajor : fusedRingPassAKernel
...
/// on: env kill-switch (DEFAULT OFF until box-proven), any other batch size,
...
enum CBv2RaggedComposedD512DecodeAttentionV1 {
    /// DEFAULT OFF pending a box run that proves the kernels compile on the
    /// ranked toolchain (a default-on submission failed pre-score during a
    /// wave of box failures; cause unbisected — local M5 compiles and runs
    /// it bit-exact at +2.2% decode). `DARKBLOOM_GEMMA4_D512_DECODE_SDPA=1`
    /// enables.
    private static let enabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
