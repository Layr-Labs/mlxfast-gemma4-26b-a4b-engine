...
enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    /// VEC8-U2-001: the vec8 paired pass A walks its ring rounds two at a
    /// time, with BOTH rounds' K and V uint4 loads issued before either
    /// round's arithmetic.
    ///
    /// The vec8 token loop is one long dependence chain: each round's loads
    /// feed its own online-softmax update before the next round's loads
    /// issue, so at most one K/V pair per lane is outstanding. Pairing two
    /// rounds and hoisting all four 16-byte loads to the top of the pair
    /// doubles the memory-level parallelism of the dominant streaming loop
    /// without moving a single arithmetic operation: round t's scores,
    /// `simd_sum`s, `fast::exp`s, and accumulator updates all complete
    /// before round t+BLOCKS's begin, exactly as in the rolled loop.
    /// Loads move; adds do not. The 127 ring rounds become 63 pairs plus
    /// one single ring round, followed by the same peeled final round —
    /// the SAME 128 tokens in the SAME order, so every output element's
    /// add sequence is unchanged and the kernel is bit-exact against the
    /// vec8 kernel by construction.
    ///
    /// Kill switch: `DARKBLOOM_CBV2_DECODE_PAIRED_PASSA_VEC8_UNROLL2=0`
    /// falls back to the vec8 kernel, which stays in the file untouched.
    static let gqaPairedPassAVec8Unroll2Enabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_CBV2_DECODE_PAIRED_PASSA_VEC8_UNROLL2"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()

    private static let fusedRingPassAPairedVec8Unroll2Kernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name:
                "cbv2_ragged8_ringwrite_sdpa_2pass_a_gqapair_vec8u2_bf16_d256_g2"
                + "_b\(blocks)_v1",
            inputNames: [
                "queries",
                "k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7",
            ...
            source: """
                ...
                float sum_lo = 0.0f;
                float sum_hi = 0.0f;
                // Rounds 0..125 as 63 pairs: all four 16-byte loads of a pair
                // issue before either round's arithmetic; round t completes
                // its softmax update before round t+BLOCKS's begins, exactly
                // as in the rolled vec8 loop.
                int token = block;
                for (; token + BLOCKS < N - BLOCKS; token += 2 * BLOCKS) {
                    const uint slot1 = (slot + uint(BLOCKS)) & ring_mask;
                    const uint4 key_raw0 = *reinterpret_cast<const device uint4*>(
                        keys + slot * D);
                    const uint4 value_raw0 = *reinterpret_cast<const device uint4*>(
                        values + slot * D);
                    const uint4 key_raw1 = *reinterpret_cast<const device uint4*>(
                        keys + slot1 * D);
                    const uint4 value_raw1 = *reinterpret_cast<const device uint4*>(
                        values + slot1 * D);
                    {
                        const T4 key_chunks[2] = {
                            as_type<T4>(key_raw0.xy), as_type<T4>(key_raw0.zw)};
                        const T4 value_chunks[2] = {
                            as_type<T4>(value_raw0.xy), as_type<T4>(value_raw0.zw)};
                        float score_lo = 0.0f;
                        float score_hi = 0.0f;
                        #pragma clang loop unroll(full)
                        for (int chunk = 0; chunk < vectors_per_lane; ++chunk) {
                    ...
                    {
                        const T4 key_chunks[2] = {
                            as_type<T4>(key_raw1.xy), as_type<T4>(key_raw1.zw)};
                        const T4 value_chunks[2] = {
                            as_type<T4>(value_raw1.xy), as_type<T4>(value_raw1.zw)};
                        float score_lo = 0.0f;
                        float score_hi = 0.0f;
                        #pragma clang loop unroll(full)
                        for (int chunk = 0; chunk < vectors_per_lane; ++chunk) {
                    ...
                    slot = (slot + uint(2 * BLOCKS)) & ring_mask;
                }
                // Round 126 (the odd ring round), byte-for-byte the vec8 body.
                if (token < N - BLOCKS) {
                    const uint4 key_raw = *reinterpret_cast<const device uint4*>(
                        keys + slot * D);
                    const uint4 value_raw = *reinterpret_cast<const device uint4*>(
                ...
                // Round 127, token == block + N - BLOCKS: the peeled final
                // round, character for character the vec4/vec8 tail.
                {
                    const bool current = token == N - 1;
                    const device T4* k = reinterpret_cast<const device T4*>(
                        current ? new_key : keys + slot * D);
    ...
    /// PASSB-VEC4-001: the exact B8/D256/BLOCKS8/COLS8 merge owns eight
    /// contiguous BF16 partials and eight contiguous BF16 outputs per lane.
    /// Carry each run as two T4 values while leaving every float conversion,
    /// multiply-add, xor reduction, division, and BF16 rounding in the
    /// incumbent order. The two chunks are written explicitly rather than
    /// nested so only one T4 stays live at a time.
    private static let passBVec4Kernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_sdpa_2pass_b_direct_bf16_d256_b8_c8_vec4_v1",
        inputNames: ["partials", "sums", "maxs"],
        outputNames: ["out"],
        source: """
            constexpr int simd_width = 32;
            ...
            for (int round = 0; round < rounds; ++round) {
                ...
                if (column < BLOCKS) {
                    const float factor = lane_factor[round];
                    const device T4* partial4 =
                        reinterpret_cast<const device T4*>(partials + column * D);
                    const T4 packed0 = partial4[0];
                    for (int component = 0; component < 4; ++component) {
                        accumulator[component] +=
                            factor * float(packed0[component]);
                    }
                    const T4 packed1 = partial4[1];
                    for (int component = 0; component < 4; ++component) {
                        accumulator[4 + component] +=
                            factor * float(packed1[component]);
                    }
                }
            }

            device T4* out4 = reinterpret_cast<device T4*>(out);
            T4 packed0;
            for (int component = 0; component < 4; ++component) {
                float reduced = accumulator[component];
                for (int stride = 1; stride < COLS; stride <<= 1) {
                    reduced += simd_shuffle_xor(reduced, ushort(stride));
                }
                if (block_lane == 0) {
    ...
    /// Default-on kill switch. Shape/dtype mismatches retain the incumbent
    /// scalar merge, including every non-BF16 or non-B8 diagnostic profile.
    private static let passBVec4Enabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_CBV2_DECODE_PASSB_VEC4"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()

    private static func selectedPassBKernel(dtype: DType) -> MLXFast.MLXFastKernel {
        guard passBVec4Enabled,
            dtype == .bfloat16,
            batch == 8,
            headDim == 256,
            blocks == 8,
            combineColumns == 8
        else { return passBKernel }
        CBv2EngageMark.once("decode-passb-vec4")
        return passBVec4Kernel
    }
    ...
    static func attendRingWriting(
        ...
        let unrolled = wide && gqaPairedPassAVec8Unroll2Enabled
        if unrolled {
            CBv2EngageMark.once("decode-gqa-paired-passa-vec8u2")
        }
        let pairedKernel =
            unrolled
            ? fusedRingPassAPairedVec8Unroll2Kernel
            : wide
        ...
        let passBDType = queries.dtype
        let output = selectedPassBKernel(dtype: passBDType)(
    ...
    private static func attend(
        ...
        let passBDType = queries.dtype
        return selectedPassBKernel(dtype: passBDType)(
            passA,
            template: [
                ("T", passBDType),
                ("D", headDim),
