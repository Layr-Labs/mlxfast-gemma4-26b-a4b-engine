...
public enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    private static let slidingPrefetchKey: String = {
        ...
        let base = slidingPrefetchPeelEnabled
...
"""
    ...
    /// The resident walk bodies after SLIDE-SOFTREF. `applied` is true
    /// only when all substituted blocks match; otherwise every body is the
    /// promoted text and the kernel keys stay unchanged.
    private static let slidingSoftRefWalks:
        (depth1: String, depth2: String, peeled: String, peeledOffset: String, applied: Bool) = {
        let promoted = (
        ...
        guard slidingSoftRefMode != 0 else {
            return (promoted.0, promoted.1, promoted.2, promoted.3, false)
        }
        ...
        let peeledOffset = slidingSoftRefRewrite(promoted.3)
        guard one.1 == 1, two.1 == 1, peeled.1 == 2, peeledOffset.1 == 2 else {
            return (promoted.0, promoted.1, promoted.2, promoted.3, false)
        }
        return (one.0, two.0, peeled.0, peeledOffset.0, true)
    }()
    ...
    private static var residentSlidingWalk: String {
        ...
        guard slidingPrefetchPeelEnabled else { return slidingSoftRefWalks.depth2 }
        return ringOffsetEnabled
            ? slidingSoftRefWalks.peeledOffset : slidingSoftRefWalks.peeled
    }
                ...
                queries: queries, keys: newKeys, values: newValues)
            {
                ...
                if slidingPrefetchDepth2 && slidingPrefetchPeelEnabled {
                    if ringOffsetEnabled {
                        CBv2EngageMark.once("sliding-prefetch-pf2-tail-peel-ring-offset")
                    } else {
