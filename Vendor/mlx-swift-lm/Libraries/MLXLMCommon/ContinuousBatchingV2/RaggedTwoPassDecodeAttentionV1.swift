...
enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    /// PASSA-VSTORE-001: the resident pass A above, with its partial store
    /// four-wide.
    ///
    /// The merge half of that kernel already reads these partials through
    /// `reinterpret_cast<const threadgroup T4*>`, so the buffer is consumed
    /// four-wide while pass A fills it one bf16 at a time: sixteen scalar
    /// threadgroup stores per lane (eight lo, eight hi) where four vector
    /// stores carry the same bytes. The sibling device-buffer pass A
    /// (`fusedRingPassAPairedVec4Kernel`) has carried the vector form for
    /// several tips; the resident kernel simply never inherited it.
    ///
    /// Exact by construction: same values, same slots, same conversion, and
    /// the store still lands before the same `threadgroup_barrier`.
    private static let portQuantFusedWriteResidentVStoreKernel: MLXFast.MLXFastKernel =
        MLXFast.metalKernel(
            name: "cbv2_ragged8_sdpa_ringwrite_q4g64_d256_g2_regpack_vec4_carry_pair_b8_resident_vstore_v1",
            inputNames: [
                "queries",
                "m0", "m1", "m2", "m3", "m4", "m5", "m6", "m7",
                "starts", "new_keys", "new_values", "write_fence",
            ...
            source: """
                ...
                // PASSA-VSTORE-001: the same eight lo and eight hi values,
                // in the same threadgroup slots, converted by the same
                // T(float) cast -- only the store width changes. `partial` is
                // `local_partials + block * D + lane * values_per_lane`, and
                // with D a multiple of four and values_per_lane a multiple of
                // four both that offset and the `+ BLOCKS * D` companion are
                // multiples of four elements, so every T4 lands on its own
                // natural boundary. The four-wide read below already
                // reinterpret_casts this very array, so the base alignment
                // this relies on is the one the merge already requires.
                constexpr bool vector_store =
                    (values_per_lane % 4 == 0) && (D % 4 == 0);
                if (vector_store) {
                    threadgroup T4* partial_lo =
                        reinterpret_cast<threadgroup T4*>(partial);
                    threadgroup T4* partial_hi =
                        reinterpret_cast<threadgroup T4*>(partial + BLOCKS * D);
                    #pragma clang loop unroll(full)
                    for (int chunk = 0; chunk < vectors_per_lane; ++chunk) {
                        T4 lo_vector;
    ...
    /// PASSA-VSTORE-001 selector. ON by default;
    /// `DARKBLOOM_GEMMA4_PASSA_VSTORE=0` restores the incumbent resident
    /// kernel byte for byte, including its cached pipeline name.
    private static let passAVectorStore: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_PASSA_VSTORE"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()

    private static var portQuantFusedWriteResidentActive: MLXFast.MLXFastKernel {
        if passAVectorStore {
            CBv2EngageMark.once("passa-vstore")
            return portQuantFusedWriteResidentVStoreKernel
        }
        return portQuantFusedWriteResidentKernel
    }
    ...
    static func attendRingQuantWriting(
        ...
        if q4ResidentMergeEnabled,
            ...
            combineThreads == 256
        {
            let resident = portQuantFusedWriteResidentActive(
                inputs,
                template: [
                    ("T", queries.dtype),
                    ("D", headDim),
...
enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    /// D512-VLOAD-001: the tile reads of the D=512 decode chain.
    ///
    /// Dispatch 1 stages a 4x4 key tile and a 4-wide query run per GQA head,
    /// and dispatch 3 stages a 4x4 value tile, and every one of those runs is
    /// four CONSECUTIVE elements of one row read as four separate scalar
    /// loads. The value plane is the heaviest byte stream of the chain and
    /// the key plane is the same size, so those scalar runs are most of the
    /// traffic the chain issues.
    ///
    /// Each run is provably four-element aligned, which is what lets it be
    /// one load instead of four: the row stride is D = 512, the plane base is
    /// offset by kv_head * row_capacity * D, the key tile column base is
    /// `bn = lane * 4 + 128 * i`, the query base adds `h * D`, and the value
    /// tile column base is `tile * avTileColumns + (4 * sg + thrN) * 4` with
    /// avTileColumns a multiple of four. Every term is a multiple of four
    /// elements, so the address is eight-byte aligned for a two-byte element
    /// type and a four-wide vector load is well formed. Each run also lies
    /// wholly inside its own row: the tile column base is at most D - 4, and
    /// the row index is the one the scalar form already read.
    ///
    /// Exactness: the loaded bits are the same bits, and each component is
    /// widened and consumed exactly where the element walk consumed it, so
    /// every product and every accumulator update is unchanged in value and
    /// in order. Bit-exact by construction; no token budget is spent.
    ///
    /// `DARKBLOOM_GEMMA4_D512_VLOAD=0` restores the incumbent scalar runs,
    /// and the incumbent pipeline names with them.
    private static let vloadEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_GEMMA4_D512_VLOAD"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()

    /// The rewrites, as literal source fragments. Each fragment's closing
    /// delimiter sits at the same column as the kernel source's own, so both
    /// are stripped by the same amount and the fragment matches the kernel
    /// body character for character. A fragment that fails to match leaves
    /// the incumbent source in place and `vloadApplied` false, so the
    /// switch-off path and a missed match are the same tree.
    private static let qkKeyTileFrom: String = """
    ...
    private static let qkKeyTileTo: String = """
                    for (int tm = 0; tm < 4; ++tm) {
                        const vec<T, 4> ktile4 =
                            *reinterpret_cast<const device vec<T, 4>*>(
                                mat + mat_offset + bn);
                        #pragma clang loop unroll(full)
                        for (int tn = 0; tn < 4; ++tn) {
                            k_tile[tm][tn] = ktile4[tn];
                        }
                        mat_offset += D;
                    }
        """

    private static let qkQueryFrom: String = """
                        #pragma clang loop unroll(full)
                        for (int tn = 0; tn < 4; ++tn) {
    ...
    private static let qkQueryTo: String = """
                        const vec<T, 4> qrun4 =
                            *reinterpret_cast<const device vec<T, 4>*>(
                                query + h * D + bn);
                        #pragma clang loop unroll(full)
                        for (int tn = 0; tn < 4; ++tn) {
                            q_coeff[tn] = static_cast<float>(qrun4[tn]);
                        }
        """

    private static let avTileFrom: String = """
                for (int tm = 0; tm < 4; ++tm) {
                    for (int tn = 0; tn < 4; ++tn) {
    ...
    private static let avTileTo: String = """
                for (int tm = 0; tm < 4; ++tm) {
                    const vec<T, 4> vtile4 =
                        *reinterpret_cast<const device vec<T, 4>*>(
                            value_plane + size_t(bm + tm) * D + out_col);
                    #pragma clang loop unroll(full)
                    for (int tn = 0; tn < 4; ++tn) {
                        v_tile[tm][tn] = vtile4[tn];
                    }
                }
        """

    private static let avRowFrom: String = """
                    #pragma clang loop unroll(full)
                    for (int tn = 0; tn < 4; ++tn) {
    ...
    private static let avRowTo: String = """
                    const vec<T, 4> vrow4 =
                        *reinterpret_cast<const device vec<T, 4>*>(
                            value_plane + size_t(bm + tm) * D + out_col);
                    #pragma clang loop unroll(full)
                    for (int tn = 0; tn < 4; ++tn) {
                        v_tile[0][tn] = vrow4[tn];
                    }
        """

    private static let qkSource: String = {
        guard vloadEnabled else { return qkSourceBase }
        return qkSourceBase
            .replacingOccurrences(of: qkKeyTileFrom, with: qkKeyTileTo)
            .replacingOccurrences(of: qkQueryFrom, with: qkQueryTo)
    }()

    private static let avSource: String = {
        guard vloadEnabled else { return avSourceBase }
        return avSourceBase
            .replacingOccurrences(of: avTileFrom, with: avTileTo)
            .replacingOccurrences(of: avRowFrom, with: avRowTo)
    }()

    /// True only when the switch is on AND every fragment above actually
    /// matched, so a stale fragment cannot ship a renamed pipeline that holds
    /// the incumbent body.
    private static let vloadApplied: Bool =
        vloadEnabled && qkSource != qkSourceBase && avSource != avSourceBase
        && !qkSource.contains(qkKeyTileFrom) && !qkSource.contains(qkQueryFrom)
        && !avSource.contains(avTileFrom) && !avSource.contains(avRowFrom)

    private static let vloadSuffix: String = vloadApplied ? "_vload_v1" : ""
    ...
    private static let qkSourceBase: String = """
            constexpr int D = 512;
            constexpr int GQA = 8;
    ...
    private static let qkKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_sdpa_d512_qk_bf16_g8_xfold_v3\(vloadSuffix)",
        inputNames: [
            "queries",
            "k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7",
            "params",
    ...
    private static let qkFencedKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_sdpa_d512_qk_fenced_bf16_g8_xfold_v3\(vloadSuffix)",
        inputNames: [
            "queries",
            "k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7",
            "params", "store_fence",
    ...
    private static let avSourceBase: String = """
            constexpr int D = 512;
            constexpr int GQA = 8;
    ...
    private static let avKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_sdpa_d512_av_bf16_g8_xfold_v3_t\(avColumnTiles)\(vloadSuffix)",
        inputNames: [
            "probs",
            "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7",
            "params",
    ...
    static func updateAndAttendWriting22(
        ...
        if vloadApplied { CBv2EngageMark.once("d512-vload") }
