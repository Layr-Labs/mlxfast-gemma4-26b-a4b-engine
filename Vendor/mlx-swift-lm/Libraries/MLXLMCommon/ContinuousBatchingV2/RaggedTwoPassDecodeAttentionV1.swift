...
public enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    public static func registerResidentNormRope(
        ...
        guard q4ResidentNormRopeEnabled,
            ...
            normalizedQueries.dim(0) == batch,
            normalizedQueries.dim(1) == queryHeads,
            normalizedQueries.dim(2) == 1,
            normalizedQueries.dim(3) == headDim,
            normalizedKeys.dtype == .bfloat16,
            normalizedKeys.ndim == 4,
            normalizedKeys.dim(0) == batch,
            normalizedKeys.dim(1) == kvHeads,
            normalizedKeys.dim(2) == 1,
            normalizedKeys.dim(3) == headDim,
            normalizedValues.dtype == .bfloat16,
            normalizedValues.ndim == 4,
            normalizedValues.dim(0) == batch,
            normalizedValues.dim(1) == kvHeads,
            normalizedValues.dim(2) == 1,
            normalizedValues.dim(3) == headDim,
            rawQueries.dtype == .bfloat16,
            rawQueries.ndim == 4,
            rawQueries.dim(0) == batch,
            rawQueries.dim(1) == 1,
            rawQueries.dim(2) == queryHeads,
            rawQueries.dim(3) == headDim,
            rawKeys.dtype == .bfloat16,
            rawKeys.ndim == 4,
            rawKeys.dim(0) == batch,
            rawKeys.dim(1) == 1,
            rawKeys.dim(2) == kvHeads,
            rawKeys.dim(3) == headDim,
            rawValues.dtype == .bfloat16,
            rawValues.ndim == 4,
            rawValues.dim(0) == batch,
            rawValues.dim(1) == 1,
            rawValues.dim(2) == kvHeads,
            rawValues.dim(3) == headDim,
            qWeight.dtype == .bfloat16,
            qWeight.ndim == 1,
            qWeight.dim(0) == headDim,
            kWeight.dtype == .bfloat16,
            kWeight.ndim == 1,
            kWeight.dim(0) == headDim,
            positionOffsets.dtype == .int32,
            positionOffsets.ndim == 1,
            positionOffsets.dim(0) == batch,
            ropeLog2Base.dtype == .float32,
            ropeLog2Base.ndim == 1,
            ropeLog2Base.dim(0) == 1
        else { return false }
...
public enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    public static func registerFullNormRope(
        ...
        guard normRopeFoldEnabled,
            ...
            ropeFrequencies.ndim == 1,
            ropeFrequencies.dim(0) == headDim / 2
        else { return false }
