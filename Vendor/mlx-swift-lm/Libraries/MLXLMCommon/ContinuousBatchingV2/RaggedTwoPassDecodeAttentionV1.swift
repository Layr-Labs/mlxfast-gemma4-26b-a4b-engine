...
enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    static func attendRing(
        ...
        prebuiltStarts: MLXArray? = nil
    ) -> MLXArray? {
        guard slidingWindowLength == sequenceLength,
            starts.count == batch,
            starts.allSatisfy({ 0 <= $0 && $0 < sequenceLength })
        else { return nil }
        // The prebuilt tensor replaces only the HOST construction of the same
        // [batch] uint32 values; shape/dtype are validated here and the host
        // `starts` gate above is unchanged. When absent or malformed, the
        // established host construction runs.
        let startArray: MLXArray
        if let prebuiltStarts, prebuiltStarts.dtype == .uint32,
            prebuiltStarts.shape == [batch]
        {
            startArray = prebuiltStarts
        } else {
