...
public enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    private static let ringStoreNormRopeKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_d512_ringstore_normrope_freqs_bf16_v1_vec1_nb1_ri2",
        inputNames: [
            "k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7",
            "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7",
        ...
        source: """
            ...
            threadgroup_barrier(mem_flags::mem_threadgroup);
            // All four simdgroups repeat the same 32-lane reduction of
            // the immutable partials. The inverse stays in registers, so
            // its shared-memory store and publication barrier disappear.
