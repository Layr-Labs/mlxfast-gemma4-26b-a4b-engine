...
enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    private static let portQuantReadKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        source: """
            ...
            const int batch_index = int(threadgroup_position_in_grid.y);
            const int block = int(threadgroup_position_in_grid.z);
            // GQA-PAIR-PORT: one simdgroup serves BOTH query heads of its
            // group. The packed words, the two metadata words and the nibble
            ...
            // Consecutive query heads are `batch_head` and `batch_head + 1`,
            // so the second head's slice of each output sits exactly one
            // head stride further on.
            if (lane == 0) {
                sum_out[0] = sum_lo;
                max_out[0] = max_lo;
                sum_out[BLOCKS] = sum_hi;
