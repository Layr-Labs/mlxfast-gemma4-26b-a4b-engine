...
public enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    /// RR publishes one partial per simdgroup and only consumes slots 0...3;
    /// the incumbent fallback still indexes all 32 lanes while zero-filling.
    /// Keep the fallback allocation unchanged and trim only RR's threadgroup
    /// scratch plane.
    private static let normRopePartialsCount: Int =
        normRopeRrEnabled ? 4 : 32
    ...
    private static let ringStoreNormRopeKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        source: """
            ...
            threadgroup float partials[\(normRopePartialsCount)];
            \(normRopeInvDecl)
