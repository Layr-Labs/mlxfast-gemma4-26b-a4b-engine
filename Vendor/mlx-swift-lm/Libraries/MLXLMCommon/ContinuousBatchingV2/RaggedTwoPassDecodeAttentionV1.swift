        ...
        MLXFast.metalKernel(
            ...
            source: """
                ...
                const device T4* q_hi_vec =
                    reinterpret_cast<const device T4*>(query + D);
                const T4 q0_lo = q_lo_vec[0];
                const T4 q1_lo = q_lo_vec[1];
                const T4 q0_hi = q_hi_vec[0];
                const T4 q1_hi = q_hi_vec[1];
                thread float q_lo[values_per_lane];
                thread float q_hi[values_per_lane];
                thread float acc_lo[values_per_lane];
                thread float acc_hi[values_per_lane];
                q_lo[0] = float(q0_lo[0]); q_lo[1] = float(q0_lo[1]);
                q_lo[2] = float(q0_lo[2]); q_lo[3] = float(q0_lo[3]);
                q_lo[4] = float(q1_lo[0]); q_lo[5] = float(q1_lo[1]);
                q_lo[6] = float(q1_lo[2]); q_lo[7] = float(q1_lo[3]);
                q_hi[0] = float(q0_hi[0]); q_hi[1] = float(q0_hi[1]);
                q_hi[2] = float(q0_hi[2]); q_hi[3] = float(q0_hi[3]);
                q_hi[4] = float(q1_hi[0]); q_hi[5] = float(q1_hi[1]);
                q_hi[6] = float(q1_hi[2]); q_hi[7] = float(q1_hi[3]);
                #pragma unroll
                for (int element = 0; element < values_per_lane; ++element) {
                    acc_lo[element] = 0.0f;
                    acc_hi[element] = 0.0f;
                ...
                device T4* part_hi_vec =
                    reinterpret_cast<device T4*>(partial + BLOCKS * D);
                part_lo_vec[0] = T4(T(acc_lo[0]), T(acc_lo[1]), T(acc_lo[2]), T(acc_lo[3]));
                part_lo_vec[1] = T4(T(acc_lo[4]), T(acc_lo[5]), T(acc_lo[6]), T(acc_lo[7]));
                part_hi_vec[0] = T4(T(acc_hi[0]), T(acc_hi[1]), T(acc_hi[2]), T(acc_hi[3]));
                part_hi_vec[1] = T4(T(acc_hi[4]), T(acc_hi[5]), T(acc_hi[6]), T(acc_hi[7]));
            """,
            ensureRowContiguous: true
        )
