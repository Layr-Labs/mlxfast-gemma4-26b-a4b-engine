...
enum CBv2RaggedTwoPassDecodeAttentionV1 {
    ...
    private static let passBKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_sdpa_2pass_b_direct_bf16_d256_b\(blocks)_v4",
        inputNames: ["partials", "sums", "maxs"],
        outputNames: ["out"],
        source: """
            constexpr int simd_width = 32;
            constexpr int values_per_lane = D / simd_width;
            constexpr int n_block = BLOCKS / simd_width;
...
enum CBv2RaggedComposedD512DecodeAttentionV1 {
    ...
    /// params: [0]=kL, [1]=K (host still packs headDim for ABI; the kernel
    /// ignores it and uses constexpr D, which is 512 and divides 128),
    ...
    private static let fusedQkKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_writesdpa_d512_qk_bf16_g8_v4",
        inputNames: [
            "queries",
            "k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7",
            "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7",
