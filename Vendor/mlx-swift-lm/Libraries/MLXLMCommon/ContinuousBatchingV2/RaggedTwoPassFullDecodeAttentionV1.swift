// Batch-wide two-pass SDPA for Gemma 4 full decode (B=8, D=512, GQA=8, runtime N).
...
enum CBv2RaggedTwoPassFullDecodeAttentionV1 {
    ...
    private static let passAKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_full8_sdpa_2pass_a_bf16_d512_g8_b\(blocks)_v1",
        inputNames: [
            "queries",
    ...
    private static let passBKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_full8_sdpa_2pass_b_bf16_d512_b\(blocks)_v1",
    ...
    static func attend(
        ...
        guard sequenceLength > 0 else { return nil }
        ...
        for index in 0 ..< batch {
            ...
            guard key.dtype == .bfloat16,
                ...
                key.dim(1) == kvHeads,
                value.dim(1) == kvHeads,
                key.dim(2) == sequenceLength,
                value.dim(2) == sequenceLength,
                key.dim(3) == headDim,
                value.dim(3) == headDim
            else { return nil }
        }
