...
// decode shape: B=8, 16 query heads, 2 KV heads, D=512, GQA=8. Stock MLX
// SDPA refuses D=512 in both the vector (L<=8) and steel (L>8) fused
// kernels, so the five full layers still run eight row-local unfused
// graphs. This path keeps the same online-softmax visit order as the
// sliding two-pass kernel and only changes D, GQA, and the per-row length
// / capacity used to address physical K/V. Ineligible shapes return nil.

import Foundation
import MLX
import MLXFast

public enum CBv2RaggedTwoPassFullDecodeAttentionV1 {
    private static let enabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment[
            "DARKBLOOM_CBV2_FULL_TWO_PASS_ATTENTION"]
        else { return true }
    ...
    /// Same partition as `sdpa_vector_2pass` / the sliding two-pass kernel.
    ...
    private static let passAKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_full_sdpa_2pass_a_bf16_d512_g8_b\(blocks)_v1",
        inputNames: [
            "queries",
        ...
        source: """
            ...
            const int N = int(seq_lens[batch_index]);
            const int cap = int(capacities[batch_index]);
            ...
            keys += kv_head * cap * D + lane * values_per_lane;
            values += kv_head * cap * D + lane * values_per_lane;
    ...
    private static let passBKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_ragged8_full_sdpa_2pass_b_bf16_d512_b\(blocks)_v1",
    ...
    /// Decode attention for eight full-attention rows. `keys[i]` / `values[i]`
    /// must be row-contiguous `[1, 2, capacity, 512]` bf16. Only the first
    /// `lengths[i]` tokens are visited. Returns nil for any ineligible shape
    /// so the caller keeps the established per-row SDPA graph.
    public static func attend(
        ...
        for index in 0 ..< batch {
            ...
            guard length >= blocks,
                ...
                key.shape == [1, kvHeads, capacity, headDim],
                value.shape == key.shape
            else { return nil }
        }

        let lengthArray = MLXArray(lengths.map(UInt32.init))
        let capacityArray = MLXArray(capacities.map(UInt32.init))
        let inputs = [queries] + keys + values + [lengthArray, capacityArray]
