    ...
    CBv2InnerStateProviding
{
    ...
    /// precondition is what keeps the mirror honest: the caller only reaches
    /// here after `cbv2QuantValuePlane` handed it a caught-up mirror.
    public func advanceAfterFusedAppendWithMirror() {
        precondition(
            valueMirror != nil && mirrorValidCount == absoluteOffset,
            "CBv2FullSequenceKV: fused mirror advance without a caught-up mirror")
