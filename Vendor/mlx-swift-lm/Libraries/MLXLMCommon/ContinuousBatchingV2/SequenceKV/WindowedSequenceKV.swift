    ...
    CBv2InnerStateProviding
{
    ...
    /// KVQ-PAIR kill switch: `MLX_KV_QUANT_FUSE_PAIR=0` keeps the fused
    /// quantized pass A on its per-head (g2) form. Default ON.
    static let quantFusePairEnabled: Bool = {
        guard let raw = ProcessInfo.processInfo.environment["MLX_KV_QUANT_FUSE_PAIR"]
        else { return true }
        return !["0", "false", "no", "off"].contains(raw.lowercased())
    }()
