    ...
    CBv2InnerStateProviding
{
    ...
    private static let quantPackPairRowsKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_kvq4g64_pack_pair_rows_d256_vec4reuse_v2",
        inputNames: ["keys", "values"],
        outputNames: ["packed_w"],
        source: """
            constexpr int D = 256;
            ...
            // One lane owns eight consecutive BF16 elements. Load them once
            // as two aligned four-wide vectors and retain the converted values
            // for both the extrema pass and the quantization pass.
            typedef vec<T, 4> T4;
            const device T4* src4 =
