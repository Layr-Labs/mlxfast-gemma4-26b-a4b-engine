...
public final class CBv2WindowedSequenceKV: CBv2SequenceKV, CBv2InnerStateProviding {
    ...
    ///
    /// Layout: `[1, kvHeads, window, headDim + 64]` uint8. Per (head, slot) the
    /// first `headDim` bytes are affine-quantized keys and the trailing 64
    /// bytes hold sixteen fp16 scales and sixteen fp16 biases (group size 16).
    ///
    ...
    /// `[kvHeads, n, headDim + 64]` uint8. The key payload is followed by
    /// sixteen fp16 scales and sixteen fp16 biases, one affine pair per 16 values.
    private static func quantPack(_ x: MLXArray) -> MLXArray {
        let f = x[0].asType(.float32)
        let grouped = f.reshaped([-1, 16, 16])
        let (scale, bias) = quantParams(grouped)
        let q = clip(round((grouped - bias) / scale), min: 0, max: 255)
            .asType(.uint8).reshaped(f.shape)
        let metadataShape = [f.dim(0), f.dim(1), 32]
    ...
    private func allocateIfNeeded(keyTemplate: MLXArray, valueTemplate: MLXArray) {
            ...
            keyTemplate.dim(3) == headDim, valueTemplate.dim(3) == headDim
        {
            quantMirror = MLXArray.zeros(
                [1, kvHeads, window, headDim + 64], dtype: .uint8)
        }
    }
}
