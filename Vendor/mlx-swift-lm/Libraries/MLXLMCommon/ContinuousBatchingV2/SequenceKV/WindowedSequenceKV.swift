    ...
    CBv2InnerStateProviding
{
    ...
    private static let quantPackKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        source: """
            ...
            // Each lane owns 8 CONTIGUOUS elements: read them as two aligned
            // four-wide vectors and fully unroll. min/max are associative and
            // commutative, so the reduction value is exact; the quantized
            // nibble loop keeps its original shift order under full unroll.
            typedef vec<T, 4> T4;
            const device T4* xq = reinterpret_cast<const device T4*>(xr + lane * per_lane);
            const T4 lo4 = xq[0];
            const T4 hi4 = xq[1];
            float e[per_lane];
    ...
    private static let quantPackPairKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        source: """
            ...
            typedef vec<T, 4> T4;
            const device T4* sq = reinterpret_cast<const device T4*>(src + lane * per_lane);
            const T4 lo4 = sq[0];
            const T4 hi4 = sq[1];
            float e[per_lane];
            #pragma clang loop unroll(full)
