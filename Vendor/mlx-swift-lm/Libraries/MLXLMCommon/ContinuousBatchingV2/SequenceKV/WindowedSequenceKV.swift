    ...
    CBv2InnerStateProviding
{
    ...
    private static let quantPackPairKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_kvq4g64_pack_pair_d256_batch_v2",
        ...
        source: """
            ...
            const int plane = local_row / HEADS;
            const int head = local_row % HEADS;
            const int source_row = batch_index * HEADS + head;
            const int lane = int(thread_position_in_threadgroup.x);
            const device T* src =
                (plane == 0 ? keys : values) + source_row * D;
...
private static let diagBF16WriteKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
    ...
    /// Batch all eight fixed-cohort decode rows into one paired-pack dispatch.
    /// Nonmatching shapes keep the promoted per-row path unchanged.
    ...
    private func writePairedMirror(
        ...
        if let prepacked {
            guard prepacked.dtype == .uint32,
                prepacked.shape == [2, kvHeads, 1, rowWords]
            else { return false }
            packed = prepacked
        } else {
