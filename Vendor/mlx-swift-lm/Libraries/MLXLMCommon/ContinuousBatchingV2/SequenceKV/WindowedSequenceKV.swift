    ...
    CBv2InnerStateProviding
{
    ...
    /// PREFILL-KV-BATCH: batched `[B, H, N, D]` keys and values -> B mirror
    /// allocations `[2, H, N, D/8 + D/64]` uint32, ONE dispatch for every
    /// row's both planes. The x index carries a row prefix on top of the
    /// per-row chunk kernel's decomposition
    /// (`row = b * (2*HEADS*N) + plane*(HEADS*N) + head*N + token`), so each
    /// threadgroup performs the SAME per-(plane, head, token) arithmetic on
    /// the SAME input bytes and writes the SAME words at the SAME offsets
    /// inside its own row's mirror — only the grid decomposition and the
    /// output selection differ. Each row's mirror is its OWN output
    /// allocation, never a view of one shared buffer: the decode-step
    /// mirror SliceUpdates and the fused kernels' in-place mirror-slot
    /// writes keep operating on exactly the per-row allocation the per-row
    /// pack produces. `1 <= B <= 8` (the kernel binds one output buffer per
    /// row); anything larger stays on the per-row dispatches.
    static func quantPackPairChunkBatchGPU(keys: MLXArray, values: MLXArray) -> [MLXArray] {
        let rows = keys.dim(0)
        let heads = keys.dim(1)
        let n = keys.dim(2)
        let headDim = keys.dim(3)
        precondition(rows >= 1 && rows <= 8, "quantPackPairChunkBatchGPU: batch must be 1...8")
        let words = headDim / 8 + headDim / 64
        let outputs = quantPackPairChunkBatchKernel(
            [keys, values],
            template: [("T", keys.dtype), ("HEADS", heads), ("N", n)],
            grid: (rows * 2 * heads * n * 32, 1, 1),
            threadGroup: (32, 1, 1),
            outputShapes: (0 ..< 8).map { $0 < rows ? [2, heads, n, words] : [1] },
            outputDTypes: Array(repeating: DType.uint32, count: 8)
        )
        return Array(outputs[..<rows])
    }

    /// The per-row arithmetic is a transcription of
    /// `quantPackPairChunkKernel`, kept separate for the same reasons the
    /// pair kernels are: a Swift-hosted kernel that changes text without
    /// changing name serves a stale body. Only the index block differs — a
    /// row prefix on the x index selects the input row and one of the eight
    ...
    private static let quantPackPairChunkBatchKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        ...
        source: """
            ...
            const int batch_index = row / (2 * plane_stride);
            const int rem = row - batch_index * (2 * plane_stride);
            const int plane = rem / plane_stride;
            const int local = rem - plane * plane_stride; // head * N + token
            const int lane = int(thread_position_in_threadgroup.x);
            const device T* src =
                (plane == 0 ? keys : values) + (batch_index * plane_stride + local) * D;
            device uint32_t* out;
            switch (batch_index) {
                case 0: out = m0; break;
                case 1: out = m1; break;
...
private static let diagBF16WriteKernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
    ...
    /// the given `[1, kvHeads, n, headDim]` chunk would take
    /// FRESH-RING-ADOPT — `update`'s own gate plus the guards that precede
    /// it there (no open speculative transaction, no stale BF16 ring). A
    /// batch of rows that all pass here would each adopt individually on
    /// the per-row commit loop.
    func canAdoptFreshFullWindowChunk(keys newKeys: MLXArray, values newValues: MLXArray) -> Bool {
        !speculativeWriteArmed && !bf16RingStale && keys == nil
    ...
    private func adoptFreshFullWindowChunk(
        ...
        return adoptFreshFullWindowChunk(
            keys: newKeys, values: newValues,
            packedMirror: Self.quantPackPairChunkGPU(keys: newKeys, values: newValues))
    }

    /// PREFILL-KV-BATCH adopt: the transitions of the private adopt above
    /// with the mirror the CALLER packed from these same chunk bytes (the
    /// batched packer, one dispatch for the whole batch). The stored mirror
    /// differs from the per-row pack output in nothing but which dispatch
    /// wrote it; every state transition is the private adopt's, statement
    /// for statement.
