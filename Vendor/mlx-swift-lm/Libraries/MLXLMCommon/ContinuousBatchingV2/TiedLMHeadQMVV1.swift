// LMH-001v3: one-group dispatch for the tied lm_head ordinary QMV at batch
// eight, extending the promoted low-residency quad stream to all eight rows.
//
...
//
// The v2 custom path reduced that grid to the two useful groups and streamed
// each weight block across four rows. This v3 path keeps the same one-row-live
// residency discipline and exact per-output accumulation order, but walks all
// eight cohort rows through the loaded weight block before advancing K. The
// tied head therefore reads each weight block once instead of twice and uses
// one x-group. Each row retains a separate accumulator and `simd_sum`.
//
...
// saving back for the slower body. The header below keeps the promoted
// `load_vector`, `load_vector_safe`, and `qdot_affine4_registered` helpers
// verbatim. The stream body retains the v2 statement order for rows 0-3 and
// repeats that exact sequence for rows 4-7 before advancing the shared weight
// block. No dot-product expression, reduction, or BF16 rounding point changes;
// only the number of independent row accumulators fed by each loaded block
...
METAL_FUNC void qmv_affine4_g64_octa_stream_impl(
...
"""
    ...
    private static let kernel: MLXFast.MLXFastKernel = MLXFast.metalKernel(
        name: "cbv2_b8_tied_lmhead_qmv_affine4_g64_octa_stream_v3",
        ...
        source: """
            ...
            qmv_affine4_g64_octa_stream_impl<T, 64, 4>(
                w,
                scales,
                biases,
