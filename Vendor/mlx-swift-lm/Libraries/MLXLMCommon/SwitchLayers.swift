...
private let routeSimdRank64Kernel: MLXFast.MLXFastKernel =
    MLXFast.metalKernel(
        ...
        source: """
            ...
            // byte. Carry the already-known stable run offset and total count
            // in otherwise unused bits for assignments covered by a pair;
            ...
            const uint packed_key = PACK_RUN_OFFSETS && run_offset < paired_count
                ? (0x80000000u | (run_count << 24) | (run_offset << 8) | key)
                : key;
