...
/// via the `numExperts` guard on `gatherSortIndices`. DEFAULT ON
/// (`DARKBLOOM_ROUTE_COUNTING_SORT=0` restores the stock argsort chain), as
/// the donor tree also ships it. The kernel is INTEGER-ONLY — predecessor
/// counting over uint32 keys with commutative atomics — so it moves no
/// floating-point operation and law L8's box-recompile presumption (which
/// governs new FLOAT paths) does not reach it: there is no arithmetic for
/// another Metal compiler to reassociate. Parity 10,500/10,500 bit-exact on
/// all three outputs incl. all-equal / boundary / reversed key suites.
/// Engage mark: `route-csort64`.
