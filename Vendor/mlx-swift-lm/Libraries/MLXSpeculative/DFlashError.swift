...
public enum DFlashError: LocalizedError, Sendable, Equatable {
    ...
    case undecodableQuantization(String)
    case unsupportedQuantization(field: String, reason: String)
    case quantizedWeightsWithoutDeclaration(String)
    case declaredQuantizationWithoutQuantizedWeights
    ...
    public var errorDescription: String? {
        ...
        case .undecodableQuantization(let reason):
            return
                "DFlash draft config.json declares a quantization block that could not be "
                + "decoded: \(reason)."
        case .unsupportedQuantization(let field, let reason):
            return
                "DFlash draft config.json declares an unsupported quantization: \(field) "
                + "\(reason)."
        case .quantizedWeightsWithoutDeclaration(let path):
            return
                "DFlash draft weights are quantized (\(path).scales is present) but "
                + "config.json declares no quantization block; refusing rather than binding "
                + "the drafter at full precision."
        case .declaredQuantizationWithoutQuantizedWeights:
            return
                "DFlash draft config.json declares a quantization block but no draft weight "
                + "carries a .scales tensor; refusing rather than silently loading an "
                + "unquantized drafter."
        }
    }
}
