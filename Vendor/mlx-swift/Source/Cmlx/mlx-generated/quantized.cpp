...
METAL_FUNC void qmv_affine4_g64_quad_stream_impl(
  ...
  // Every caller reaching this specialized g64 tier has K % 64 == 0 (the
  // dispatch gate checks it), so the final block always holds a whole number
  // of complete eight-value lane packets -- K = 2816 leaves all 32 lanes
  // active. An active lane therefore never needs the generic dynamic
  // safe-tail loops; the fixed unrolled load preserves each lane's qdot and
  // simd_sum order exactly. Same aligned-tail argument as the pair and
  // affine-8 stream tails above.
  const uint active_tail_lanes =
      uint((in_vec_size - k) / values_per_thread);
  if (simd_lid < active_tail_lanes) {
    for (int row = 0; row < results_per_simdgroup; row++) {
