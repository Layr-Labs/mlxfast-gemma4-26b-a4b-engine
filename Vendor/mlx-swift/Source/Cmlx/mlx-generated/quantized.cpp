...
// The 16-byte path exists only for 16-bit T (the pinned bf16 decode cell);
// any other T keeps the stock scalar loader, so every ahead-of-time
// instantiation of this header (float included) compiles and stays exact.
template <typename T, bool is16 = (sizeof(T) == 2)>
struct qmv_x8_loader {
  static inline float load(const device T* x, thread float* x_thread) {
    return load_vector<T, float, 8, 4>(x, x_thread);
  }
};

template <typename T>
struct qmv_x8_loader<T, true> {
  static inline float load(const device T* x, thread float* x_thread) {
...
inline float qmv_load_x8_vec(const device T* x, thread float* x_thread) {
  return qmv_x8_loader<T>::load(x, x_thread);
}
