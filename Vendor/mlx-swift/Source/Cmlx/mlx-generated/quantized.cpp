...
// COHORT-RUN-LOOKBACK window. Both Gemma 4 gather-QMV run elections resolve
// an assignment's distance from its run start by walking backwards one
// assignment at a time, and every step's compare gates the next device load,
// so the whole chain serializes at kernel entry ahead of any weight traffic.
// Every leader threadgroup of every routed plane pays it.
//
// The ranked decode cohort is eight rows by eight routed experts per row, so
// batch_shape[0] == 64 assignments hold each expert at most eight times and a
// run offset is always below eight. Issue that whole look-back window as
// INDEPENDENT loads and fold it with ALU work instead. `matched` is monotone,
// so the fold counts exactly the leading same-expert prefix the walk counts.
// A window that is entirely same-expert -- unreachable at this geometry, and
// only reachable at all if a cohort shape ever admits a longer run -- falls
// through to the incumbent walk from the window's far edge, so the resolved
// offset is identical for every input and the elections downstream are
// unchanged: loads-only rescheduling, no arithmetic and no store moves.
//
// The window is a compile-time flip. Zero restores the incumbent walk
// verbatim: the fold contributes nothing and the fall-through walk starts at
// `assignment` exactly as before.
constexpr int gemma4_run_lookback_window = 7; // 0 restores the plain walk
