...
METAL_FUNC void qmv_affine8_g64_pair_impl(
  ...
  // Aligned tail, as in the sibling g64 specializations. The `affine_qmv`
  // byte tier only reaches here under `in_vec_size % 64 == 0`, so the final
  // block is a whole number of complete four-value lane packets and the
  // dynamic safe-tail loop can be replaced by the fixed unrolled load.
  const uint active_tail_lanes =
      uint((in_vec_size - k) / values_per_thread);
  if (simd_lid < active_tail_lanes) {
    float sum0 = load_vector<T, float, values_per_thread, 8>(x0, x0_thread);
