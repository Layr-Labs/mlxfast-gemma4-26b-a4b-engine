...
// Four independent affine-4 dot products over one packed weight vector. Keep
// each scalar expression tree identical to qdot while sharing only immutable
// packed-weight, scale, and bias reads across the four activation rows.
template <typename U, int values_per_thread>
...
// Exact four-row specialization for the ruled ordinary affine4/g64 B == 8
// QMV path. Two active x-groups each own four activation rows. Every lane reads
// each immutable packed weight, scale, and bias once, then advances four
// independent load/qdot/K-accumulation/simd-reduction chains in stock order.
template <typename T>
...
[[kernel]] void affine_qmv(
  ...
  if (!batched && group_size == 64 && bits == 4 && ntg.x == 8 &&
    ...
    // Claim four adjacent rows in two active x-groups and let the remaining
    // host groups return. The isolated quad helper shares immutable weights,
    // scales, and biases while preserving every row's arithmetic order.
    const int first_m = int(tid.x) * 4;
    if (first_m >= 8) {
      return;
    }
    qmv_affine4_g64_quad_impl<T>(
        w,
        scales,
        biases,
