...
// Two independent affine-8 dot products over one byte weight vector ALREADY
// held in registers: the caller performs one aligned 4-byte load and unpacks
// it, so each byte is exactly the byte the device loop read (little endian).
// Keep the per-row scalar accumulation order of qdot while sharing each
// weight load. Only the residence of `w` differs (thread instead of device).
template <typename U, int values_per_thread>
...
METAL_FUNC void qmv_affine8_g64_pair_impl(
  ...
  for (; k <= in_vec_size - block_size; k += block_size) {
    ...
    for (int row = 0; row < results_per_simdgroup; row++) {
      ...
      thread uint8_t wb[4];
      wb[0] = uint8_t(wv & 0xffu);
      wb[1] = uint8_t((wv >> 8) & 0xffu);
      wb[2] = uint8_t((wv >> 16) & 0xffu);
      wb[3] = uint8_t(wv >> 24);
      const device T* sl = scales + row * in_vec_size_g;
      const device T* bl = biases + row * in_vec_size_g;
      float dot0;
      ...
      qdot_affine8_pair<float, values_per_thread>(
          wb, x0_thread, x1_thread, sl[0], bl[0], sum0, sum1, dot0, dot1);
      result0[row] += dot0;
      result1[row] += dot1;
    }
...
METAL_FUNC void qmv_affine8_g64_quad_stream_impl(
  ...
  for (; k <= in_vec_size - block_size; k += block_size) {
    for (int row = 0; row < results_per_simdgroup; row++) {
      // One aligned 4-byte load, unpacked into the incumbent byte cache: this
      // tier only dispatches with in_vec_size % 64 == 0, so the row stride,
