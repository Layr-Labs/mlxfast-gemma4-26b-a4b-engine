...
METAL_FUNC void gather_qmv_gemma4_down_tile(
  ...
  // Leaders at run_offset % 4 == 0 serve up to four same-expert
  // assignments from one down-plane weight stream.
  if ((run_offset & 3) != 0) {
    return;
  }
  uint run_len = 1;
  ...
  if (run_len > 1) {
    ...
    if (run_len == 3) {
      for (int t = 0; t < gemma4_down_tile_span; t++) {
        ...
        qmv_affine4_g64_triple_stream_impl<T, group_size, bits>(
            ...
            tile_y0, tile_y1, tile_y2, in_vec_size, tile_tid, simd_gid, simd_lid);
      }
      return;
    }
    ...
    for (int t = 0; t < gemma4_down_tile_span; t++) {
      ...
      qmv_affine4_g64_quad_stream_impl<T, group_size, bits>(
          tile_w, tile_scales, tile_biases, tile_x0, tile_x1, tile_x2, tile_x3,
          tile_y0, tile_y1, tile_y2, tile_y3, in_vec_size, tile_tid, simd_gid, simd_lid);
    }
    return;
  }
  for (int t = 0; t < gemma4_down_tile_span; t++) {
