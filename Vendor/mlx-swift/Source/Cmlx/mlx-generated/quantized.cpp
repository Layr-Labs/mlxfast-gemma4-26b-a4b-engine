...
METAL_FUNC void qmv_affine4_g64_pair_tilewalk_impl(
  ...
  for (int t = 0; t < SPAN; t++) {
    ...
    for (int row = 0; row < results_per_simdgroup; row++) {
      const float r0 = simd_sum(result0[t][row]);
      const float r1 = simd_sum(result1[t][row]);
      if (simd_lid == 0) {
...
METAL_FUNC void qmv_affine4_g64_single_tilewalk_impl(
  ...
  for (int t = 0; t < SPAN; t++) {
    ...
    for (int row = 0; row < results_per_simdgroup; row++) {
      const float r = simd_sum(result[t][row]);
      if (simd_lid == 0) {
...
// GATHER-INTRA gate.  Compile-time flip, ON here.  `false` restores the
// incumbent tile-call walk and the runtime-K leader calls; the only residue
// is `qmv_affine4_g64_pair_impl` taking `in_vec_size` BY VALUE instead of by
// `constant` reference, which was screened byte-identical on both
// architectures (canonical-name inner-Mach-O `__text` diff, negative control
// clean).
constant constexpr bool gemma4_gather_intra = true;
...
[[kernel]] void affine_gather_qmv(
  ...
  if (gemma4_pair_geometry) {
    ...
    if (run_len > 1) {
      ...
      const int run_in_vec_size =
          (gemma4_gather_intra && gemma4_down_tile) ? 2816 : in_vec_size;
