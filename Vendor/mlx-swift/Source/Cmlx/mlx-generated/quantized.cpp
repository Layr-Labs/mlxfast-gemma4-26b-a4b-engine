...
// The span is 2 here. The preamble (run_offset scan, route_word load,
// has_pair test, five base pointers) is per-survivor, so a wider span
// amortizes more of it -- but a ranked draw at span 8 measured decode
// 2.64240757805 against 2.64998390190 for the same tree at span 4, so
// this gather is bound by resident group count, not by the preamble.
// Span 2 goes the other way: 176 survivors per assignment instead of 88.
template <typename T, int group_size, int bits>
METAL_FUNC void gather_qmv_gemma4_down_tile(
    ...
    uint simd_gid,
    uint simd_lid) {
  constexpr int gemma4_down_tile_span = 2; // ranked: span 8 = 2.64240757805, span 4 = 2.64998390190
