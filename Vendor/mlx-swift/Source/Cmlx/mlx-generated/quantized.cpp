...
METAL_FUNC void qmv_affine4_g64_pair_impl(
  ...
  // NARROW: these three arrays stay live across both x loads and the whole
  // row loop, and each held a widened copy of a 16-bit source. Keeping the
  // source width halves their footprint; the widening moves to the point of
  // use, where exactly one row is live.
  thread ushort packed_lo[results_per_simdgroup];
  thread ushort packed_hi[results_per_simdgroup];
  ...
  for (; k <= in_vec_size - block_size; k += block_size) {
    for (int row = 0; row < results_per_simdgroup; row++) {
      const device ushort* wl =
          (const device ushort*)(ws + row * in_vec_size_w);
      packed_lo[row] = wl[0];
      packed_hi[row] = wl[1];
    ...
    for (int row = 0; row < results_per_simdgroup; row++) {
      ...
      qdot_affine4_pair_word<float, values_per_thread>(
          (uint(packed_hi[row]) << 16) | uint(packed_lo[row]),
          x0_thread, x1_thread, float(scale_local[row]),
          float(bias_local[row]), sum0, sum1, dot0, dot1);
      result0[row] += dot0;
      result1[row] += dot1;
    }
