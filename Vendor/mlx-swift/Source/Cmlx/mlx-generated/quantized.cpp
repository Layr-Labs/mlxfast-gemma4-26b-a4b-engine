...
[[kernel]] void affine_gather_qmv(
  ...
  if (gemma4_pair_geometry) {
    ...
    //
    // down_proj only. gate_proj and up_proj have K = 2816, which is eleven
    // whole 256-value blocks, so the promoted `<=` K-loop bound already runs
    // them with no partial-tail block at all and the wide block measures
    // inside noise on them. down_proj has K = 704, which leaves a genuine
    // 192-value remainder that the narrow body pays for through
    // `load_vector_safe` and `qdot_safe` on every call, and that is where the
    // two-pack block earns its keep. Leaving gate and up on the promoted path
    // keeps two thirds of the expert weight traffic bit-for-bit unchanged.
    if (in_vec_size == 704) {
