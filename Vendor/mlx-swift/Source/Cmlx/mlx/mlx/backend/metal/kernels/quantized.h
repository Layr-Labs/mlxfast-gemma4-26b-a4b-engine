...
METAL_FUNC void qmv_affine4_g64_quad_stream_impl(
  ...
  // eight-value lane packets.  In particular K=2816 leaves exactly 32 active
  // lanes; use the fixed unrolled load instead of four dynamic safe-tail
  // loops while preserving each lane's qdot and simd_sum order.
  const uint active_tail_lanes =
      uint((in_vec_size - k) / values_per_thread);
  if (simd_lid < active_tail_lanes) {
    for (int row = 0; row < results_per_simdgroup; row++) {
