...
// SB-INTERLEAVE (C3). `sbint` selects the INTERLEAVED placement of a routed
// expert's affine scale/bias metadata:
//
//   incumbent   scale = scales[row * G + g]   in one bfloat16 plane
//               bias  = biases[row * G + g]   in a SECOND bfloat16 plane
//   interleaved scale = sb[(row * G + g) * 2]     one plane, [N, K/64, 2]
//               bias  = sb[(row * G + g) * 2 + 1]
//
// Same bytes, same values, same count: the two planes are merged pairwise, so
// a lane's scale and its bias are ADJACENT and load as ONE `vec<T, 2>` (4 B at
// bfloat16) instead of two 2 B loads from two separate address streams. The
// weight-side load count per simdgroup-block goes 12 -> 8 and the hot weight
// streams 3 -> 2.
//
// The three strides that change are the only three that mention scales or
// biases: the per-output-row stride (G -> 2G), the lane's group offset inside
// a block (x2) and the K-block advance (x2). Nothing about the codes, the
// per-(output, input) qdot terms, the accumulators, the K-loop order or the
// simd_sum moves -- the SAME scale and the SAME bias reach the SAME
// `x * scale + bias` in the SAME iteration, so every output element keeps its
// incumbent add sequence. `sbint == false` reproduces the incumbent
// addressing byte for byte.
//
// The host never flips this by itself: the interleaved carrier is bound to
// BOTH the scales and the biases buffer, so its per-expert stride is exactly
// twice the split planes', and `affine_gather_qmv` reads that off `s_strides`
// and `b_strides` at dispatch. Layout and decode therefore cannot drift.
template <typename T, int group_size, int bits, bool laneblock, bool sbint>
METAL_FUNC void qmv_impl(
  ...
  // even 1 tile in the matrix. Neither placement ever enters here.
  // DOWN-LANEBLOCK's only plane is out_vec_size = 2816 and SB-INTERLEAVE's
  // planes are out_vec_size 704 (gate/up) and 2816 (down), so the condition
  // is already false for both; the `!laneblock && !sbint` makes that
  // structural instead of incidental, which keeps this arm's incumbent
  // (row-major codes, split metadata) addressing unreachable from a relaid
  // plane or an interleaved carrier.
  if (!laneblock && !sbint &&
      out_vec_size < (num_simdgroups * results_per_simdgroup)) {
    ws +=
  ...
  // In this case the last tile is moved back to redo some output values
    ...
    // SB-INTERLEAVE addressing, three numbers -- all of them the same
    // doubling, because one (scale, bias) pair occupies two elements of the
    // interleaved carrier where the split planes hold one:
    //   sb_pair        elements per quantization group   1 -> 2
    //   sb_row_stride  distance to the next output row   G -> 2G
    //   sb_block_step  distance to the next K-block      B -> 2B
    // Under `!sbint` all three collapse to the incumbent constants.
    constexpr int sb_pair = sbint ? 2 : 1;
    const int sb_row_stride = in_vec_size_g * sb_pair;
    const int sb_block_step = (block_size / group_size) * sb_pair;
    ws += used_out_row * in_vec_size_w + simd_lid * w_lane_step;
    scales += used_out_row * sb_row_stride +
        (simd_lid / scale_step_per_thread) * sb_pair;
    biases += used_out_row * sb_row_stride +
        (simd_lid / scale_step_per_thread) * sb_pair;
    x += tid.x * in_vec_size + simd_lid * values_per_thread;
    ...
    for (; k <= in_vec_size - block_size; k += block_size) {
      ...
      for (int row = 0; row < results_per_simdgroup; row++) {
        ...
        const device T* sl = scales + row * sb_row_stride;

        U s;
        U b;
        if (sbint) {
          const vec<T, 2> sb = *((const device vec<T, 2>*)sl);
          s = sb.x;
          b = sb.y;
        } else {
          s = sl[0];
          b = biases[row * sb_row_stride];
        }
        result[row] +=
            qdot<U, values_per_thread, bits>(wl, x_thread, s, b, sum);
      }
      ...
      ws += w_block_step;
      scales += sb_block_step;
      biases += sb_block_step;
      x += block_size;
    }
...
// `sbint` has the meaning documented on `qmv_impl`: the interleaved
// [N, K/64, 2] scale/bias carrier. The two are orthogonal -- one moves
// codes, the other moves affine metadata -- and combine by taking the
// `uint4` code load from `laneblock` and the `vec<T, 2>` metadata load
// from `sbint` in the same iteration.
template <typename T, const int group_size, const int bits, bool laneblock, bool sbint>
METAL_FUNC void qmv_affine4_g64_pair_impl(
  ...
  const int in_vec_size_w = in_vec_size / 2;
  const int in_vec_size_g = in_vec_size / 64;
  // SB-INTERLEAVE: one (scale, bias) pair occupies two elements of the
  // interleaved carrier where the split planes hold one, so every
  // scale/bias stride doubles and nothing else moves.
  constexpr int sb_pair = sbint ? 2 : 1;
  const int sb_row_stride = in_vec_size_g * sb_pair;
  const int sb_block_step = (block_size / 64) * sb_pair;
  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +
      simd_gid * results_per_simdgroup;
  ...
  scales += out_row * sb_row_stride +
      (simd_lid / scale_step_per_thread) * sb_pair;
  biases += out_row * sb_row_stride +
      (simd_lid / scale_step_per_thread) * sb_pair;
  ...
  for (; k <= in_vec_size - block_size; k += block_size) {
    if (laneblock) {
      ...
      for (int row = 0; row < results_per_simdgroup; row++) {
        if (sbint) {
          const vec<T, 2> sb =
              *((const device vec<T, 2>*)(scales + row * sb_row_stride));
          scale_local[row] = sb.x;
          bias_local[row] = sb.y;
        } else {
          scale_local[row] = scales[row * sb_row_stride];
          bias_local[row] = biases[row * sb_row_stride];
        }
      }
    } else {
      for (int row = 0; row < results_per_simdgroup; row++) {
...
// stock qmv_impl.
template <typename T, const int group_size, const int bits, bool sbint>
...
//
// `sbint` has the meaning documented on `qmv_impl` and carried by
// `qmv_affine4_g64_pair_impl`: the interleaved [N, K/64, 2] scale/bias
// carrier. RUN-TRIPLE hands this arm the SAME metadata the pair arm gets,
// so it takes the SAME `sbint` argument from the SAME stride test.
template <typename T, const int group_size, const int bits, bool laneblock, bool sbint>
METAL_FUNC void qmv_affine4_g64_triple_stream_impl(
  ...
  for (; k <= in_vec_size - block_size; k += block_size) {
    if (laneblock) {
      ...
      for (int row = 0; row < results_per_simdgroup; row++) {
        packed[row] =
            *((const device uint*)(ws + row * w_row_stride));
        if (sbint) {
          const vec<T, 2> sb =
              *((const device vec<T, 2>*)(scales + row * sb_row_stride));
          scale_local[row] = sb.x;
...
[[kernel]] void affine_qmv(
  ...
  if (!batched && group_size == 64 && bits == 4 && ntg.x == 8 &&
    ...
    qmv_affine4_g64_pair_impl<T, 64, 4, false, false>(
        w,
        scales,
        biases,
        x + first_m * in_vec_size,
  ...
  qmv_impl<T, group_size, bits, false, false>(
      w,
      scales,
      biases,
      x,
...
METAL_FUNC void qmv_affine4_g64_singles_impl(
  ...
  // SB-INTERLEAVE: see qmv_impl. Only the scale/bias strides double.
  constexpr int sb_pair = SBINT ? 2 : 1;
  const int sb_row_stride = in_vec_size_g * sb_pair;
  const int sb_block_step = (block_size / qgroup) * sb_pair;
  const int out_row = tid.y * (num_simdgroups * results_per_simdgroup) +
      simd_gid * results_per_simdgroup;
  const int used_out_row = min(out_vec_size - results_per_simdgroup, out_row);
  if (out_row >= out_vec_size) {
...
// SB-INTERLEAVE (C3): `sbint` is forwarded verbatim to both arms this
// strip-walk elects between, so a tile served by the pair impl and a tile
// served by stock qmv_impl decode the SAME carrier the same way.
template <typename T, int group_size, int bits, bool sbint>
METAL_FUNC void gather_qmv_gemma4_down_tile(
  ...
  if (run_len >= 2u) {
    ...
    if (run_len == 3u) {
      ...
      for (int t = 0; t < gemma4_down_tile_span; t++) {
        ...
        if (laneblock) {
          qmv_affine4_g64_triple_stream_impl<T, group_size, bits, true, sbint>(
              tile_w,
              tile_scales,
              tile_biases,
              tile_x0,
          ...
          qmv_affine4_g64_triple_stream_impl<T, group_size, bits, false, sbint>(
              tile_w,
              tile_scales,
              tile_biases,
              tile_x0,
    ...
    for (int t = 0; t < gemma4_down_tile_span; t++) {
      ...
      tile_tid.y = tid.y + uint(t);
      if (laneblock) {
        qmv_affine4_g64_pair_impl<T, group_size, bits, true, sbint>(
            tile_w,
            tile_scales,
            tile_biases,
            tile_x0,
        ...
        qmv_affine4_g64_pair_impl<T, group_size, bits, false, sbint>(
            tile_w,
            tile_scales,
            tile_biases,
            tile_x0,
  ...
  for (int t = 0; t < gemma4_down_tile_span; t++) {
    ...
    tile_tid.y = tid.y + uint(t);
    if (laneblock) {
      qmv_impl<T, group_size, bits, true, sbint>(
          tile_w,
          tile_scales,
          tile_biases,
          tile_x0,
      ...
      qmv_impl<T, group_size, bits, false, sbint>(
          tile_w,
          tile_scales,
          tile_biases,
          tile_x0,
...
[[kernel]] void affine_gather_qmv(
  ...
  // SB-INTERLEAVE (C3) admission. The host binds the interleaved
  // [N, K/64, 2] carrier to BOTH the scales and the biases buffer, so its
  // per-expert ELEMENT stride is exactly twice the split planes'
  // out_vec_size * (in_vec_size / group_size). Under the gemma4 gate the
  // incumbent scales array is [128, N, K/64] contiguous and its stride is
  // that product exactly, so nothing but the carrier can present the
  // doubled stride. Reading the layout off the buffer's own metadata --
  // rather than off a flag smuggled through an index word -- is what makes
  // the host's kill switch total: no interleaved plane bound, no doubled
  // stride, no interleaved arm, and no read site anywhere in this kernel
  // needs to mask anything back out.
  const int64_t sb_split_expert_stride =
      (int64_t)out_vec_size * (int64_t)(in_vec_size / group_size);
  const bool gemma4_sb_interleaved = gemma4_pair_geometry &&
      s_strides[0] == 2 * sb_split_expert_stride &&
      b_strides[0] == s_strides[0];
  if (gemma4_pair_geometry) {
    ...
    if (gemma4_down_tile && in_vec_size == 704) {
      if (gemma4_sb_interleaved) {
        gather_qmv_gemma4_down_tile<T, group_size, bits, true>(
            w,
            scales,
            biases,
            x,
        ...
        gather_qmv_gemma4_down_tile<T, group_size, bits, false>(
            w,
            scales,
            biases,
            x,
    ...
    if (run_len > 1) {
      ...
      if (run_len == 2) {
        if (gemma4_sb_interleaved) {
          qmv_affine4_g64_pair_impl<T, group_size, bits, false, true>(
              run_w,
              run_scales,
              run_biases,
              run_x0,
          ...
          qmv_affine4_g64_pair_impl<T, group_size, bits, false, false>(
              run_w,
              run_scales,
              run_biases,
              run_x0,
      ...
      if (run_len == 3) {
        if (gemma4_sb_interleaved) {
          qmv_affine4_g64_triple_stream_impl<T, group_size, bits, false, true>(
              run_w,
              run_scales,
              run_biases,
              run_x0,
          ...
          qmv_affine4_g64_triple_stream_impl<T, group_size, bits, false, false>(
              run_w,
              run_scales,
              run_biases,
              run_x0,
      ...
      if (gemma4_sb_interleaved) {
        ...
        qmv_affine4_g64_quad_stream_impl<T, group_size, bits, false>(
            run_w,
            run_scales,
            run_biases,
            run_x0,
    ...
    } else if (gemma4_sb_interleaved) {
      qmv_impl<T, group_size, bits, false, true>(
          single_w, single_scales, single_biases, single_x, single_y,
          in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
    } else {
