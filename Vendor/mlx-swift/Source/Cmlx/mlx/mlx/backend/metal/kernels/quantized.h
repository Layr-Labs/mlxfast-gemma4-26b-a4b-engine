...
// Eight-row weight-stream sharing for ordinary affine4/g64 QMV. Same body as
// qmv_affine4_g64_quad_stream_impl: two simdgroups by four output rows, values
// per thread = 8, block_size = 256, scale_step_per_thread = 8, one packed
// weight + scale/bias fetch per K-block, then input rows walk a SINGLE
// eight-value x buffer. qdot_affine4_registered keeps the bits==4 qdot
// accumulation order; only the streamed row count grows from 4 to 8.
template <typename T, const int group_size, const int bits>
...
[[kernel]] void affine_qmv(
  ...
  if (!batched && group_size == 64 && bits == 4 && ntg.x == 8 &&
    ...
    if (out_vec_size >= 1024) {
      // WIDE-N oct-stream: one packed-weight stream feeds all eight cohort
      // rows in a single active x-group (first_m = tid.x * 8). Remaining host
      // groups return. Per-row qdot, K-loop and simd_sum stay the stock
      // qmv_impl sequence -- only loads are shared. Floor 1024 keeps
      // router.proj (N = 128) on the pair kernel byte-for-byte.
