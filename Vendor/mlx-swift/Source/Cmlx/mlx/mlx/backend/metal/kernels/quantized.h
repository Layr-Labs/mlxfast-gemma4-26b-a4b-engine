  ...
  for (; k < in_vec_size - block_size; k += block_size) {
    ...
    for (int row = 0; row < results_per_simdgroup; row++) {
      ...
      qdot_affine4_quad<float, values_per_thread>(
          wl, x0_thread, x1_thread, x2_thread, x3_thread,
          sl[0], bl[0], sum0, sum1, sum2, sum3,
          dot0, dot1, dot2, dot3);
      result0[row] += dot0;
      result1[row] += dot1;
      result2[row] += dot2;
...
[[kernel]] void affine_qmv(
  ...
  if (!batched && group_size == 64 && bits == 4 && ntg.x == 8 &&
    ...
    // Claim adjacent rows in two active x-groups of 4 rows each and let the
    // remaining 6 host groups return. The quad helper shares each packed-weight
    // load across 4 rows while preserving each row's qdot, K-loop, and simd_sum order.
