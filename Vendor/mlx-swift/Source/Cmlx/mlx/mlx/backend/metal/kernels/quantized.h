...
METAL_FUNC void gemma4_qmv_mma8_affine4_g64_impl(
  ...
  // SINK: one B fragment, rebuilt immediately before the accumulate that
  // reads it, instead of eight built up front and all held live across the
  // whole step chain. An fp32 8x8 fragment is two floats per lane, so the
  // eight cost sixteen registers of the block's live set to cache values each
  // of which is wanted exactly once. The eight accumulates already chain
  // through C, so no step gains a dependency it did not have.
