...
inline T gemma4_geglu_compiled_tape(T gate, T up) {
  // bf16 tanh has rounded to +/-1 by this conservative gate threshold.
  // Retain the typed product and signed-zero tape; -infinity uses the
  // original arithmetic because its half_gate * zero is NaN.
  if constexpr (metal::is_same_v<T, bfloat>) {
    const float gate_f = float(gate);
    if (gate_f >= 3.125f) {
      return static_cast<T>(gate * up);
    }
    if (gate_f <= -3.125f && metal::isfinite(gate_f)) {
      const T half_gate = static_cast<T>(static_cast<T>(0.5f) * gate);
      const T gelu = static_cast<T>(half_gate * static_cast<T>(0.0f));
  ...
  const T curved = (metal::is_same_v<T, bfloat> && metal::isfinite(float(gate)))
      ? metal::fast::tanh(scaled) : metal::precise::tanh(scaled);
