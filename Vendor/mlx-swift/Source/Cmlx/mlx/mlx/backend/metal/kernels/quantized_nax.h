...
struct QuantizedBlockLoader {
  ...
  struct PrefetchData {
    uint32_t w[(BCOLS_PACKED * BROWS < tgp_size) ? 1 : (BCOLS_PACKED * BROWS) / tgp_size];
    T scale;
    T bias;
    bool valid;
  };
  ...
  PrefetchData prefetch_unsafe() const {
    ...
    d.scale = *scales;
    d.bias = *biases;
    auto src_u32 = (const device uint32_t*)src;
    STEEL_PRAGMA_UNROLL
    for (int i = 0; i < n_reads; i++) {
      d.w[i] = src_u32[i];
    }
  ...
  void commit_unsafe(const thread PrefetchData& d) const {
    ...
    for (int i = 0; i < n_reads; i++) {
      dequantize<T, pack_factor, bits>(
          (const thread uint8_t*)&d.w[i], d.scale, d.bias, dst + i * pack_factor);
    }
  }
  ...
  while (n < tgp_bm) {
    ...
    dispatch_bool(align_M || !is_unaligned_sm, [&](auto kAlignedM) {
      dispatch_bool(align_N || !is_unaligned_bn, [&](auto kAlignedN) {
        if constexpr (DARKBLOOM_GEMMA4_GATHER_PREFETCH != 0) {
