...
template <typename T, int M, int N, class MMAFrag_t>
METAL_FUNC void tile_matmad(
    thread MMATile<T, M, N, MMAFrag_t>& D,
    thread MMATile<T, M, 1, MMAFrag_t>& A,
    thread MMATile<T, 1, N, MMAFrag_t>& B,
    thread MMATile<T, M, N, MMAFrag_t>& C) {
  STEEL_PRAGMA_UNROLL
  for (short i = 0; i < M; ++i) {
    STEEL_PRAGMA_UNROLL
    for (short j = 0; j < N; ++j) {
      MMAFrag_t::mma(
          D.frag_at(i, j), A.frag_at(i, 0), B.frag_at(0, j), C.frag_at(i, j));
    }
  }
}
