...
> The LIVE ranked track is DFlash
...
> window), on the ranked self-hosted runner for that track. For the live window
> and scoring see
...
- `prefillBandUpTolerance = 0.03`, `prefillBandDownTolerance = 0.03`.
- `decodeBandUpTolerance = 0.01`, `decodeBandDownTolerance = 0.025`.

Acceptance bands (see `AcceptanceBand`): prefill and decode are single noisy
...
- **Prefill: +/-3% symmetric.** Prefill is not a real optimization axis here, so
  it is a health gate -- both a regression and a lucky-fast reading past 3% fail.
- **Decode: +1% regression / -2.5% gain.** Decode is the axis the score rewards, so
  the up (regression) side is tight at +1%; the down (gain) side caps a *single
  submission's* decode improvement at 2.5%. Larger wins are still welcome -- they
...
The magnitudes are derived from the gemma4 2026-08-25 box-3 calibration
session (band shape preserved from the qwen-era precedent; tight side =
ceil-to-half-percent of 10x the per-axis session CV, decode down side =
qwen's 2.5x down:up asymmetry ratio; never looser than the qwen absolutes,
never tighter than 4x the measured session spread). The full derivation is
the BAND DERIVATION comment block in `Sources/MLXFastCore/Constants.swift`.
...
the ranked score denominator is the live paired baseline measured on the
ranked box (next section), never these numbers. The values below are the
Gemma 4 26B A4B 2026-08-25 calibration: the mean prefill/decode
seconds-per-token from four consecutive cool-gated (40C, read via `macmon`)
runs of `./benchmark.sh --local-iterate` on the stock committed tree, on
box 3 -- the ranked box for this track per David's 2026-08-25 ruling --
with a fresh worker per phase and zero warmup, at engine `dec515a5` /
benchd `c2327d15` (benchctl built from that same benchd commit), against
the 1024_1024 public local-submit golden (sha256
`36290b93b1445f354b9b8e3d5ba592976830b40dd924324f822ec55a87140be4`, 29576
bytes). Decode samples ranged
`0.0123643369140625`-`0.012390104164062499` (CV 0.0937%, spread 0.2084%);
prefill samples ranged `0.000326931234375`-`0.00032883076953125` (CV
0.2712%, spread 0.581%). Evidence directory:
`calibration-20260825T102741Z`:
...
The ranked runner is a self-hosted Apple M5 Max; active ranked capacity is one
box. The v2 paired baseline at the on-box baseline workspace
`laguna-xs-2.1-serial-v2/current` is built from
...
- The baseline sample is additionally sanity-checked against the box's
  ...
  (the on-box state file `laguna-xs-2.1-serial-v2/baseline-calibration.json`),
