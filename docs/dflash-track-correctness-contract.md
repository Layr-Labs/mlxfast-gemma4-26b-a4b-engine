...
Measurement (reference box, 2026-07-30): 6 diverse prompts x K in {4,8,16} x 128 tokens
via `mlx-bench dflash --parity-check --parity-top-k 5`
(log: the operator account's `dflash-parity-stats.log`):
...
re-runs the target" must mean a SECOND PROCESS:
  ...
  ${MLXFAST_BASELINE_WS} (the on-box baseline workspace <track>/current) at the
...
absolute 0.75 / relative 0.02, pending calibration on the reference box by the same method
...
honest candidate-vs-reference logit deltas on the reference box, sized with headroom over
...
# Validation record (2026-07-30, reference box)

What has actually been executed, as opposed to written. Everything below ran on
the reference box against the organizer-provisioned target
(the on-box target cache `dflash/laguna-xs-2.1-dflash-v1/target`, 13/13 hardlinks
...
The first measurement ever taken at a long prompt (1755 tokens, K=8, reference box,
...
produced three findings that change the contract. All numbers below are from the reference box,
...
Every run ever recorded on the reference box was audited against that rule using the
...
under 4 x 0.01303 = 0.052 s. Measured on the reference box at the ranked window:
...
## Measured on the reference box, `fix3-golden.json` (varied prose, 512-token seed)
...
Re-measured under live post-run replay of the candidate's own chain. Reference box,
...
## Measured after both changes (reference box, live replay, `--tokens 128`)
...
The DFlash go-live runbook's step B — the step the ranked workflow's own
...
On the ranked hidden golden, measured end-to-end on the reference box at the frozen window
...
the on-box `measure-dflash-job.sh` on every serving box — the only site
...
the on-box state file `laguna-xs-2.1-dflash-v1/baseline-calibration.json`,
...
## Measured at the actual ranked window (512 decode tokens, ranked hidden golden, reference box)
...
K=2, 512 decode tokens, ranked hidden golden, reference box, 2026-07-31 — three
...
The pool goldens were generated on the reference box from operator-independent public-domain
...
entry), from the Amendment 30 pool-characterisation campaign on the ranked
self-hosted runner for that track:
...
the on-box `measure-dflash-job.sh` was previously pinned EQUAL to the
...
per entry); a confirming 4-pair re-measurement on the ranked self-hosted runner
for that track is the recommended
