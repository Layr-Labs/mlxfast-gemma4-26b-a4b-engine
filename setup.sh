#!/usr/bin/env bash
# Bootstrap system tools and build the Swift-only Gemma 4 26B A4B harness.
set -euo pipefail

# REPOINTED 2026-08-24 (lane/gemma4-setup-repoint) from the Qwen 3.8 27B
# backbone this script downloaded before the repoint onto the Gemma 4 26B A4B
# MLX QAT 4-bit checkpoint, in lockstep with
# MLXFastConstants.referenceModelRepository / _Revision (already gemma-pointed
# in Sources/MLXFastCore/Constants.swift), Tests/MLXFastTests/Model/
# Gemma4A4BArtifactFixtureSupport.swift's gemma4A4BRepository /
# gemma4A4BRevision, and fixtures/gemma4_26b_a4b_track.json's
# target.upstream_model_id / target.upstream_revision -- four independent
# sources that now agree byte-for-byte on this pin.
#
# UNLIKE THE RETIRED QWEN BACKBONE, this checkpoint is NOT our own conversion:
# it is fetched directly from the published mlx-community QAT 4-bit release,
# the same checkpoint darkbloom (Layr-Labs/d-inference) actually serves
# (docs/gemma4-port-notes.md section 1). The repository is PUBLIC, so the
...
REFERENCE_MODEL_REPO="${MLXFAST_REFERENCE_MODEL_REPO:-mlx-community/gemma-4-26B-A4B-it-qat-4bit}"
REFERENCE_REVISION="${MLXFAST_REFERENCE_REVISION:-0e3cbab38ce568cf6e23543010d08d03b731910c}"
...
DEFAULT_REFERENCE_BASE_URL="https://huggingface.co/mlx-community/gemma-4-26B-A4B-it-qat-4bit/resolve/${REFERENCE_REVISION}"
...
REFERENCE_MANIFEST_PATH="${MLXFAST_REFERENCE_MANIFEST_PATH:-fixtures/reference_gemma4_26b_a4b_qat4bit.sha256}"
...
# 40 GiB covers the ~14.6 GiB pinned snapshot (15641239658 bytes across 11
# manifest records) plus headroom for the text-tower transform's output
# alongside it -- unchanged from the retired Qwen backbone's own default,
# which sized the same 40 GiB floor for a checkpoint of nearly identical
# total size (15153237117 bytes, 10 records).
...
# the delegated run setup-gemma4-assistant.sh drives, where REFERENCE_DIR is the
...
# direct ./setup.sh run -- but this downloader is also driven as a subprocess by
# setup-gemma4-assistant.sh and setup-gemma4-dflash.sh, and those runs used to
# emit a solid wall of "setup.sh:" lines under a command the reader never
# invoked, with the stager's own two lines lost at either end. The stagers set
# this to their own name so the transcript names the command that produced it.
# Cosmetic only: nothing about what is downloaded, verified or written depends
# on it. FAIL CLOSED on a label that is not a plain command name, so a stray
# value cannot inject shell-looking text into every line of a setup transcript.
...
DEFAULT_REFERENCE_DIR="reference_weights/gemma4-26b-a4b-qat4bit"
...
builds mlx.metallib, and downloads the Gemma 4 26B A4B MLX QAT 4-bit reference
...
Important environment variables:
  ...
  MLXFAST_SETUP_SUMMARY_ROLE         Which artifact this run provisioned, which
                                     ...
                                     'mtp-head' by setup-gemma4-assistant.sh, where the
  ...
  MLXFAST_SETUP_LOG_LABEL            Command name this run prefixes its output
                                     ...
                                     the stagers that drive this downloader as a
  ...
  MLXFAST_SKIP_SWIFT_BUILD=1         Reuse the Swift products from a previous
                                     ...
                                     anyway. Used by setup-gemma4-assistant.sh, whose
...
After setup:
  ...
  # Benchmarking/grading lives in benchd, a PINNED PREBUILT: ./tools/fetch-benchd.sh
EOF
}
...
print_setup_summary() {
  ...
  case "${SETUP_SUMMARY_ROLE}" in
    ...
    mtp-head)
      ...
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: MLXFAST_SETUP_SUMMARY_ROLE must be 'target' or 'mtp-head'" >&2
      return 1
      ;;
  esac
...
${SETUP_LOG_LABEL}: summary
  ...
  benchd worker: $(dirname "${SWIFT_BIN}")/mlxfast-runtime-worker (+ sibling mlx.metallib)
...
EOF
  ...
  next:
    ...
    run the paired benchmarker via ./benchmark.sh (benchd is a pinned prebuilt; ./tools/fetch-benchd.sh resolves it)
EOF
    return 0
  fi
...
# (fixtures/reference_gemma4_26b_a4b_qat4bit.sha256) pins README.md,
# chat_template.jinja, config.json, generation_config.json, the index,
# processor_config.json (vision-tower preprocessor config; unused by the
# text-only transform but published and downloaded like everything else) and
# tokenizer.json/tokenizer_config.json -- 11 records, NO .gitattributes. The
# assistant (fixtures/gemma4_assistant.sha256, which setup-gemma4-assistant.sh
# points MLXFAST_REFERENCE_MANIFEST_PATH at) pins the same shape minus
# processor_config.json and with a single model.safetensors shard instead of
# three -- 8 records, also NO .gitattributes; see that manifest's header for
# why its tokenizer files are pinned even though the loader never reads them.
#
# A hard-coded list was wrong for both of them, in both directions: it demanded
# .gitattributes neither repository's manifest pins (a fresh ./setup.sh died on
...
#
# MULTI-SHARD (the 26B target): upstream publishes
...
# tensors live where.
#
# SINGLE-SHARD (the DFlash drafter): upstream publishes NO index, because a
# one-file checkpoint has nothing to index. `z-lab/gemma-4-26B-A4B-it-DFlash`
# ships exactly `config.json` + `model.safetensors`, and
# fixtures/gemma4_dflash_drafter.sha256 pins exactly those two. The three
# callers used to hard-require the index regardless, so staging the drafter
# died on "missing model.safetensors.index.json" for a file that does not exist
# upstream and never will -- which is why tools/stage-ranked-heads.sh could not
# stage it, and therefore why the ranked DFlash arm could not run at all.
#
...
build_swift_harness() {
  ...
  # setup-gemma4-assistant.sh delegates its download to this script, and that delegated
  # run would otherwise repeat the whole SwiftPM build -- ~25 seconds of
  # no-op relinking on the normal `./setup.sh && ./setup-gemma4-assistant.sh` path,
  ...
  # already present and executable. A standalone ./setup-gemma4-assistant.sh on a fresh
  ...
  if [[ "${MLXFAST_SKIP_SWIFT_BUILD:-0}" == "1" ]]; then
    if ensure_swift_harness_ready 2>/dev/null; then
      echo "${SETUP_LOG_LABEL}: MLXFAST_SKIP_SWIFT_BUILD=1 and both products are present; reusing ${SWIFT_BIN} and ${RUNTIME_WORKER_BIN}"
      return 0
    fi
    echo "${SETUP_LOG_LABEL}: MLXFAST_SKIP_SWIFT_BUILD=1 but a product is missing; building anyway"
  fi
  echo "${SETUP_LOG_LABEL}: building trusted Swift harness and participant runtime worker"
  ...
  if [[ ! -x "${RUNTIME_WORKER_BIN}" ]]; then
    echo "${SETUP_LOG_LABEL}: participant runtime worker missing at ${RUNTIME_WORKER_BIN}; build failed or MLXFAST_RUNTIME_WORKER_EXECUTABLE is wrong" >&2
    return 1
  fi
}
...
stage_runtime_worker_for_benchd() {
  # The worker builds into its isolated scratch root (.build-worker/release) with
  # mlx.metallib beside it, but benchd resolves the worker at .build/release and
  ...
  # exist. See tools/stage-runtime-worker.sh for the isolation rationale.
  echo "${SETUP_LOG_LABEL}: staging the scored runtime worker + mlx.metallib for benchd"
  tools/stage-runtime-worker.sh || return 1
}
...
download_reference_weights_locked() {
  ...
  # Single-shard checkpoints (the DFlash drafter) publish no index upstream, so
  # the shard list comes from the pin instead. See resolve_reference_shard_list.
  if ! shard_list="$(resolve_reference_shard_list "${reference_dir}")"; then
    return 1
  fi
  start_mlx_metallib_build || return 1
