...
do_boost() {
  ...
  # (the benchmarker's offer checks `status` before calling boost, so on the
  ...
  if [[ -n "${manual_fans}" ]]; then
    ...
    echo "fan-control.sh: refusing to overwrite its targets. Restore automatic control first with: tools/fan-control.sh normal (or stop the other fan tool), then retry" >&2
    exit 1
  fi
  explain_sudo
  for (( i = 0; i < fan_count; i++ )); do
  ...
  sudo -k
  echo "fan-control.sh: fan boost active; restore automatic control with: tools/fan-control.sh normal" >&2
}
